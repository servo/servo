...
// TODO(pcwalton): Split this into `InternalCanvasMsg` and `ExternalCanvasMsg`, with only
// `InternalCanvasMsg` deriving `Deserialize`/`Serialize`.
#[derive(Clone)]
...
impl Serialize for FromLayoutMsg {
    fn serialize<S>(&self, _: &mut S) -> Result<(),S::Error> where S: Serializer {
        panic!("can't serialize a `FromLayoutMsg`!")
    }
}

impl Deserialize for FromLayoutMsg {
    fn deserialize<D>(_: &mut D) -> Result<FromLayoutMsg,D::Error> where D: Deserializer {
        panic!("can't deserialize a `FromLayoutMsg`!")
    }
}
