...
struct HighlightTraversalState {
    ...
    maybe_box_fragment: Option<(ArcRefCell<BoxFragment>, PhysicalRect<Au>)>,
}
...
impl DisplayListBuilder<'_> {
    ...
    fn paint_dom_inspector_highlight(&mut self, highlight: HighlightTraversalState) {
        ...
        if let Some((box_fragment, containing_block)) = highlight.maybe_box_fragment {
...
impl InspectorHighlight {
    fn register_fragment_of_highlighted_dom_node(
        ...
        debug_assert_eq!(clip_chain_id, state.clip_chain_id);

        let fragment_relative_rect = match fragment {
            Fragment::Box(fragment) | Fragment::Float(fragment) => {
                state.maybe_box_fragment = Some((fragment.clone(), *containing_block));
