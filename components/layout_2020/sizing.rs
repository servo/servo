...
pub(crate) fn outer_inline(
    ...
    let content_size = LazyCell::new(|| {
        ...
        .map(|block_size| {
            let min_block_size = content_min_size
                .block
                .maybe_resolve_extrinsic(available_block_size)
                .unwrap_or(auto_minimum.block);
            let max_block_size = content_max_size
                .block
                .maybe_resolve_extrinsic(available_block_size);
    ...
    let (min_min_content, min_max_content, min_depends_on_block_constraints) =
        resolve_non_initial(content_min_size.inline)
            .unwrap_or_else(|| (auto_minimum.inline, auto_minimum.inline, false));
    let (max_min_content, max_max_content, max_depends_on_block_constraints) =
        resolve_non_initial(content_max_size.inline).map_or(
            (None, None, false),
            |(min_content, max_content, depends_on_block_constraints)| {
                (
                    Some(min_content),
                    Some(max_content),
