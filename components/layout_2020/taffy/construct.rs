...
impl TaffyContainer {
    pub fn construct<'dom>(
        ...
        let mut builder = TaffyContainerBuilder {
            context,
            info,
...
struct TaffyContainerBuilder<'a, 'dom, Node> {
...
impl<'a, 'dom, Node: 'dom> TraversalHandler<'dom, Node> for TaffyContainerBuilder<'a, 'dom, Node>
where
    Node: NodeExt<'dom>,
{
...
impl<'a, 'dom, Node: 'dom> TaffyContainerBuilder<'a, 'dom, Node>
where
    Node: NodeExt<'dom>,
{
    ...
    fn finish(mut self) -> TaffyContainer {
        self.wrap_any_text_in_anonymous_block_container();
        ...
        let mut children = std::mem::take(&mut self.jobs)
            ...
            .filter_map(|job| match job {
                FlexLevelJob::TextRuns(runs) => {
                    ...
                    Some(ArcRefCell::new(TaffyItemBox::new(
                        TaffyItemBoxInner::InFlowBox(IndependentFormattingContext::NonReplaced(
                            non_replaced,
                        )),
                    )))
                },
        ...
        children.sort_by_key(|child| match &child.borrow().taffy_level_box {
            TaffyItemBoxInner::InFlowBox(item) => item.style().clone_order(),
            ...
            TaffyItemBoxInner::OutOfFlowAbsolutelyPositionedBox(_) => 0,
        });

        TaffyContainer { children }
    }
}
