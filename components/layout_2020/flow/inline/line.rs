...
impl LineItemLayoutInlineContainerState {
    fn new(
        ...
        Self {
            ...
            parent_offset,
            baseline_offset,
            had_any_line_items: false,
            saw_start_padding_border_margin: false,
            saw_end_padding_border_margin: false,
...
impl<'a> LineItemLayout<'a> {
    ...
    fn prepare_layout_for_inline_box(&mut self, new_inline_box: Option<InlineBoxIdentifier>) {
        ...
        while let Some(token) = path.next() {
            match token {
    ...
    pub(super) fn layout(&mut self, iterator: &mut IntoIter<LineItem>) -> Vec<Fragment> {
        while let Some(item) = iterator.next() {
            ...
            self.state.had_any_line_items = true;
            match item {
                LineItem::StartInlineBoxPaddingBorderMargin(_) => {
                    self.state.saw_start_padding_border_margin = true;
                },
                LineItem::EndInlineBoxPaddingBorderMargin(_) => {
                    self.state.saw_end_padding_border_margin = true;
                },
    ...
    fn start_inline_box(&mut self, identifier: &InlineBoxIdentifier) {
        ...
        let positioning_context_or_start_offset_in_parent =
            match PositioningContext::new_for_style(&style) {
                ...
                None => Either::Second(self.current_positioning_context_mut().len().into()),
            };
    ...
    fn end_inline_box(&mut self) {
        ...
        let inner_state = std::mem::replace(&mut self.state, outer_state);
        ...
        if !inner_state.had_any_line_items && inner_state.fragments.is_empty() {
            return;
        }

        let mut padding = inline_box_state.pbm.padding.clone();
        let mut border = inline_box_state.pbm.border.clone();
        let mut margin = inline_box_state.pbm.margin.auto_is(Au::zero);
        if !inner_state.saw_start_padding_border_margin {
            padding.inline_start = Au::zero();
            border.inline_start = Au::zero();
            margin.inline_start = Au::zero();
        }
        if !inner_state.saw_end_padding_border_margin {
        ...
        let mut content_rect = LogicalRect {
            ...
            size: LogicalVec2 {
                ...
                block: inline_box_state.base.font_metrics.line_gap.into(),
            },
        };
        ...
        self.state.inline_advance += inner_state.inline_advance + pbm_sums.inline_sum().into();
    ...
    fn calculate_inline_box_block_start(
        ...
        match inline_box_state.base.style.clone_vertical_align() {
            ...
            GenericVerticalAlign::Keyword(VerticalAlignKeyword::Bottom) => {
                ...
                Au::from(self.line_metrics.block_size) - line_height + half_leading
            },
            _ => {
    ...
    fn layout_text_run(&mut self, text_item: TextRunLineItem) {
        ...
        self.state.fragments.push(Fragment::Text(TextFragment {
            ...
            justification_adjustment: self.justification_adjustment.into(),
        }));
    }
    ...
    fn layout_atomic(&mut self, mut atomic: AtomicLineItem) {
        ...
        self.state.inline_advance += atomic.size.inline.into();
