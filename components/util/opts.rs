...
pub struct Opts {
    ...
    /// If true, instrument the runtime for each thread created and dump
    /// that information to a JSON file that can be viewed in the thread
    /// profile viewer.
...
pub struct DebugOptions {
    ...
    /// Instrument each thread, writing the output to a file.
    pub profile_threads: bool,
    ...
    pub fn new(debug_string: &str) -> Result<DebugOptions, &str> {
        ...
        for option in debug_string.split(',') {
            match option {
                ...
                "profile-threads" => debug_options.profile_threads = true,
...
pub fn print_debug_usage(app: &str) -> ! {
    ...
    print_option("profile-threads", "Instrument each thread, writing the output to a file.");
...
pub fn from_cmdline_args(args: &[String]) {
    ...
    let opts = Opts {
        ...
        profile_threads: debug_options.profile_threads,
