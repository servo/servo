...
pub fn read_numbers<I>(iter: &mut Peekable<I>) -> Option<i64>
    ...
    iter.take_while_ref(is_ascii_digit).map(|d| {
        d as i64 - '0' as i64
...
pub fn read_fraction<I>(iter: &mut Peekable<I>, mut divisor: f64, value: &mut f64)
    where I: Iterator<Item=char> + Clone {
    ...
    *value = iter.take_while_ref(is_ascii_digit).map(|d|
        d as i64 - '0' as i64
    ).fold(*value, |accumulator, d| {
        divisor *= 10f64;
        accumulator + (d as f64) / divisor
    });
}

pub fn read_exponent<I>(iter: &mut Peekable<I>) -> Option<i32>
    where I: Iterator<Item=char> + Clone {
    ...
    read_numbers(iter).and_then(|value| value.to_i32().unwrap_or(0).checked_mul(exp_sign))
}
