...
struct PlayerInner {
    ...
    download_buffering_enabled: Arc<AtomicBool>,
}
...
impl PlayerInner {
    ...
    pub fn set_download_buffering_enabled(&mut self, enable: bool) -> Result<(), PlayerError> {
...
impl GStreamerPlayer {
    ...
    fn setup(&self) -> Result<(), PlayerError> {
        ...
        let download_buffering_enabled = Arc::new(AtomicBool::new(
            self.download_buffering_enabled.load(Ordering::Relaxed),
        ));
        let download_flag = download_buffering_enabled.clone();
...
impl Player for GStreamerPlayer {
    ...
    fn set_download_buffering_enabled(&self, enable: bool) -> Result<(), PlayerError> {
        ...
        // download_buffering_enabled flag before calling setup, so that the
        // "deep-element-added" signal handler can set the download property on
        // the urisourcebin element in time.
        self.download_buffering_enabled
            .store(enable, Ordering::Relaxed);
        self.setup()?;
        if let Some(inner) = self.inner.borrow().as_ref() {
            inner
                .lock()
                .unwrap()
                .set_download_buffering_enabled(enable)?;
        }
        Ok(())
    }
