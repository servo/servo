...
impl Pipeline {
    ...
    pub(crate) fn send_throttle_messages(&self, throttled: bool) {
        ...
        let message = ScriptThreadMessage::SetThrottled(self.id, throttled);
        if let Err(error) = self.event_loop.send(message) {
            warn!("Sending SetThrottled to script failed ({error}).");
        }
        let message = CompositorMsg::SetThrottled(self.webview_id, self.id, throttled);
        self.compositor_proxy.send(message);
    }
}
