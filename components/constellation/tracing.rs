    ...
    impl LogTarget for ConstellationMsg {
        fn log_target(&self) -> &'static str {
            event_tracing::log_target!(self {
                ...
                constellation<compositor@GetBrowsingContext(..),
                constellation<compositor@GetPipeline(..),
                constellation<compositor@GetFocusTopLevelBrowsingContext(..),
                constellation<compositor@IsReadyToSaveImage(..),
                constellation<compositor@Keyboard(..),
                constellation<compositor@AllowNavigationResponse(..),
                constellation<compositor@LoadUrl(..),
                constellation<compositor@ClearCache,
                constellation<compositor@TraverseHistory(..),
                constellation<compositor@WindowSize(..),
                constellation<compositor@TickAnimation(..),
                constellation<compositor@WebDriverCommand(..),
                constellation<compositor@Reload(..),
                constellation<compositor@LogEntry(..),
                constellation<compositor@NewWebView(..),
                constellation<compositor@CloseWebView(..),
                constellation<compositor@SendError(..),
                constellation<compositor@FocusWebView(..),
                constellation<compositor@BlurWebView,
                ...
                constellation<compositor@EnableProfiler(..),
                constellation<compositor@DisableProfiler,
                constellation<compositor@ExitFullScreen(..),
                constellation<compositor@MediaSessionAction(..),
                constellation<compositor@WebViewVisibilityChanged(..),
                constellation<compositor@IMEDismissed,
                constellation<compositor@ReadyToPresent(..),
                constellation<compositor@Gamepad(..),
            })
        }
    }
}
