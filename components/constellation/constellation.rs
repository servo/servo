    ...
    SWF: ServiceWorkerManagerFactory,
{
    ...
    fn update_browsing_context(
        ...
        if let Some(new_pipeline) = self.pipelines.get_mut(&new_pipeline_id) {
            ...
            if !is_hidden {
                new_pipeline.send_throttle_messages(false);
            }
    ...
    fn root_pipeline_for_webview(&self, webview_id: WebViewId) -> Option<&Pipeline> {
        let browsing_context_id = BrowsingContextId::from(webview_id);
    ...
    fn set_webview_hidden(&mut self, webview_id: WebViewId, hidden: bool) {
        ...
        let Some(root_pipeline) = self.root_pipeline_for_webview(webview_id) else {
            warn!("Tried to hide WebView ({webview_id:?}) after closure");
            return;
        };
