...
use msg::constellation_msg::{NavigationDirection, PipelineNamespace, PipelineNamespaceId, WindowSizeType};
            ...
            if let Some(parent_pipeline) = self.pipelines.get(&parent_pipeline_id) {
                if let Some(frame_id) = self.pipelines.get(&pipeline_id).and_then(|pipeline| pipeline.frame) {
                    if let Some(frame) = self.frames.get(&frame_id) {
                        ...
                        parent_pipeline.trigger_mozbrowser_event(Some(pipeline_id), event);
                    }
                }
            }
        }
