...
type PendingApprovalNavigations = HashMap<(PipelineId, ServoUrl), (LoadData, bool)>;
    ...
    STF: ScriptThreadFactory<Message = Message>,
{
    ...
    fn handle_request_from_compositor(&mut self, message: FromCompositorMsg) {
        ...
        match message {
            ...
            FromCompositorMsg::AllowNavigationResponse(
                top_level_browsing_context_id,
                pipeline_id,
                url,
                allowed,
            ) => {
                let key = (pipeline_id, url.clone());
                let pending = self.pending_approval_navigations.remove(&key);
                match pending {
    ...
    fn schedule_navigation(
        ...
        let key = (source_id, load_data.url.clone());
        match self.pending_approval_navigations.entry(key) {
            Entry::Occupied(_) => {
                return warn!(
