          ...
          STF: ScriptThreadFactory<Message=Message>
{
    ...
    fn joint_session_future<'a>(&'a self, frame_id_root: FrameId) -> impl Iterator<Item = &'a FrameState> + 'a {
        struct SortableFrameState<'b>(&'b FrameState);

        impl<'b> Ord for SortableFrameState<'b> {
            fn cmp(&self, other: &Self) -> Ordering {
                self.0.instant.cmp(&other.0.instant)
            }
        }

        impl<'b> PartialOrd for SortableFrameState<'b> {
            fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
                Some(self.cmp(other))
            }
        }

        impl<'b> PartialEq for SortableFrameState<'b> {
            fn eq(&self, other: &Self) -> bool {
                self.0.instant == other.0.instant
            }
        }

        impl<'b> Eq for SortableFrameState<'b> {}

        self.full_frame_tree_iter(frame_id_root)
            .map(|frame| frame.next.iter().rev().map(|frame_state| SortableFrameState(frame_state)))
            .kmerge()
            .map(|SortableFrameState(frame_state)| frame_state)
    }
    ...
    fn joint_session_past<'a>(&'a self, frame_id_root: FrameId) -> impl Iterator<Item = &'a FrameState> + 'a {
        struct SortableFrameState<'b>(Instant, &'b FrameState);
        ...
        self.full_frame_tree_iter(frame_id_root)
            .map(|frame| frame.prev.iter().rev().scan(frame.instant, |prev_instant, entry| {
                ...
                *prev_instant = entry.instant;
                Some(SortableFrameState(instant, entry))
            }))
            .kmerge()
            .map(|SortableFrameState(_, entry)| entry)
    }
