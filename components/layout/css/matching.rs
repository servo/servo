...
impl StyleSharingCandidate {
    ...
    fn new<'le, E: TElement<'le>>(element: &E) -> Option<StyleSharingCandidate> {
        ...
        let style = unsafe {
            match element.as_node().maybe_borrow_data_unchecked() {
                None => return None,
                Some(data_ref) => {
        ...
        let parent_style = unsafe {
            match parent_element.as_node().maybe_borrow_data_unchecked() {
                None => return None,
...
impl<'le, ConcreteElement> PrivateElementMatchMethods<'le, ConcreteElement>
    ...
    fn share_style_with_candidate_if_possible(&self,
        ...
        let parent_data: Option<&PrivateStyleData> = unsafe {
            parent_node.maybe_borrow_data_unchecked().map(|d| &*d)
        };
        match parent_data {
            Some(parent_data_ref) => {
                // Check parent style.
        ...
        for (i, &(ref candidate, ())) in style_sharing_candidate_cache.iter().enumerate() {
            match self.share_style_with_candidate_if_possible(parent.clone(), candidate) {
                Some(shared_style) => {
                    ...
                    let style = &mut node.mutate_data().style;
...
impl<'ln, ConcreteNode> MatchMethods<'ln, ConcreteNode>
    ...
    unsafe fn cascade_node(&self,
        ...
        let parent_style = match parent {
            None => None,
            Some(parent_node) => {
                let parent_style = (*parent_node.borrow_data_unchecked()).style.as_ref().unwrap();
                Some(parent_style)
            }
        };

        let mut data_ref = self.mutate_data();
