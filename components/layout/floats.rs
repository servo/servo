...
impl SpeculatedFloatPlacement {
    ...
    pub fn compute_floats_out(&mut self, flow: &mut Flow) {
        if flow.is_block_like() &&
                flow.as_block().formatting_context_type() != FormattingContextType::None {
            *self = flow::base(flow).speculated_float_placement_in
        }
    ...
    pub fn compute_floats_in_for_first_child(parent_flow: &mut Flow) -> SpeculatedFloatPlacement {
        if parent_flow.is_block_like() &&
                parent_flow.as_block().formatting_context_type() != FormattingContextType::None {
            SpeculatedFloatPlacement::zero()
        } else {
            flow::base(parent_flow).speculated_float_placement_in
        }
    }
}
