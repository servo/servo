...
/// Some childern node of a given DOM node do not have `REBUILD` restyle damage.
...
impl PreviouslyBuiltChildernMatcher {
    ...
    fn match_and_advance<'dom>(
        ...
        // the node has `REBUILD` restyle damage, to find the box produced by
        // current given node.
        loop {
            let curr = self.next()?;
...
impl<'dom, 'style> BlockContainerBuilder<'dom, 'style> {
    ...
    fn previously_built_children_matcher(&mut self) -> &mut PreviouslyBuiltChildernMatcher {
        self.previously_built_children_matcher
            .as_mut()
            .expect("Reconstructing block container, but require repairing")
    }
...
impl<'dom> TraversalHandler<'dom> for BlockContainerBuilder<'dom, '_> {
    ...
    fn need_clear_pseudo_element_box(&self, node: &ServoLayoutNode<'dom>) -> bool {
        ...
        if damage.contains(RestyleDamage::REBUILD) {
            return true;
        }
...
impl<'dom> BlockContainerBuilder<'dom, '_> {
    fn try_reuse_block_level_box(
        ...
        if self.previously_built_children_matcher.is_none() {
            return None;
        }

        if info.get_restyle_damage().contains(RestyleDamage::REBUILD) {
            return None;
        }

        self.previously_built_children_matcher()
            .match_and_advance(info)
    }
    ...
    fn handle_inline_level_element(
        ...
        self.ensure_inline_formatting_context_builder()
            .start_inline_box(InlineBox::new(info, propagated_data), None);
    ...
    fn handle_block_level_element(
        ...
        if let Some(reused_block_level_box) = self.try_reuse_block_level_box(info) {
            if info.get_restyle_damage().contains(RestyleDamage::REPAIR) {
                self.block_level_boxes
                    .push(BlockLevelJob::Repair(BlockLevelRepairJob {
                        info: info.clone(),
                        repaired_box: reused_block_level_box,
    ...
    pub(crate) fn repair<'dom>(
        ...
        match self {
            ...
            BlockLevelBox::OutOfFlowAbsolutelyPositionedBox(positioned_box) => positioned_box
                .borrow_mut()
                .context
                .repair(context, info, repaired_contents, display_inside),
        ...
        let block_level_box = match self.kind {
            BlockLevelCreator::SameFormattingContextBlock(intermediate_block_container) => {
                ...
                ArcRefCell::new(BlockLevelBox::SameFormattingContextBlock {
                    base: LayoutBoxBase::new(info.into(), info.style.clone(), self.propagated_data),
                    contents,
                    contains_floats,
                })
            },
...
impl BlockLevelJob<'_> {
    fn finish(self, context: &LayoutContext) -> ArcRefCell<BlockLevelBox> {
        match self {
            BlockLevelJob::Copy(copied_box) => copied_box,
