...
impl FragmentDisplayListBuilding for Fragment {
    ...
    fn build_display_list_for_text_fragment(&self,
        ...
        let color = if let Some(shadow) = text_shadow {
            // If we're painting a shadow, paint the decorations and the text the same color as the shadow.
            self.style().resolve_color(shadow.color)
        } else {
            // Otherwise, paint the decorations and the text the same color as the text color.
            text_color
        };
