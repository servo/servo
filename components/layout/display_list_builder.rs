...
fn establishes_containing_block_for_absolute(can_establish_containing_block: EstablishContainingBlock,
                                             ...
                                             -> bool {
    can_establish_containing_block == EstablishContainingBlock::CanEstablish &&
...
impl BlockFlowDisplayListBuilding for BlockFlow {
    ...
    fn setup_clipping_for_block(&mut self,
        ...
        if can_establish_containing_block == EstablishContainingBlock::CanEstablish {
    ...
    fn setup_scroll_root_for_position(&mut self,
        ...
        let (left, right, top, bottom) = match self.sticky_position() {
            (MaybeAuto::Auto, MaybeAuto::Auto, MaybeAuto::Auto, MaybeAuto::Auto) => return,
            positions => positions,
        };
        ...
        let to_sticky_info = |margin: MaybeAuto, constraint_edge: Au, moving_edge: Au|
                             -> Option<StickySideConstraint> {
            match margin {
                MaybeAuto::Auto => None,
                MaybeAuto::Specified(value) => {
                    Some(StickySideConstraint {
                        margin: value.to_f32_px(),
                        max_offset: (constraint_edge - moving_edge).to_f32_px(),
                    })
                }
            }
        };

        let top = to_sticky_info(top, constraint_rect.max_y(), border_box_in_parent.max_y());
        let bottom = to_sticky_info(bottom, constraint_rect.min_y(), border_box_in_parent.min_y() - margins.top);
        let left = to_sticky_info(left, constraint_rect.max_x(), border_box_in_parent.max_x());
        let right = to_sticky_info(right, constraint_rect.min_x(), border_box_in_parent.min_x() - margins.left);

        let sticky_frame_info = StickyFrameInfo::new(top, right, bottom, left);
...
impl InlineFlowDisplayListBuilding for InlineFlow {
    fn collect_stacking_contexts_for_inline(&mut self, state: &mut DisplayListBuildState) {
        ...
        for fragment in self.fragments.fragments.iter_mut() {
            ...
            if establishes_containing_block_for_absolute(EstablishContainingBlock::CanEstablish,
