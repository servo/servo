...
fn convert_gradient_stops(gradient_stops: &[ColorStop],
    ...
    let mut stops = Vec::with_capacity(gradient_stops.len());
            ...
            None => {
                if stop_run.is_none() {
                    ...
                    let start_offset = if i == 0 {
                        ...
                        position_to_offset(gradient_stops[i - 1].position.unwrap(), length)
                    };
                    let (end_index, end_offset) =
                        match gradient_stops[i..]
                                        .iter()
                                        .enumerate()
                                        .find(|&(_, ref stop)| stop.position.is_some()) {
                            None => (gradient_stops.len() - 1, 1.0),
