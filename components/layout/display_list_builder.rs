...
use geom::{Point2D, Point3D, Rect, Size2D, SideOffsets2D};
use geom::Matrix4;
...
impl FragmentDisplayListBuilding for Fragment {
    ...
    fn create_stacking_context(&self,
        ...
        match self.style().get_effects().perspective {
            LengthOrNone::Length(d) => {
                ...
                perspective = pre_transform.mul(&perspective_matrix).mul(&post_transform);
            }
            LengthOrNone::None => {}
        }
...
impl BlockFlowDisplayListBuilding for BlockFlow {
    ...
    fn will_get_layer(&self) -> bool {
        // TODO(gw): The first condition below was previously valid as
        // this was only called from absolute block function. But now
        // it is called from static block too - so probably needs an
        // additional check.
