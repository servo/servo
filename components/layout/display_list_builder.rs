        ...
        if let Some(image) = image {
            ...
            let (mut origin_x, mut origin_y) = match background.background_origin {
                ...
                background_origin::T::content_box => {
                    let padding = style.logical_padding().to_physical(style.writing_mode);
                    let padding_left = model::specified(padding.left, bounds.size.width);
                    let padding_top = model::specified(padding.top, bounds.size.height);
