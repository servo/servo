...
pub trait FragmentDisplayListBuilding {
    ...
    /// Returns:
    ///
    /// 1. Rendering bounds. The background will start in the uppper-left corner
    ///    and fill the whole area.
    /// 2. Background tile size. Some backgrounds are repeated. These are the
    ///    dimensions of a single image of the background.
    /// 3. Spacing between tiles. Some backgrounds are not repeated seamless
    ///    but have seams between them like tiles in real life.
    /// 4. A clip area. While the background is rendered according to all the
...
impl FragmentDisplayListBuilding for Fragment {
                                        ...
                                        Rect<Au>)
    {
        ...
        return (bounds, tile_size, tile_spacing, css_clip);
    }
    ...
    fn build_display_list_for_webrender_image(&self,
        ...
        let (bounds, tile_size, spacing, css_clip) = self.compute_background_placement(
            state, style, absolute_bounds, Some(image), index);

        // Create the image display item.
        let base = state.create_base_display_item(&bounds,
                                                  LocalClip::Rect(css_clip.to_rectf()),
                                                  self.node,
                                                  style.get_cursor(Cursor::Default),
                                                  display_list_section);
    ...
    fn build_display_list_for_background_gradient(&self,
                                                  ...
                                                  index: usize)
    {
        let (bounds, tile_size, tile_spacing, css_clip) = self.compute_background_placement(
