            ...
            for operation in operations {
                match *operation {
                    transform::ComputedOperation::Translate(tx, ty, _) => {
                        let tx = model::specified(tx, self.base.block_container_inline_size);
                        let ty = model::specified(ty, self.base.block_container_explicit_block_size.unwrap_or(Au(0)));
                        clip = clip.translate(&Point2D::new(-tx, -ty))
                    }
                    _ => {}
                };
            }
