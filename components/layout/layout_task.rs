...
pub struct LayoutTaskData {
    pub sync: LayoutTaskSync,
    pub unsync: LayoutTaskUnsync,
}

pub struct LayoutTaskSync {
...
impl LayoutTaskUnsync {
    pub fn layout_root(&self) -> Option<FlowRef> {
        self.root_flow.clone()
    }
}
...
pub struct LayoutTask {
    ...
    /// A channel on which new animations that have been triggered by
    /// style recalculation can be sent.
...
impl LayoutTask {
    ...
    fn build_shared_layout_context<'a>(&self,
                                       rw_unsync: &LayoutTaskUnsync,
                                       rw_sync: &'a LayoutTaskSync,
                                       screen_size_changed: bool,
                                       reflow_root: Option<&LayoutNode>,
                                       url: &Url,
                                       goal: ReflowGoal)
                                       -> SharedLayoutContext<'a> {
        // FIXME: Building a shared layout context currently involves accessing
        // LayoutTaskUnsync, which means the data could become outdated.
        SharedLayoutContext {
            image_cache_task: self.image_cache_task.clone(),
            screen_size: rw_sync.screen_size.clone(),
            ...
            stylist: &rw_sync.stylist,
            url: (*url).clone(),
            reflow_root: reflow_root.map(|node| node.opaque()),
            visible_rects: rw_sync.visible_rects.clone(),
            generation: rw_unsync.generation,
            new_animations_sender: Mutex::new(self.new_animations_sender.clone()),
            goal: goal,
            running_animations: rw_unsync.running_animations.clone(),
        }
    }
    ...
    fn handle_request<'a>(&'a self,
        ...
        match port_to_read {
            ...
            PortToRead::FontCache => {
                ...
                rw_data.unsync.outstanding_web_fonts.fetch_sub(1, Ordering::SeqCst);
    ...
    fn repaint<'a>(&'a self,
        ...
        let mut rw_data = &mut *rw_data;
        let mut layout_context = self.build_shared_layout_context(&rw_data.unsync,
                                                                  &rw_data.sync,
                                                                  false,
                                                                  None,
                                                                  &self.url,
    ...
    fn handle_request_helper<'a>(&'a self,
        ...
        match request {
            ...
            Msg::GetRPC(response_chan) => {
                let rpc = LayoutRPCImpl(self.rw_data.clone(), self.constellation_chan.clone());
                response_chan.send(box rpc as Box<LayoutRPC + Send>).unwrap();
            },
            Msg::Reflow(data) => {
            ...
            Msg::GetCurrentEpoch(sender) => {
                let rw_data = self.lock_rw_data(possibly_locked_rw_data);
                sender.send(rw_data.unsync.epoch).unwrap();
            },
            Msg::GetWebFontLoadState(sender) => {
                let rw_data = self.lock_rw_data(possibly_locked_rw_data);
                let outstanding_web_fonts =
                    rw_data.unsync.outstanding_web_fonts.load(Ordering::SeqCst);
    ...
    fn collect_reports<'a>(&'a self,
        ...
        let rw_data = self.lock_rw_data(possibly_locked_rw_data);
        let stacking_context = rw_data.unsync.stacking_context.as_ref();
        reports.push(Report {
        ...
        if let Some(ref traversal) = rw_data.unsync.parallel_traversal {
    ...
    fn exit_now<'a>(&'a self,
        ...
        {
            let mut rw_data = self.lock_rw_data(possibly_locked_rw_data);
            if let Some(ref mut traversal) = (&mut *rw_data).unsync.parallel_traversal {
                traversal.shutdown()
            }
    ...
    fn handle_add_stylesheet<'a>(&'a self,
        ...
        let mut rw_data = self.lock_rw_data(possibly_locked_rw_data);
        if mq.evaluate(&rw_data.sync.stylist.device) {
            add_font_face_rules(&sheet,
                                &rw_data.sync.stylist.device,
                                &self.font_cache_task,
                                &self.font_cache_sender,
                                &rw_data.unsync.outstanding_web_fonts);
            rw_data.sync.stylist.add_stylesheet(sheet);
        }
    ...
    fn handle_set_quirks_mode<'a>(&'a self,
                                  possibly_locked_rw_data:
                                    &mut Option<MutexGuard<'a, LayoutTaskData>>) {
        let mut rw_data = self.lock_rw_data(possibly_locked_rw_data);
        rw_data.sync.stylist.add_quirks_mode_stylesheet();
        LayoutTask::return_rw_data(possibly_locked_rw_data, rw_data);
    }
    ...
    fn process_node_geometry_request<'a>(&'a self,
        ...
        rw_data.unsync.client_rect_response = iterator.client_rect;
    }
    ...
    fn process_resolved_style_request<'a>(&'a self,
        ...
        let layout_node = match layout_node {
            None => {
                ...
                rw_data.unsync.resolved_style_response = None;
                return;
            }
        ...
        match property.clone() {
            ...
            if applies && style.get_box().display != display::computed_value::T::none => {
                ...
                sequential::iterate_through_flow_tree_fragment_border_boxes(layout_root,
                                                                            &mut iterator);
                rw_data.unsync.resolved_style_response = iterator.result.map(|r| r.to_css_string());
            },
            ...
            property => {
                rw_data.unsync.resolved_style_response =
    ...
    fn process_offset_parent_query<'a>(&'a self,
        ...
        match parent_info_index {
            Some(parent_info_index) => {
                ...
                rw_data.unsync.offset_parent_response = OffsetParentResponse {
                    node_address: Some(parent.node_address.to_untrusted_node_address()),
                    rect: Rect::new(origin, size),
                };
            }
            None => {
                rw_data.unsync.offset_parent_response = OffsetParentResponse::empty();
            }
        }
    }
        ...
        profile(time::ProfilerCategory::LayoutDispListBuild,
            ...
            flow::mut_base(flow_ref::deref_mut(layout_root)).stacking_relative_position =
                LogicalPoint::zero(writing_mode).to_physical(writing_mode,
                                                             shared_layout_context.screen_size);
            ...
            if data.goal == ReflowGoal::ForDisplay {
                ...
                let root_size = {
                    ...
                    if shared_layout_context.stylist.constrain_viewport().is_some() {
    ...
    fn handle_reflow<'a>(&'a self,
        ...
        let old_screen_size = rw_data.sync.screen_size;
        let current_screen_size = Size2D::new(Au::from_f32_px(initial_viewport.width.get()),
                                              Au::from_f32_px(initial_viewport.height.get()));
        rw_data.sync.screen_size = current_screen_size;
        ...
        if screen_size_changed {
            ...
            rw_data.sync.stylist.set_device(device);

            if let Some(constraints) = rw_data.sync.stylist.constrain_viewport() {
                debug!("Viewport constraints: {:?}", constraints);

                // other rules are evaluated against the actual viewport
                rw_data.sync.screen_size = Size2D::new(Au::from_f32_px(constraints.size.width.get()),
                                                           Au::from_f32_px(constraints.size.height.get()));
                let device = Device::new(MediaType::Screen, constraints.size);
                rw_data.sync.stylist.set_device(device);

                // let the constellation know about the viewport constraints
                let ConstellationChan(ref constellation_chan) = self.constellation_chan;
        ...
        let needs_dirtying = rw_data.sync.stylist.update();
        ...
        {
            let mut rw_data = &mut *rw_data;
            let mut rw_data_unfrozen = &mut rw_data.unsync;
            // Create a layout context for use throughout the following passes.
            let mut shared_layout_context = self.build_shared_layout_context(rw_data_unfrozen,
                                                                             &rw_data.sync,
                                                                             screen_size_changed,
                                                                             Some(&node),
                                                                             &self.url,
            ...
            if node.is_dirty() || node.has_dirty_descendants() || rw_data.sync.stylist.is_dirty() {
                ...
                profile(time::ProfilerCategory::LayoutStyleRecalc,
                    ...
                    match rw_data_unfrozen.parallel_traversal {
                        None => {
                ...
                rw_data_unfrozen.root_flow = self.try_get_layout_root((*node).clone());
            }
            ...
            self.perform_post_style_recalc_layout_passes(&data.reflow_info,
                                                         &mut rw_data_unfrozen,
                                                         &mut shared_layout_context);
        }

        if let Some(mut root_flow) = rw_data.unsync.layout_root() {
    ...
    fn set_visible_rects<'a>(&'a self,
        ...
        let inflation_amount =
            Size2D::new(rw_data.sync.screen_size.width * DISPLAY_PORT_THRESHOLD_SIZE_FACTOR,
                        rw_data.sync.screen_size.height * DISPLAY_PORT_THRESHOLD_SIZE_FACTOR);
        for &(ref layer_id, ref new_visible_rect) in &new_visible_rects {
            match rw_data.sync.visible_rects.get(layer_id) {
                None => {
        ...
        if !must_regenerate_display_lists {
            ...
            rw_data.sync.visible_rects = Arc::new(old_visible_rects);
            return true
        }
        ...
        self.perform_post_main_layout_passes(&reflow_info, &mut rw_data.unsync, &mut layout_context);
        true
    }
    ...
    pub fn tick_animations(&self, rw_data: &mut LayoutTaskData) {
        ...
        if let Some(mut root_flow) = rw_data.unsync.layout_root() {
            ...
            let animations = &*rw_data.unsync.running_animations;
            profile(time::ProfilerCategory::LayoutStyleRecalc,
                    self.profiler_metadata(),
                    self.time_profiler_chan.clone(),
                    || {
    ...
    pub fn reflow_with_newly_loaded_web_font<'a>(
        ...
        if rw_data.unsync.root_flow.as_ref().is_none() {
            return
        }
        self.perform_post_style_recalc_layout_passes(&reflow_info,
                                                     &mut rw_data.unsync,
    ...
    fn perform_post_style_recalc_layout_passes<'a>(&'a self,
        ...
        if let Some(mut root_flow) = rw_data.layout_root() {
            ...
            animation::update_animation_state(self, &mut *rw_data, self.id);
    ...
    unsafe fn handle_reap_layout_data(&self, layout_data: LayoutData) {
        ...
        layout_data_wrapper.remove_compositor_layers(&Mutex::new(self.constellation_chan.clone()));
    }
