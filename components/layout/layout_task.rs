...
impl LayoutTask {
    ...
    fn process_resolved_style_request<'a>(&'a self,
        ...
        /*let layout_node = match pseudo {
            PseudoElementType::Before => node.get_before_pseudo().unwrap(),
            PseudoElementType::After => node.get_after_pseudo().unwrap(),
            _ => layout_node
        };*/
                ...
                let position = layout_data.as_ref().map(|layout_data| {
                    match layout_data.data.flow_construction_result {
                        ConstructionResult::Flow(ref flow_ref, _) => {
                            let base = flow::base(flow_ref.deref());
                            base.stacking_relative_position
                        }
                        _ => panic!("tried to resolve style of element without flow ref"),
                    }
                }).unwrap_or(Point2D::new(Au(0), Au(0)));
    ...
    fn handle_reflow<'a>(&'a self,
        ...
        match data.query_type {
            ...
            ReflowQueryType::ResolvedStyleQuery(node, ref property) =>
                self.process_resolved_style_request(node, property, &mut root_flow, &mut rw_data),
    ...
    fn process(&mut self, _: &Fragment, border_box: &Rect<Au>) {
        self.result =
            Some(match self.property {
                     PositionProperty::Left => border_box.min_x() + self.position.x,
                     PositionProperty::Right => border_box.max_x() + self.position.x,
                     PositionProperty::Bottom => border_box.max_y() + self.position.y,
                     PositionProperty::Top => border_box.min_y() + self.position.y,
                     PositionProperty::Width => border_box.max_x() - border_box.min_x(),
                     PositionProperty::Height => border_box.max_y() - border_box.min_y(),
        });
    }
