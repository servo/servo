...
impl BlockFlow {
    ...
    pub fn get_explicit_block_containing_size(&self, layout_context: &LayoutContext) -> Option<Au> {
        if self.is_root() || self.is_fixed() {
            let screen_size = LogicalSize::from_physical(self.fragment.style.writing_mode,
                                                         layout_context.shared.screen_size);
            Some(screen_size.block)
        } else if self.base.flags.contains(IS_ABSOLUTELY_POSITIONED) {
            self.base.absolute_cb.get_explicit_block_containing_size(layout_context)
        } else {
            self.base.block_container_explicit_block_size
        }
    }
    ...
    pub fn propagate_assigned_inline_size_to_children<F>(&mut self,
        ...
        let parent_container_size = self.get_explicit_block_containing_size(layout_context);
        let explicit_content_size = self.get_explict_block_size(parent_container_size);
