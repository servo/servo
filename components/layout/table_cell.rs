...
impl TableCellFlow {
    ...
    fn assign_block_size_table_cell_base<'a>(&mut self, layout_context: &'a LayoutContext<'a>) {
        ...
        let position_start = flow::base(self).children.front().map(|kid| {
            flow::base(kid).position.start.b
        });
        if let Some(position_start) = position_start {
            let position_end = {
                let kid = flow::base(flow::base(self).children.back().unwrap());
                kid.position.start.b + kid.position.size.block
            };
            let kids_size = position_end - position_start;
