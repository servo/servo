...
impl PositioningContext {
    ...
    pub(crate) fn layout_collected_children(
        ...
        // Sometimes we create temporary PositioningContexts just to collect hoisted absolutes and then
        // these are processed later. In that case and if this fragment doesn't establish a containing
        // block for absolutes at all, we just do nothing. All hoisted fragments will later be passed
        // up to a parent PositioningContext.
        if !new_fragment
            .style
            .establishes_containing_block_for_absolute_descendants(new_fragment.base.flags)
        {
