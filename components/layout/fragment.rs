        ...
        match self.specific {
            ...
            SpecificFragmentInfo::InlineBlock(ref info) => {
                ...
                let baseline_offset = match flow.baseline_offset_of_last_line_box_in_flow() {
                    ...
                    None => {
                        /*println!("no baseline offset of last line box in flow; content block \
                                  size={:?}",
                                 self.style.content_block_size());*/
                        /*match self.style.content_block_size() {
                            LengthOrPercentageOrAuto::Auto |
                            LengthOrPercentageOrAuto::Percentage(_) |
                            LengthOrPercentageOrAuto::Calc(_) => {
                                // TODO(pcwalton): Calculate this properly with the parent size.
                                flow::base(&**flow).position.size.block
                            }
                            LengthOrPercentageOrAuto::Length(length) => {
                                let block_flow = flow.as_block();
                                length + block_flow.fragment.border_padding.block_start +
                                    block_flow.fragment.border_padding.block_end
                            }
                        }*/
                        let block_flow = flow.as_block();
                        block_flow.fragment.border_box.size.block
                    }
                };
                ...
                let metrics = InlineMetrics::new(
                    baseline_offset + start_margin,
                    flow::base(&**flow).position.size.block - baseline_offset + end_margin,
                    baseline_offset);
                /*println!("baseline_offset={:?} (border box {:?}) inline block metrics={:?}",
                         baseline_offset,
                         self.border_box,
                         metrics);*/
                metrics
