...
impl Serialize for Fragment {
    fn serialize<S: Serializer>(&self, serializer: &mut S) -> Result<(), S::Error>{
        ...
        try!(serializer.serialize_struct_elt(&mut state, "id", &self.debug_id)); //change the length!
        try!(serializer.serialize_struct_elt(&mut state, "border_box", &self.border_box)); //change the length!
        try!(serializer.serialize_struct_elt(&mut state, "margin", &self.margin)); //change the length!
