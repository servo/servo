    ...
    pub(crate) fn push_box_fragment(&self, box_fragment: &BoxFragmentWithStyle<'_>) -> Self {
        ...
        let touch_action = {
            ...
            if scrolls_via_user_input(overflow.x) || scrolls_via_user_input(overflow.y) {
                // This box can be panned by touch, so it is the element that implements
                // the gesture for touches on its descendants: its own `touch-action`
                // is stored on its scroll node and applied by the compositor, and the
                // chain of restrictions starts fresh for its descendants.
                TouchAction::Auto
            } else {
                self.touch_action
