...
fn overflow_direction(writing_mode: &WritingMode) -> OverflowDirection {
    match (writing_mode.block_start_physical_side(), writing_mode.inline_start_physical_side()) {
        ...
        (PhysicalSide::Right, PhysicalSide::Bottom) => OverflowDirection::LeftAndUp,
        (PhysicalSide::Left, PhysicalSide::Bottom) => OverflowDirection::RightAndUp,
        _ => OverflowDirection::RightAndDown,
    }
}
...
impl FragmentBorderBoxIterator for FragmentLocatingScrollIterator {
    fn process(&mut self, fragment: &Fragment, level: i32, border_box: &Rect<Au>) {
        ...
        match self.level {
            ...
            Some(start_level) => {
                let rect = match overflow_direction(&fragment.style.writing_mode) {
                    OverflowDirection::RightAndDown => {
