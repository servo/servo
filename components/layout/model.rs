...
impl SizeConstraint {
    /// Create a `SizeConstraint` for an axis. The `border` param is present
    /// when the value of `box-sizing` is `border-box`.
    pub fn new(container_size: Option<Au>, min_size: LengthOrPercentage,
               max_size: LengthOrPercentageOrNone, border: Option<Au>)
               -> SizeConstraint {
        let min_size = match container_size {
        ...
        let max_size = match container_size {
    ...
    /// Make sure the size is to be used for content box.
