...
pub fn item_alignment(input: stylo::AlignFlags) -> Option<taffy::AlignItems> {
    let keyword = match input.value() {
        ...
        // Taffy doesn't support participation in last-baseline sharing, so use the
        // fallback alignment for `last baseline`, which behaves as `end`.
        // <https://drafts.csswg.org/css-align/#baseline-values>
        // This notably produces the correct static position for absolutely-positioned
        // children with `align-self: last baseline`.
        stylo::AlignFlags::LAST_BASELINE => taffy::AlignItemsKeyword::End,
        // Should never be hit. But no real reason to panic here.
        _ => return None,
    };
