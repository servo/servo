...
fn run_queue_with_node_list<F>(queue: &mut WorkQueue<SharedLayoutContextWrapper,UnsafeLayoutNode>,
                               mut callback: F)
                               where F: FnMut(&mut WorkQueue<SharedLayoutContextWrapper,
...
pub fn traverse_dom_preorder(root: LayoutNode,
    ...
    run_queue_with_node_list(queue, |queue| {
        queue.push(WorkUnit {
            fun:  recalc_style,
...
pub fn traverse_flow_tree_preorder(
    ...
    unsafe {
        let queue: &mut WorkQueue<SharedLayoutContextWrapper, UnsafeFlowList> =
            mem::transmute(queue);
