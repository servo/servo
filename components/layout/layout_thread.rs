...
use script_traits::{OpaqueScriptLayoutChannel, StackingContextScrollState};
use sequential;
use serde_json;
...
impl LayoutThread {
        ...
        profile(time::ProfilerCategory::LayoutDispListBuild,
            ...
            if flow::base(&**layout_root).restyle_damage.contains(REPAINT) ||
                ...
                let mut root_stacking_context = StackingContext::new(
                        StackingContextId::new(flow::base(&**layout_root).flow_id()),
    ...
    fn set_stacking_context_scroll_states<'a, 'b>(
        ...
        for new_scroll_state in &new_scroll_states {
            if let Some(ref root_flow) = self.root_flow {
                if flow::base(&**root_flow).flow_id() ==
                        new_scroll_state.stacking_context_id.id() {
