...
pub struct TextInput<T: ClipboardProvider> {
    ...
    /// The maximum number of code units (chars) this text input is allowed to hold.
    ///
...
impl<T: ClipboardProvider> TextInput<T> {
    ...
    fn selection_char_count(&self) -> usize {
        self.fold_selection_slices(0, |count, slice| *count += slice.chars().count())
    }
    ...
    pub fn replace_selection(&mut self, insert: DOMString) {
        if let Some((begin, end)) = self.get_sorted_selection() {
            let allowed_to_insert_count = if let Some(max_length) = self.max_length {
                let len_after_selection_replaced = self.char_count() - self.selection_char_count();
