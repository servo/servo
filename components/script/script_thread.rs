...
pub struct DocumentsIter<'a> {
    values: Values<'a, PipelineId, JS<Document>>,
}

impl<'a> Iterator for DocumentsIter<'a> {
    type Item = Root<Document>;

    fn next(&mut self) -> Option<Root<Document>> {
        self.values.next().map(|doc| Root::from_ref(&**doc))
    }
}
...
impl<'a> Drop for ScriptMemoryFailsafe<'a> {
    ...
    fn drop(&mut self) {
        match self.owner {
            Some(owner) => {
                for document in owner.documents.borrow().iter() {
...
impl ScriptThread {
    ...
    fn handle_msgs(&self) -> bool {
        ...
        for document in self.documents.borrow().iter() {
            ...
            match resize_event {
                Some(size) => resizes.push((document.window().upcast::<GlobalScope>().pipeline_id(), size)),
                None => ()
            }
        }
        ...
        for document in self.documents.borrow().iter() {
            let pending_reflows = document.window().get_pending_reflow_count();
    ...
    fn load(&self, metadata: Metadata, incomplete: InProgressLoad) -> Root<ServoParser> {
        ...
        impl<'a> AutoContextRemover<'a> {
            fn new(script_thread: &'a ScriptThread, pipeline_id: PipelineId) -> AutoContextRemover<'a> {
                AutoContextRemover {
        ...
        impl<'a> Drop for AutoContextRemover<'a> {
            fn drop(&mut self) {
                if !self.neutered {
                    self.script_thread.documents.borrow_mut().remove(self.pipeline_id);
                }
            }
        }
        ...
        let mut context_remover = AutoContextRemover::new(self, incomplete.pipeline_id);
...
fn shut_down_layout(document: &Document) {
    // Tell the layout thread to begin shutting down, and wait until it
    // processed this message.
    let (response_chan, response_port) = channel();
    let chan = document.window().layout_chan().clone();
    if chan.send(message::Msg::PrepareToExit(response_chan)).is_ok() {
        let _ = response_port.recv();
    }

    // Drop our references to the JSContext and DOM objects.
    document.window().clear_js_runtime();

    // If the window is for an active document, then
