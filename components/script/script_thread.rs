...
use net_traits::{AsyncResponseTarget, AsyncResponseListener, CoreResourceMsg};
use net_traits::{IpcSend, LoadConsumer, LoadContext, LoadData as NetLoadData};
use net_traits::{Metadata, ResourceThreads};
...
impl ScriptThread {
    ...
    fn start_page_load_about_blank(&self, incomplete: InProgressLoad) {
        ...
        context.headers_available(Ok(meta));
        context.data_available(vec![]);
        context.response_complete(Ok(()));
    }
