...
impl ScriptThread {
    ...
    fn handle_msgs(&self) -> bool {
        ...
        use self::MixedMessage::{FromScheduler, FromScript, FromServiceWorker};
        ...
        let mut event = {
            ...
            if ret == script_port.id() {
                ...
                FromServiceWorker(self.custom_message_port.recv().unwrap())
            } else {
                        ...
                        Err(_) => match self.devtools_port.try_recv() {
                            Err(_) => match self.image_cache_port.try_recv() {
                                Err(_) => match self.custom_message_port.try_recv() {
                                    Err(_) => break,
                                    Ok(ev) => event = FromServiceWorker(ev)
                                },
        ...
        for msg in sequential {
            ...
            let result = self.profile_event(category, move || {
                match msg {
                    ...
                    FromServiceWorker(inner_msg) => self.handle_msg_from_serviceworker(inner_msg),
    ...
    fn categorize_msg(&self, msg: &MixedMessage) -> ScriptThreadEventCategory {
        match *msg {
            ...
            MixedMessage::FromServiceWorker(_) => ScriptThreadEventCategory::ServiceWorkerEvent
        }
    }
    ...
    fn handle_msg_from_serviceworker(&self, msg: IpcSender<Option<CustomResponse>>) {
