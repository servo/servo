...
impl<TH: TypeHolderTrait> WorkletMethods<TH> for Worklet<TH> {
    ...
    fn AddModule(&self, module_url: USVString, options: &WorkletOptions) -> Rc<Promise<TH>> {
        // Step 1.
        let promise = Promise::<TH>::new(self.window.upcast());
...
impl<TH: TypeHolderTrait> Drop for Worklet<TH> {
    fn drop(&mut self) {
        let script_thread = ScriptThread::<TH>::worklet_thread_pool();
    ...
    fn fetch_and_invoke_a_worklet_script(
        ...
        for sender in &[
            ...
            let _ = sender.send(WorkletControl::FetchAndInvokeAWorkletScript {
                ...
                promise: TrustedPromise::<TH>::new(promise.clone()),
            });
        }
        self.wake_threads();
    }
    ...
    fn spawn(role: WorkletThreadRole, init: WorkletThreadInit) -> Sender<WorkletControl<TH>> {
        ...
        thread::spawn(move || {
            ...
            let mut thread = RootedTraceableBox::new(WorkletThread {
                ...
                control_buffer: None,
                runtime: unsafe { new_rt_and_cx::<TH>() },
                should_gc: false,
