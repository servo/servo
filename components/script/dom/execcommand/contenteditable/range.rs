...
impl Range {
    ...
    pub(crate) fn block_extend(&self, cx: &mut JSContext, document: &Document) -> DomRoot<Range> {
        ...
        if !start_node.is_block_start_point(start_offset as usize, cx.no_gc()) {
            loop {
                ...
                if start_node.is_block_boundary_point(start_offset, cx.no_gc()) {
                    break;
                }
            }
        }
        ...
        if !end_node.is_block_end_point(end_offset, cx.no_gc()) {
            loop {
                ...
                if end_node.is_block_boundary_point(end_offset, cx.no_gc()) {
                    break;
                }
            }
        }
