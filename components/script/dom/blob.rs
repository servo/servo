...
impl Blob {
    ...
    /// XXX: it is still possible to bring down the whole program, but the API might be
    ///      intuitive to use (since we read FS and cache in a transparent style)
            ...
            BlobImpl::File(ref id, ref slice) => {
                match *slice.borrow() {
                    ...
                    None => {
                        ...
                        let s = read_file(global.r(), id.clone()).expect("Read file in get_slice failed");
...
fn read_file(global: GlobalRef, id: Uuid) -> Result<DataSlice, ()> {
    ...
    Some(DataSlice::from_bytes(bytes));
}
    ...
    for blobpart in &blobparts {
        match blobpart {
            ...
            &BlobOrString::Blob(ref b) => {
                ret.append(&mut b.get_slice().bytes.to_vec());
            },
        }
    }
