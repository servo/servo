...
impl Element {
    ...
    pub fn new(local_name: DOMString, namespace: Namespace, prefix: Option<DOMString>, document: &Document) -> Root<Element> {
...
impl<'a> AttributeHandlers for &'a Element {
    fn get_attribute(self, namespace: &Namespace, local_name: &Atom) -> Option<Root<Attr>> {
        ...
        attributes.iter().map(|attr| attr.root()).find(|attr| attr.r().namespace() == namespace)
    }
