        ...
        match &*where_ {
            "beforebegin" => {
                if let Some(parent) = node.GetParentNode() {
                    Node::pre_insert(node,
                                     &parent,
                                     Some(self_node)).map(Some)
                } else {
                    Ok(None)
                }
            }
            "afterbegin" => {
                Node::pre_insert(node,
                                 &self_node,
                                 self_node.GetFirstChild().r()).map(Some)
            }
            "beforeend" => {
                Node::pre_insert(node,
                                 &self_node,
            ...
            "afterend" => {
                if let Some(parent) = self_node.GetParentNode() {
                    Node::pre_insert(node,
                                     &parent,
                                     self_node.GetNextSibling().r()).map(Some)
                } else {
                    Ok(None)
                }
            }
...
impl ElementMethods for Element {
    ...
    fn InsertAdjacentElement(&self, where_: DOMString, element: &Element)
        ...
        if let Some(inserted_node) = try!(self.insert_adjacent(where_, element.upcast())) {
            Ok(Root::downcast(inserted_node))
        } else {
            Ok(None)
        }
    }
