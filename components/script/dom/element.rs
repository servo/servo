...
impl AtomicElementFlags {
    ...
    fn get(&self) -> ElementFlags {
        // TODO: Which atomic ordering should this use?
        ElementFlags::from_bits_truncate(self.0.load(Ordering::SeqCst) as u8)
    }

    fn insert(&self, flags: ElementFlags) {
        self.0.fetch_or(flags.bits() as usize, Ordering::SeqCst);
    }
}
