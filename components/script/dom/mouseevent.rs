...
impl MouseEventMethods<crate::DomTypeHolder> for MouseEvent {
    ...
    fn OffsetX(&self, can_gc: CanGc) -> i32 {
        ...
        if event.dispatching() {
            ...
            let node_offset = target
                .and_then(|target| {
                    target
                        .downcast::<Node>()
                        .map(|node| node.client_rect(can_gc).origin.x)
                })
                .unwrap_or_default();
            return self.ClientX() - node_offset;
        }
        ...
        if event.dispatching() {
            ...
            let node_offset = target
                .and_then(|target| {
                    target
                        .downcast::<Node>()
                        .map(|node| node.client_rect(can_gc).origin.y)
                })
                .unwrap_or_default();
            return self.ClientY() - node_offset;
        }
