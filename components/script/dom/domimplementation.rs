...
impl<TH: TypeHolderTrait> DOMImplementationMethods<TH> for DOMImplementation<TH> {
    ...
    fn CreateHTMLDocument(&self, title: Option<DOMString>) -> DomRoot<Document<TH>> {
...

        {
            ...
            let doc_node = doc.upcast::<Node<TH>>();
            let doc_html =
                DomRoot::upcast::<Node<TH>>(HTMLHtmlElement::new(local_name!("html"), None, &doc));
            doc_node.AppendChild(&doc_html).expect("Appending failed");

            {
                // Step 5.
                let doc_head = DomRoot::upcast::<Node<TH>>(HTMLHeadElement::new(
                    local_name!("head"),
                    None,
                    &doc,
                ));
                ...
                if let Some(title_str) = title {
                    // Step 6.1.
                    let doc_title = DomRoot::upcast::<Node<TH>>(HTMLTitleElement::new(
                        local_name!("title"),
                        None,
                        &doc,
                    ));
