...
impl<'a> DOMImplementationMethods for &'a DOMImplementation {
    ...
    fn CreateHTMLDocument(self, title: Option<DOMString>) -> Root<Document> {
...

        {
            // Step 4.
            let doc_node = NodeCast::from_ref(doc.r());
            let doc_html = NodeCast::from_root(HTMLHtmlElement::new("html".to_owned(), None, doc.r()));
            assert!(doc_node.AppendChild(doc_html.r()).is_ok());

            {
                // Step 5.
                let doc_head = NodeCast::from_root(HTMLHeadElement::new("head".to_owned(), None, doc.r()));
                ...
                match title {
                    ...
                    Some(title_str) => {
                        // Step 6.1.
                        let doc_title = NodeCast::from_root(HTMLTitleElement::new("title".to_owned(), None, doc.r()));
