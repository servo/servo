...
trait PrivateNodeIteratorHelpers {
    fn traverse_following(self) -> Fallible<Option<Temporary<Node>>>;
    fn traverse_preceding(self) -> Fallible<Option<Temporary<Node>>>;
...
impl<'a> PrivateNodeIteratorHelpers for JSRef<'a, NodeIterator> {
    ...
    fn traverse_following(self) -> Fallible<Option<Temporary<Node>>> {
        ...
        // Step 3-1.
        for following_node in node.r().following_node(self.root_node.root().r()) {
    ...
    fn traverse_preceding(self) -> Fallible<Option<Temporary<Node>>> {
        ...
        // Step 3-1.
        for preceding_node in node.r().preceding_node(self.root_node.root().r()) {
