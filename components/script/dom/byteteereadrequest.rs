    ...
    pub(crate) fn chunk_steps(&self, chunk: &Box<Heap<JSVal>>, can_gc: CanGc) -> Fallible<()> {
        ...
        if !self.canceled_1.get() && !self.canceled_2.get() {
            ...
            if let Err(error) = clone_result {
                ...
                let cloned_view = clone_result.unwrap();
                rooted!(in(*cx) let mut view_value = UndefinedValue());
                cloned_view.get_buffer_view_value(cx, view_value.handle_mut());
        ...
        if !self.canceled_1.get() {
            ...
            let chunk1_view =
                HeapBufferSource::<ArrayBufferViewU8>::new(BufferSource::ArrayBufferView(
                    RootedTraceableBox::from_box(Heap::boxed(chunk1.get().to_object())),
                ));
