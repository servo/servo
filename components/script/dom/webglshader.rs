...
impl WebGLShader {
        ...
        if let Some(ref source) = *self.source.borrow() {
            ...
            match validator.compile_and_translate(&[source.as_bytes()]) {
                Ok(translated_source) => {
                    ...
                    self.renderer
                        .send(CanvasMsg::WebGL(CanvasWebGLMsg::ShaderSource(self.id, translated_source))).unwrap();
                    self.renderer
                        .send(CanvasMsg::WebGL(CanvasWebGLMsg::CompileShader(self.id))).unwrap();
    ...
    pub fn set_source(&self, source: String) {
        *self.source.borrow_mut() = Some(source);
        // NOTE: We don't send the ShaderSource message here, we do it on `compile()`
    }
}
