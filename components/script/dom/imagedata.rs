    ...
    pub fn new(global: GlobalRef, width: u32, height: u32, data: Option<Vec<u8>>) -> Root<ImageData> {
        ...
        unsafe {
            ...
            if let Some(vec) = data {
                let js_object_data: *mut uint8_t = JS_GetUint8ClampedArrayData(js_object, &mut false, ptr::null());
    ...
    pub fn get_data_array(&self, global: &GlobalRef) -> Vec<u8> {
        unsafe {
            ...
            let data: *const uint8_t =
                JS_GetUint8ClampedArrayData(self.Data(cx), &mut false, ptr::null()) as *const uint8_t;
