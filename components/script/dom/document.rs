...
impl Document {
    ...
    pub fn refresh_base_element(&self) {
        ...
        self.base_element.set(base.as_ref().map(JS::from_rooted));
    }
...
impl DocumentMethods for Document {
    ...
    fn GetElementsByTagName(&self, tag_name: DOMString) -> Root<HTMLCollection> {
        let window = self.window.root();
        HTMLCollection::by_tag_name(window.r(), self.upcast(), tag_name)
    }
    ...
    fn GetElementsByTagNameNS(&self, maybe_ns: Option<DOMString>, tag_name: DOMString)
                              -> Root<HTMLCollection> {
        let window = self.window.root();
        HTMLCollection::by_tag_name_ns(window.r(), self.upcast(), tag_name, maybe_ns)
    }
    ...
    fn CreateEvent(&self, mut interface: DOMString) -> Fallible<Root<Event>> {
        ...
        match &*interface {
            "uievents" | "uievent" =>
                Ok(Root::upcast(UIEvent::new_uninitialized(&window))),
            "mouseevents" | "mouseevent" =>
                Ok(Root::upcast(MouseEvent::new_uninitialized(&window))),
            "customevent" =>
                Ok(Root::upcast(CustomEvent::new_uninitialized(GlobalRef::Window(&window)))),
            "htmlevents" | "events" | "event" =>
                Ok(Event::new_uninitialized(GlobalRef::Window(&window))),
            "keyboardevent" | "keyevents" =>
                Ok(Root::upcast(KeyboardEvent::new_uninitialized(&window))),
            "messageevent" =>
                Ok(Root::upcast(MessageEvent::new_uninitialized(GlobalRef::Window(&window)))),
            _ =>
    ...
    fn dispatch_load(&self) {
        ...
        browsing_context.frame_element().map(|frame_element| {
            ...
            let event = Event::new(GlobalRef::Window(frame_window.r()), "load".to_owned(),
                                   EventBubbles::DoesNotBubble,
                                   EventCancelable::NotCancelable);
            event.r().fire(frame_element.upcast());
        });
