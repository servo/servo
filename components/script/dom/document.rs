...
impl Document {
    ...
    pub fn set_pending_parsing_blocking_script(&self, script: Option<&HTMLScriptElement>) {
        ...
        self.pending_parsing_blocking_script.set(script.map(JS::from_ref));
    }
    ...
    /// https://html.spec.whatwg.org/multipage/scripting.html#prepare-a-script 15.d and 15.e.
    pub fn process_asap_scripts(&self) {
        ...
        while in_order_scripts.len() > 0 {
            let script = Root::from_rooted(in_order_scripts[0]);
            if !script.r().is_ready_to_be_executed() {
                break;
            }
            script.r().execute();
        ...
        while idx < asap_scripts_set.len() {
            let script = Root::from_rooted(asap_scripts_set[idx]);
            if !script.r().is_ready_to_be_executed() {
                idx += 1;
                continue;
            }
