...
use embedder_traits::{
    ...
    TouchId, WheelEvent, ScrollEvent,
};
...
use script_traits::{
    AnimationState, AnimationTickType, ConstellationInputEvent, DocumentActivity, ScriptMsg, UntrustedNodeAddress,
};
use servo_arc::Arc;
...
impl Document {
    ...
    pub(crate) fn handle_scroll_event(
        ...
        // <https://drafts.csswg.org/cssom-view/#scro`lling-events>
        ...
        if event.external_id.is_root() {
            ...
            DomRoot::upcast::<EventTarget>(document).fire_bubbling_event(Atom::from("scroll"), can_gc);
        } else {
            // Otherwise, fire an event named scroll at target.
            let Some(node_id) = node_id_from_scroll_id(event.external_id.0 as usize) else {
                return;
            };
            let node = unsafe { node::from_untrusted_node_address(UntrustedNodeAddress::from_id(node_id)) };
