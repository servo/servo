...
pub(crate) fn is_valid_namespace_prefix(p: &str) -> bool {
    ...
    !p.chars().any(|c| matches!(c, '\u{0009}' |
                                    '\u{000A}' |
                                    '\u{000C}' |
                                    '\u{000D}' |
                                    '\u{0020}' |
...
pub(crate) fn is_valid_attribute_local_name(name: &str) -> bool {
    ...
    !name.chars().any(|c| matches!(c, '\u{0009}' |
                                    '\u{000A}' |
                                    '\u{000C}' |
                                    '\u{000D}' |
                                    '\u{0020}' |
...
pub(crate) fn validate_and_extract(
    ...
    match prefix {
        ...
        Some(_) if qualified_name == "xmlns" && *namespace != *XMLNS_NAMESPACE => Err(Error::Namespace),
        ...
        Some(p) if *namespace == *XMLNS_NAMESPACE && qualified_name != "xmlns" && p != "xmlns" => Err(Error::Namespace),
        // Step 12. Return (namespace, prefix, localName).
        _ => Ok((namespace, prefix.map(Prefix::from), LocalName::from(local_name)))
    }
}
