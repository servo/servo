...
mod dummy {
    ...
    use std::cell::RefCell;
    use std::rc::Rc;
    thread_local!(pub static LIVE_REFERENCES: Rc<RefCell<Option<Box<Any>>>> =
            Rc::new(RefCell::new(None)));
}
use self::dummy::LIVE_REFERENCES;
...
impl<TH: TypeHolderTrait> TrustedPromise<TH> {
    ...
    pub fn new(promise: Rc<Promise<TH>>) -> TrustedPromise<TH> {
        LIVE_REFERENCES.with(|ref r| {
            ...
            let live_references = live_references
                .downcast_ref::<LiveDOMReferences<TH>>()
                .expect("Should have given type");
            let ptr = &*promise as *const Promise<TH>;
...
impl<T: DomObject> Trusted<T> {
    ...
    pub fn new(ptr: &T) -> Trusted<T> {
        LIVE_REFERENCES.with(|ref r| {
            ...
            let live_references = r.as_ref().unwrap();
            let live_references = live_references
                .downcast_ref::<LiveDOMReferences<T::TypeHolder>>()
                .expect("Should have given type");
...
#[allow(unrooted_must_root)]
pub unsafe fn trace_refcounted_objects<TH: TypeHolderTrait>(tracer: *mut JSTracer) {
    ...
    LIVE_REFERENCES.with(|ref r| {
            ...
            .expect("Should have given type");
        {
            ...
            for obj in table.keys() {
                let reflectable = &*(*obj as *const Reflector<TH>);
