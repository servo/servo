...
impl<TH: TypeHolderTrait> TrustedPromise<TH> {
    ...
    pub fn new(promise: Rc<Promise<TH>>) -> TrustedPromise<TH> {
        LIVE_REFERENCES.with(|ref r| {
            ...
            let live_references = live_references.downcast_ref::<LiveDOMReferences<TH>>().unwrap();
...
impl<T: DomObject> Trusted<T> {
    ...
    pub fn new(ptr: &T) -> Trusted<T> {
        LIVE_REFERENCES.with(|ref r| {
            let r = r.borrow();
            let live_references = r.as_ref().unwrap();
            let live_references = live_references.downcast_ref::<LiveDOMReferences<T::TypeHolder>>().unwrap();
