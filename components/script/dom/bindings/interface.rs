...
unsafe extern "C" fn fun_to_string_hook(cx: *mut JSContext, obj: HandleObject,
                                        _indent: u32) -> *mut JSString {
    let js_class = JS_GetClass(obj.get());
    let object_class = &*(js_class as *const NonCallbackInterfaceObjectClass);
    JS_NewStringCopyZ(cx, object_class.representation.as_ptr() as *const libc::c_char)
}
