...
impl NonCallbackInterfaceObjectClass {
    ...
    pub const fn new(
        ...
        NonCallbackInterfaceObjectClass {
            ...
            proto_id: proto_id,
            proto_depth: proto_depth as u16,
        }
    }
...
unsafe extern "C" fn has_instance_hook(cx: *mut JSContext,
        obj: HandleObject,
        handle: MutableHandleValue,
        rval: *mut bool) -> bool {
    match has_instance(cx, obj, handle.handle()) {
        Ok(result) => {
            *rval = result;
            true
        }
...
unsafe fn has_instance(
    ...
    let js_class = JS_GetClass(*interface_object.ptr);
    let object_class = &*(&js_class as *const _ as *const NonCallbackInterfaceObjectClass);
