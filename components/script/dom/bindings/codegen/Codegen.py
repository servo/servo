...
from WebIDL import BuiltinTypes, IDLBuiltinType, IDLNullValue, IDLType
...
def DOMClass(descriptor):
  ...
  { %s },
  %s, %s
}""" % (prototypeChainString, toStringBool(descriptor.nativeIsISupports), nativeHooks)
...
class PropertyDefiner:
    ...
    def generatePrefableArray(self, array, name, specTemplate, specTerminator,
        ...
        prefableSpecs = []
        if doIdArrays:
            prefableIds = []  # noqa: FIXME: this is never used
...
def getJSToNativeConversionTemplate(type, descriptorProvider, failureCode=None,
    ...
    if type.isSequence():
        ...
        args = (toStringBool(descriptorProvider.workers),
...
""" % args)
    ...
    if type.isUnion():
        ...
        if len(interfaceMemberTypes) > 0:
            ...
            for memberType in interfaceMemberTypes:
                ...
                interfaceObject.append(CGGeneric(
                    "(failed = !%s.TrySetTo%s(cx, ${val}, ${valPtr}, tryNext)) || !tryNext" % (unionArgumentObj, name)
                ))
                names.append(name)
        ...
        if len(arrayObjectMemberTypes) > 0:
            ...
            arrayObject = CGGeneric(
                "done = (failed = !%s.TrySetTo%s(cx, ${val}, ${valPtr}, tryNext)) || !tryNext;"
    ...
    if type.isGeckoInterface():
        ...
        if descriptor.castable:
            ...
            templateBody += (
                "jsval tmpVal = ${val};\n" +
                typePtr + " tmp;\n"
                "if (NS_FAILED(xpc_qsUnwrapArg<" + typeName + ">(cx, ${val}, &tmp, static_cast<" + typeName + "**>"
                "(getter_AddRefs(${holderName})), &tmpVal))) {\n")
    ...
    if type.isEnum():
        ...
        template = (
            "{\n"
            "  bool ok;\n"
            "  int index = FindEnumStringIndex<%(invalidEnumValueFatal)s>(\n"
            "    cx, ${val}, %(values)s, \"%(enumtype)s\", &ok);\n"
            "  if (!ok) {\n"
            "    return false;\n"
            "  }\n"
...
%s
    ...
    if type.isEnum():
        ...
        } + setValue("JS::StringValue(%s_str)" % result)
        return ("""MOZ_ASSERT(uint32_t(%(result)s) < ArrayLength(%(strings)s));
JSString* %(resultStr)s = JS_NewStringCopyN(cx, %(strings)s[uint32_t(%(result)s)].value,
                                            %(strings)s[uint32_t(%(result)s)].length);
if (!%(resultStr)s) {
  return false;
}
""" % args, False)
...
def isResultAlreadyAddRefed(descriptor, extendedAttributes):
    ...
    return not descriptor.workers and 'resultNotAddRefed' not in extendedAttributes
...
""").substitute({"structName": self.type.__str__(),
...
class CGProxySpecialOperation(CGPerSignatureCall):
    ...
    def __init__(self, descriptor, operation):
        ...
        extendedAttributes = descriptor.getExtendedAttributes(operation)  # noqa: FIXME: variable does no get used
...
class CGDOMJSProxyHandler_defineProperty(ClassMethod):
    ...
    def getBody(self):
        ...
        return set + """return mozilla::dom::DOMProxyHandler::defineProperty(%s);""" % (
    ...
    return JS_GetPropertyById(cx, expando, id, vp);
        ...
        if indexedGetter:
            getIndexedOrExpando = (
                ...
                "  %s* self = UnwrapProxy(proxy);\n" +
                CGIndenter(
                    CGProxyIndexedGetter(self.descriptor, templateValues)
                ).define()) % (self.descriptor.nativeType)
            getIndexedOrExpando += """
    ...
    def getBody(self):
        ...
        if stringifier:
            ...
            if not infallible:
                ...
                error = None
            call = CGCallGenerator(error, [], "", returnType, extendedAttributes,
                                   self.descriptor, nativeName, False, object="UnwrapProxy(proxy)")
            return call.define() + """
...
#define REGISTER_PROTO(_dom_class, _pref_check) \\
  aNameSpaceManager->RegisterDefineDOMInterface(
    NS_LITERAL_STRING(#_dom_class), _dom_class##Binding::DefineDOMInterface, _pref_check);\n\n"""
...
class CGBindingRoot(CGThing):
    ...
    def __init__(self, config, prefix, webIDLFile):
        ...
        for x in descriptorsForForwardDeclaration:
            nativeType = x.nativeType  # noqa: FIXME: variable is not used
