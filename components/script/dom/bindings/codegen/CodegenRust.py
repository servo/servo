...
def getJSToNativeConversionInfo(type, descriptorProvider, failureCode=None,
    ...
    if type.isDictionary():
        ...
        if type.inner.isGenericOverTypeHolder():
            name += '<TH>'
        declType = CGGeneric("%s::%s" % (CGDictionary.makeModuleName(type.inner), name))
        empty = "%s::%s::empty()" % (CGDictionary.makeModuleName(type.inner), CGDictionary.makeDictionaryName(type.inner))
...
class PropertyDefiner:
    ...
    def generateGuardedArray(self, array, name, specTemplate, specTerminator,
        ...
        specs = []
        prefableSpecs = []
        prefableTemplate = '    Guard::new(%s, unsafe{%s[%d]})'
        ...
        def specData(m):
            ...
            if "selfHostedName" in m:
                ...
                if m.get("methodInfo", True):
                    ...
                    jitinfo = "&%s as *const _ as *const JSJitInfo" % CGMemberJITInfo(self.descriptor, m['method'], False).define()
...
class CGAbstractMethod(CGThing):
    ...
    def __init__(self, descriptor, name, returnType, args, inline=False,
        ...
        if descriptor and descriptor.isGeneric and self.templateArgs == None:
...
class CGAbstractStaticBindingMethod(CGAbstractMethod):
    ...
    def __init__(self, descriptor, name):
        ...
        CGAbstractMethod.__init__(self, descriptor, name, "bool", args, extern=True, templateArgs=['TH: TypeHolderTrait'])
        self.exposureSet = descriptor.interface.exposureSet

    def definition_body(self):
        preamble = "let global = GlobalScope::<TH>::from_object(JS_CALLEE(cx, vp).to_object());\n"
    ...
    def struct(self):
        ...
        return (string.Template(
                ...
                "}").substitute({"selfName": self.makeClassName(d),
                                 ...
                                 "mustRoot": mustRoot,
                                 "derive": ', '.join(derive),
                                 "typeParams": MaybeTypeHolded(self.dictionary.isGenericOverTypeHolder()).getTypeParams()}))
    ...
    def __init__(self, config, prefix, webIDLFile):
        ...
        for t in typedefs:
            ...
            if t.innerType.isUnion() and not t.innerType.nullable():
                typeName = getRetvalDeclarationForType(t.innerType, config.getDescriptorProvider(), isGeneric=False).define()
    ...
    def InheritTypes(config):
        ...
        for descriptor in descriptors:
            ...
            for baseName in chain:
                allprotos.append(CGGeneric("impl<TH: TypeHolderTrait> DerivedFrom<%s<TH>> for %s<TH> {}\n" % (baseName, name)))
            if chain:
