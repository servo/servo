...
class CGConstructorEnabled(CGAbstractMethod):
    ...
    def __init__(self, descriptor):
        CGAbstractMethod.__init__(self, descriptor,
                                  ...
                                  [Argument("&mut SafeJSContext", "aCx"),
...
rooted!(in(cx.deref_mut()) let private = PrivateValue(raw as *const libc::c_void));
let obj = NewProxyObject(cx.deref_mut(), handler,
                         Handle::from_raw(UndefinedHandleValue),
                         proto.get());
assert!(!obj.is_null());
SetProxyReservedSlot(obj, 0, &private.get());
rooted!(in(cx.deref_mut()) let obj = obj);\
""" % (descriptor.name)
    else:
        create += ("rooted!(in(cx.deref_mut()) let obj = JS_NewObjectWithGivenProto(\n"
                   "    cx.deref_mut(), &Class.base as *const JSClass, proto.handle()));\n"
                   "assert!(!obj.is_null());\n"
                   "\n"
...
def CopyUnforgeablePropertiesToInstance(descriptor):
    ...
    if descriptor.proxy:
        copyCode += """\
rooted!(in(cx.deref_mut()) let mut expando = ptr::null_mut::<JSObject>());
ensure_expando_object(cx.deref_mut(), obj.handle().into(), expando.handle_mut());
"""
        obj = "expando"
    else:
        obj = "obj"
...
assert!(%(copyFunc)s(cx.deref_mut(), %(obj)s.handle(), unforgeable_holder.handle()));
...
class CGWrapMethod(CGAbstractMethod):
    ...
    def __init__(self, descriptor):
        ...
        assert not descriptor.isGlobal()
        args = [Argument('&mut SafeJSContext', 'cx'),
...
rooted!(in(cx.deref_mut()) let mut proto = ptr::null_mut::<JSObject>());
let _ac = JSAutoRealm::new(cx.deref_mut(), scope.get());
...
class CGWrapGlobalMethod(CGAbstractMethod):
    ...
    def definition_body(self):
        ...
        members = ["%s(cx.deref_mut(), obj.handle(), %s, obj.handle());" % (function, array.variableName())
...
rooted!(in(cx.deref_mut()) let mut obj = ptr::null_mut::<JSObject>());
...
let _ac = JSAutoRealm::new(cx.deref_mut(), obj.get());
rooted!(in(cx.deref_mut()) let mut proto = ptr::null_mut::<JSObject>());
GetProtoObject(cx, obj.handle(), proto.handle_mut());
assert!(JS_SplicePrototype(cx.deref_mut(), obj.handle(), proto.handle()));
let mut immutable = false;
assert!(JS_SetImmutablePrototype(cx.deref_mut(), obj.handle(), &mut immutable));
assert!(immutable);
...
if !ConstructorEnabled(&mut SafeJSContext::from_ptr(cx), global) {
    return;
}

rooted!(in(cx) let mut proto = ptr::null_mut::<JSObject>());
%s(&mut SafeJSContext::from_ptr(cx), global, proto.handle_mut());
...
rooted!(in(cx) let mut prototype = ptr::null_mut::<JSObject>());
{
    ...
    if !proto_val.is_object() {
        ...
        GetProtoObject(&mut SafeJSContext::from_ptr(cx), global_object.handle(), prototype.handle_mut());
    } else {
        // Step 6
