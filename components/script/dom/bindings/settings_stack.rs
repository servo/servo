...
pub unsafe fn trace<TH: TypeHolderTrait>(tracer: *mut JSTracer) {
    STACK.with(|stack| {
        for e in &*stack.borrow() {
            let stack_entry = (*e).downcast_ref::<StackEntry<TH>>().unwrap();
...
impl<TH: TypeHolderTrait> Drop for AutoEntryScript<TH> {
    ...
    fn drop(&mut self) {
        STACK.with(|stack| {
            ...
            let entry = (*entry).downcast_ref::<StackEntry<TH>>().unwrap();
...
pub fn entry_global<TH: TypeHolderTrait>() -> DomRoot<GlobalScope<TH>> {
    STACK.with(|stack| {
        stack.borrow()
             .iter()
             .rev()
             .map(|entry| (*entry).downcast_ref::<StackEntry<TH>>().unwrap())
...
impl<TH: TypeHolderTrait> Drop for AutoIncumbentScript<TH> {
    ...
    fn drop(&mut self) {
        STACK.with(|stack| {
            ...
            let entry = (*entry).downcast_mut::<StackEntry<TH>>().unwrap();
            // Step 3.
