...
pub fn GetPropertyOnPrototype(cx: *mut JSContext, proxy: JSHandleObject, id: JSHandleId, found: *mut bool,
    ...
    unsafe {
      ...
      if !JS_HasPropertyById(cx, object_handle(&proto), id, &mut hasProp) {
          return false;
      }
      *found = hasProp;
...
pub fn CreateDOMGlobal(cx: *mut JSContext, class: *const JSClass) -> *mut JSObject {
    unsafe {
        ...
        let obj = NewGlobalObject(cx, class, ptr::null_mut(), 0 /*FireOnNewGlobalHook*/);
        if obj.is_null() {
            return ptr::null_mut();
        }
