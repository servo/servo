...
pub unsafe fn is_array_like(cx: *mut JSContext, value: HandleValue) -> bool {
    ...
    if is_array {
        ...
        if let Ok(_) = root_from_object::<FileList>(object, cx) {
            return true;
        }
        if let Ok(_) = root_from_object::<HTMLCollection>(object, cx) {
            return true;
        }
        if let Ok(_) = root_from_object::<HTMLFormControlsCollection>(object, cx) {
            return true;
        }
        if let Ok(_) = root_from_object::<HTMLOptionsCollection>(object, cx) {
            return true;
        }
        if let Ok(_) = root_from_object::<NodeList>(object, cx) {
            return true;
        }
        false
    }
