...
use js;
use js::glue::{IsWrapper, RUST_JS_NumberValue, RUST_JSID_IS_STRING};
use js::glue::{RUST_JSID_TO_STRING, UnwrapObject};
use js::rust::{ToUint64, ToInt64};
use js::rust::{ToUint32, ToInt32};
use js::rust::{ToUint16, ToNumber, ToBoolean, ToString};
use js::jsapi::{JS_GetReservedSlot, JSContext, JSObject, JSString};
...
pub unsafe fn private_from_proto_chain(mut obj: *mut JSObject,
    ...
    if dom_class.interface_chain[proto_depth as usize] as u16 == proto_id {
        ...
        let value = JS_GetReservedSlot(obj, DOM_OBJECT_SLOT);
        Ok(value.to_private())
    } else {
        debug!("bad prototype");
        Err(())
