...
impl<'a> From<&'a str> for USVString {
    fn from(contents: &str) -> USVString {
        USVString::from(String::from(contents))
    }
}

impl<'a> From<Cow<'a, str>> for USVString {
    fn from(contents: Cow<'a, str>) -> USVString {
        match contents {
            Cow::Owned(s) => USVString::from(s),
            Cow::Borrowed(s) => USVString::from(s),
        }
    }
}
