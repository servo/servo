...
//! creating a stack-based root via the `root` method through the `Rootable<T>`
//! trait. This returns a `Root<T>`, which causes the JS-owned value to be
//! uncollectable for the duration of the `Root` object's lifetime. A reference
//! to the object can then be obtained from the `Root` object. These references
//! are not allowed to outlive their originating `Root<T>`.
//!
...
use dom::node::Node;
use js::jsapi::{JSObject, Heap, HandleObject, JSTracer};
use js::jsval::JSVal;
