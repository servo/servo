...
impl File {
    fn new_inherited(blob: &Blob, name: DOMString, modified: Option<i64>) -> File {
       let mut bytes = Vec::new();
       bytes.extend_from_slice(blob.get_data().get_all_bytes().as_slice());
    ...
    pub fn new(global: GlobalRef, blob: &Blob, name: DOMString, modified: Option<i64>) -> Root<File> {
        reflect_dom_object(box File::new_inherited(blob, name, modified),
                           global,
                           FileBinding::Wrap)
    }
