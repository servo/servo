...
impl WebGLTexture {
    ...
    fn is_complete(&self) -> bool {
        ...
        // Ensure non-zero width, height, and depth
        if image_info.width == 0 || image_info.height == 0 || image_info.depth == 0 {
            return false;
        }

        // If the texture is a cube map, ensure it is cube complete
        if self.target.get().unwrap() == constants::TEXTURE_CUBE_MAP && !self.is_cube_complete() {
            return false;
        }

        // TODO(ConnorGBrewster): WebGLSampler, check if texture requires mipmap due to `GL_NEAREST` or `GL_LINEAR`
        // for now assume a mipmap is required
        if !self.is_mipmap_complete() {
            return false;
        }
    ...
    fn is_mipmap_complete(&self) -> bool {
        let max_level = self.max_effective_mipmap_level();

        if self.base_mipmap_level > max_level {
            return false;
        }
        ...
        let mut ref_width = image_info.width;
        let mut ref_height = image_info.height;
        let mut ref_depth = image_info.depth;

        for level in self.base_mipmap_level..max_level {
            for face in 0..self.face_count.get() {
                let current_image_info = self.image_info_at_face(face, level);

                if current_image_info.width != ref_width ||
                   current_image_info.height != ref_height ||
                   current_image_info.depth != ref_width {
                    return false;
                }
            }

            if ref_width == 1 && ref_height == 1 && ref_depth == 1 {
                break;
            }
    ...
    fn max_effective_mipmap_level(&self) -> u32 {
        // TODO(ConnorGBrewster): Check WebGLSampler for GL_NEAREST or GL_LINEAR. If this is the case
