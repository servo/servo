...
pub struct WebGLTexture {
    ...
    id: u32,
    /// The target this texture was bound the first time
...
pub trait WebGLTextureHelpers {
    ...
    fn tex_parameter(self,
                     target: u32,
                     name: u32,
                     maybe_int_value: Option<i32>,
                     maybe_float_value: Option<f32>) -> WebGLResult<()>;
}
...
impl<'a> WebGLTextureHelpers for &'a WebGLTexture {
    ...
    fn tex_parameter(self,
                     ...
                     maybe_int_value: Option<i32>,
                     maybe_float_value: Option<f32>) -> WebGLResult<()> {
        debug_assert!(maybe_int_value.is_some() || maybe_float_value.is_some());
        let (int_value, _float_value) = match (maybe_int_value, maybe_float_value) {
            (Some(int_value), None) => (int_value, int_value as f32),
            (None, Some(float_value)) => (float_value as i32, float_value),
            _ => unreachable!(),
        };
