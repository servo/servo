...
impl Window {
    ...
    ///
    /// Returns true if the reflow was not suppressed, false otherwise.
    ...
    /// off-main-thread layout.
    ///
    /// Returns true if the reflow was not supressed, false otherwise. This
    /// return value is useful for script queries, that wait for a lock that
    /// layout might hold if the first layout hasn't happened yet (which may
    /// happen in the only case a query reflow may bail out, that is, if the
