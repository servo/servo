...
impl Window {
    ...
    pub fn get_cx(&self) -> JSContext {
        match &*self.current_state.borrow_mut() {
            WindowState::Zombie(_) => panic!("Window should be alive when get_cx is called"),
    ...
    pub fn window_proxy(&self) -> DomRoot<WindowProxy> {
        match &*self.current_state.borrow() {
            WindowState::Zombie(_) => {
                panic!("Window should be alive when window_proxy is called")
            },
    ...
    pub fn pending_image_notification(&self, response: PendingImageResponse) {
        ...
        match &*self.current_state.borrow() {
            WindowState::Zombie(_) => {
                panic!("Window should be alive when pending_image_notification is called")
            },
...
impl WindowMethods for Window {
    ...
    fn Document(&self) -> DomRoot<Document> {
        let document = match &*self.current_state.borrow() {
            WindowState::Zombie(document) => document.get(),
            WindowState::Alive(data) => data.document.get(),
        };
        document.expect("Document accessed before initialization.")
    }
...
impl Window {
    ...
    pub fn clear_js_runtime(&self) {
        ...
        match &*self.current_state.borrow() {
            WindowState::Zombie(_) => {
                panic!("Window should be alive when clear_js_runtime is called")
            },
            WindowState::Alive(data) => {
        ...
        // Note that switching to zombie state must happen after the call to Gc,
        // because calling Gc requires having a JS runtime.
        let document = match &*self.current_state.borrow() {
            WindowState::Zombie(_) => {
                panic!("Window should be alive when clear_js_runtime is called")
            },
    ...
    pub fn force_reflow(&self, reflow_goal: ReflowGoal, reason: ReflowReason) -> bool {
        ...
        for image in complete.pending_images {
            ...
            let node = match &*self.current_state.borrow_mut() {
                WindowState::Zombie(_) => {
                    panic!("Window should be alive when force_reflow is called")
                },
                WindowState::Alive(data) => {
    ...
    pub fn reflow(&self, reflow_goal: ReflowGoal, reason: ReflowReason) -> bool {
        ...
        if self.prepare_for_screenshot && for_display {
            ...
            let has_pending_images = match &*self.current_state.borrow() {
                WindowState::Zombie(_) => {
                    panic!("Window should be alive when reflow is called")
                },
    ...
    pub fn offset_parent_query(&self, node: &Node) -> (Option<DomRoot<Element>>, UntypedRect<Au>) {
        ...
        let element = response.node_address.and_then(|parent_node_address| {
            let node = match &*self.current_state.borrow_mut() {
                WindowState::Zombie(_) => {
                    panic!("Window should be alive when offset_parent_query is called")
                },
                WindowState::Alive(data) => {
                    let js_runtime = data.js_runtime.as_ref().unwrap();
    ...
    pub fn init_window_proxy(&self, window_proxy: &WindowProxy) {
        match &*self.current_state.borrow_mut() {
            WindowState::Zombie(_) => {
                panic!("Window should be alive when init_window_proxy is called")
            },
            WindowState::Alive(data) => {
    ...
    pub fn init_document(&self, document: &Document) {
        ...
        match &*self.current_state.borrow_mut() {
            WindowState::Zombie(_) => {
                panic!("Window should be alive when init_document is called")
            },
            WindowState::Alive(data) => {
