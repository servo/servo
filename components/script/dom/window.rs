...
impl<'a> WindowMethods for JSRef<'a, Window> {
    ...
    fn Close(self) {
        self.script_chan.send(MainThreadScriptMsg::ExitWindow(self.id.clone())).unwrap();
    }
...
impl<'a> WindowHelpers for JSRef<'a, Window> {
    ...
    fn load_url(self, href: DOMString) {
        ...
        match url.fragment {
            Some(fragment) => {
                self.script_chan.send(MainThreadScriptMsg::TriggerFragment(self.id, fragment)).unwrap();
            },
            None => {
                self.script_chan.send(MainThreadScriptMsg::TriggerLoad(self.id, LoadData::new(url))).unwrap();
            }
        }
    }
