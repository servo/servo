...
impl Window {
    ...
    pub fn force_reflow(&self, goal: ReflowGoal, query_type: ReflowQueryType, reason: ReflowReason) {
        ...
        if for_display && self.suppress_reflow.get() {
            debug!("Suppressing reflow for goal {:?} reason {:?} before FirstLoad or RefreshTick",
                   goal, reason);
            return;
        }
    ...
    pub fn hit_test_query(&self, hit_test_request: Point2D<f32>) -> Option<UntrustedNodeAddress> {
        self.reflow(ReflowGoal::ForDisplay,
                    ReflowQueryType::HitTestQuery(hit_test_request),
                    ReflowReason::Query);
        self.layout_rpc.hit_test().node_address
    }

    pub fn mouse_over_query(&self, mouse_over_request: Point2D<f32>) -> Vec<UntrustedNodeAddress> {
        self.reflow(ReflowGoal::ForDisplay,
                    ReflowQueryType::MouseOverQuery(mouse_over_request),
                    ReflowReason::Query);
        self.layout_rpc.mouse_over().node_addresses
    }
...
fn debug_reflow_events(id: PipelineId, goal: &ReflowGoal, query_type: &ReflowQueryType, reason: &ReflowReason) {
    ...
    debug_msg.push_str(match *query_type {
        ...
        ReflowQueryType::HitTestQuery(_n) => "\tHitTestQuery",
        ReflowQueryType::MouseOverQuery(_n) => "\tMouseOverQuery",
