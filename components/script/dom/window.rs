...
impl WindowMethods for Window {
    ...
    fn Closed(&self) -> bool {
        self.window_proxy
            .get()
            .map(|ref proxy| proxy.is_browsing_context_discarded() || self.is_closing.get())
            .unwrap_or(true)
    }
            ...
            if is_script_closable {
                ...
                let task = task!(window_close_browsing_context: move || {
                    ...
                    if document.prompt_to_unload(false) {
                        ...
                        // "exit pipeline" msg is handled by the script-thread, and we dont' want
                        // tasks executing in between.
                        //
                        ...
                        // when the script-thread exists the pipeline.
