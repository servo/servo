...
fn navigate(window: &Window, direction: NavigationDirection) {
...
impl<'a> HistoryMethods for &'a History {
    ...
    fn Back(self) {
        navigate(&self.window.root(), NavigationDirection::Back)
    }

    // https://html.spec.whatwg.org/multipage/#dom-history-forward
    fn Forward(self) {
        navigate(&self.window.root(), NavigationDirection::Forward)
    }
}
