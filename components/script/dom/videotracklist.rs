...
impl VideoTrackList {
    ...
    pub fn set_selected(&self, idx: usize, value: bool) {
        if let Some(track) = self.item(idx) {
            // Ensure a track's selected attribute will change.
            if track.selected() != value {
                if let Some(selected_idx) = self.selected_index() {
                    if selected_idx != idx && value {
                        // Unselect the current selected track and set the track
                        // found at the given index as selected.
                        self.tracks
                            .borrow()
                            .iter()
                            .find(|track| track.selected())
                            .map(|track| track.set_selected(false));

                        track.set_selected(true);
                    } else if selected_idx == idx && !value {
                        // Deselect the currently selected track.
                        // A task is queued to fire a change event named change
                        // when no track is selected in its stead.
                    ...
                    // There is no previously selected track and the track found at the
                    // given index is set to selected.
                    track.set_selected(true);

                    // Whenever a previously unselected track
                    // is selected a task is queued to to fire an event named change.
                    let global = &self.global();
                    let this = Trusted::new(self);
                    let (source, canceller) = global
                        .as_window()
