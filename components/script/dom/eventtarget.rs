...
impl EventTarget {
    ...
    fn get_compiled_event_handler(
        ...
        // Step 3.7. If body is not parsable as FunctionBody or if parsing detects an early error, then follow these substeps:
        //
        // is written as though we call the parser separately
        ...
        if handler.get().is_null() {
            // Step 3.9. Let realm be settings object's realm.
        ...
        // Step 1.14. Return eventHandler's value.
        if is_error {
    ...
    pub(crate) fn recompile_all_event_handlers_for_element(&self, element: &Element) {
        ...
        for (ty, entries) in handlers.iter() {
            for entry in entries.iter() {
                ...
                let attr_name = LocalName::from(ty.as_ref());
                let Some(source) = element.get_attribute(&ns!(), &attr_name) else {
                    *inline_listener.borrow_mut() = InlineEventListener::Null;
                    continue;
                };
