...
pub enum CommonEventHandler<TH: TypeHolderTrait> {
    EventHandler(#[ignore_malloc_size_of = "Rc"] Rc<EventHandlerNonNull<TH>>),

    ErrorEventHandler(#[ignore_malloc_size_of = "Rc"] Rc<OnErrorEventHandlerNonNull<TH>>),

    BeforeUnloadEventHandler(#[ignore_malloc_size_of = "Rc"] Rc<OnBeforeUnloadEventHandlerNonNull<TH>>),
}
                ...
                match *handler {
                    ...
                    CommonEventHandler::BeforeUnloadEventHandler(ref handler) => {
                        if let Some(event) = event.downcast::<BeforeUnloadEvent<TH>>() {
                            // Step 5
                            if let Ok(value) =
                                handler.Call_(object, event.upcast::<Event<TH>>(), exception_handle)
                            {
...
impl<TH: TypeHolderTrait> EventTarget<TH> {
    ...
    pub fn dispatch_event_with_target(
        &self,
    ...
    fn set_inline_event_listener(&self, ty: Atom, listener: Option<InlineEventListener<TH>>) {
