...
impl<TH: TypeHolderTrait> HTMLTableElement<TH> {
    ...
    fn get_first_section_of_type(
        &self,
        atom: &LocalName,
    ) -> Option<DomRoot<HTMLTableSectionElement<TH>>> {
        self.upcast::<Node<TH>>()
            .child_elements()
    ...
    where
        P: FnMut(&DomRoot<Element<TH>>) -> bool,
    {
    ...
    fn get_rows(&self) -> TableRowFilter<TH> {
        TableRowFilter {
            sections: self
                ...
                .children()
                .filter_map(|ref node| {
                    node.downcast::<HTMLTableSectionElement<TH>>()
...
impl<TH: TypeHolderTrait> HTMLTableElementMethods<TH> for HTMLTableElement<TH> {
    ...
    fn CreateTBody(&self) -> DomRoot<HTMLTableSectionElement<TH>> {
        ...
        let reference_element = last_tbody.and_then(|t| t.upcast::<Node<TH>>().GetNextSibling());
    ...
    fn InsertRow(&self, index: i32) -> Fallible<DomRoot<HTMLTableRowElement<TH>>> {
        ...
        if number_of_row_elements == 0 {
            ...
            if let Some(last_tbody) = node
                ...
                .filter_map(DomRoot::downcast::<Element<TH>>)
                .find(|n| {
                    n.is::<HTMLTableSectionElement<TH>>() && n.local_name() == &local_name!("tbody")
                }) {
                last_tbody
                    .upcast::<Node<TH>>()
                    .AppendChild(new_row.upcast::<Node<TH>>())
            ...
            ith_row_parent
                .upcast::<Node<TH>>()
                .InsertBefore(
                    new_row.upcast::<Node<TH>>(),
                    Some(ith_row.upcast::<Node<TH>>()),
