...
impl<'a> CharacterDataMethods for JSRef<'a, CharacterData> {
    ...
    fn SubstringData(self, offset: u32, count: u32) -> Fallible<DOMString> {
        Ok(self.data.borrow().as_slice().slice(offset as usize, count as usize).to_owned())
    }
    ...
    fn ReplaceData(self, offset: u32, count: u32, arg: DOMString) -> ErrorResult {
        ...
        let mut data = self.data.borrow().as_slice().slice(0, offset as usize).to_owned();
        data.push_str(arg.as_slice());
        data.push_str(self.data.borrow().as_slice().slice((offset + count) as usize, length as usize));
        *self.data.borrow_mut() = data;
