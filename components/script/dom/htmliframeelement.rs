...
impl<TH: TypeHolderTrait> HTMLIFrameElement<TH> {
    ...
    pub fn navigate_or_reload_child_browsing_context(
        ...
        match nav_type {
            NavigationType::InitialAboutBlank => {
                ...
                ScriptThread::<TH>::process_attach_layout(
                    new_layout_info,
    ...
    fn process_the_iframe_attributes(&self, mode: ProcessingMode) {
        ...
        if mode == ProcessingMode::FirstTime && !self
            .upcast::<Element<TH>>()
            .has_attribute(&local_name!("src"))
        {
...
impl<TH: TypeHolderTrait> HTMLIFrameElementMethods<TH> for HTMLIFrameElement<TH> {
    ...
    fn Sandbox(&self) -> DomRoot<DOMTokenList<TH>> {
        self.sandbox
            .or_init(|| DOMTokenList::new(self.upcast::<Element<TH>>(), &local_name!("sandbox")))
    }
    ...
    fn GetContentWindow(&self) -> Option<DomRoot<WindowProxy<TH>>> {
        self.browsing_context_id
            ...
            .and_then(|browsing_context_id| {
                ScriptThread::<TH>::find_window_proxy(browsing_context_id)
            })
    }
    ...
    fn GetContentDocument(&self) -> Option<DomRoot<Document<TH>>> {
        ...
        // Step 4.
        let current = GlobalScope::<TH>::current()
