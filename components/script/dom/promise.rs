...
unsafe extern fn native_handler_callback(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    ...
    match v.to_int32() {
        v if v == NativeHandlerTask::Resolve as i32 => handler.resolved_callback(cx, HandleValue::from_raw(args.get(0))),
        v if v == NativeHandlerTask::Reject as i32 => handler.rejected_callback(cx, HandleValue::from_raw(args.get(0))),
