...
impl<TH: TypeHolderTrait> Promise<TH> {
    ...
    pub fn duplicate(&self) -> Rc<Promise<TH>> {
        ...
        unsafe { Promise::<TH>::new_with_js_promise(self.reflector().get_jsobject(), cx) }
    }
...
#[allow(unsafe_code)]
unsafe extern "C" fn native_handler_callback<TH: TypeHolderTrait>(
    cx: *mut JSContext,
    argc: u32,
    vp: *mut JSVal,
) -> bool {
...
#[allow(unsafe_code)]
fn create_native_handler_function<TH: TypeHolderTrait>(
    cx: *mut JSContext,
    holder: HandleObject,
    ...
    unsafe {
        let func =
            NewFunctionWithReserved(cx, Some(native_handler_callback::<TH>), 1, 0, ptr::null());
        assert!(!func.is_null());
