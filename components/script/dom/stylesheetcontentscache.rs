...
use malloc_size_of::{MallocConditionalSizeOf, MallocSizeOf};
...
impl StylesheetContentsCacheKey {
    ...
    pub(crate) fn is_unique_owned(&self) -> bool {
        // The cache itself already holds one reference.
        Rc::strong_count(&self.stylesheet_text) <= UNIQUE_OWNED
    }
}

impl MallocSizeOf for StylesheetContentsCacheKey {
    fn size_of(&self, ops: &mut malloc_size_of::MallocSizeOfOps) -> usize {
        self.stylesheet_text.conditional_size_of(ops) + self.base_url.size_of(ops)
    }
}
...
impl StylesheetContentsCache {
    fn is_or_contains_import_rule(rule: &CssRule, guard: &SharedRwLockReadGuard) -> bool {
        if let CssRule::Import(_) = rule {
            return true;
        }

        if let CssRule::LayerBlock(layer_rule) = rule {
            return layer_rule
                .rules
                .read_with(guard)
                .0
                .iter()
                .any(|rule| Self::is_or_contains_import_rule(rule, guard));
        }
