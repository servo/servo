...
pub fn run_worker_event_loop<T, TimerMsg, WorkerMsg, Event>(worker_scope: &DomRoot<T>,
                                                            worker: Option<&TrustedWorkerAddress>)
where
    TimerMsg: Send,
    WorkerMsg: QueuedTaskConversion + Send,
    T: WorkerEventLoopMethods<TimerMsg = TimerMsg,  WorkerMsg = WorkerMsg, Event = Event> + DomObject {
    let scope = worker_scope.worker_global_scope();
    ...
    let event = {
        if ret == worker_handle.id() {
            worker_scope.from_worker_msg(task_queue.take_tasks().recv().unwrap())
    ...
    while !scope.is_closing() {
        ...
        match task_queue.take_tasks().try_recv() {
    ...
    for event in sequential {
        ...
        if let Some(worker) = worker {
            worker_scope.handle_worker_post_event(worker);
        }
        worker_scope.global_scope().perform_a_microtask_checkpoint();
    }
}
