...
impl PartialEq for TimeRange {
    fn eq(&self, other: &TimeRange) -> bool {
        self.is_overlapping(other) || self.is_contiguous(other)
    }
}
...
impl Ord for TimeRange {
    fn cmp(&self, other: &TimeRange) -> Ordering {
        ...
        // This can be exactly the same, overlapping or contiguous
        Ordering::Equal
    }
}
...
impl TimeRangesContainer {
    ...
    pub fn start(&self, index: u32) -> Result<f64, ()> {
        if index >= self.len() {
            return Err(());
        }
        Ok(self.ranges[index as usize].start)
    }

    pub fn end(&self, index: u32) -> Result<f64, ()> {
        if index >= self.len() {
            return Err(());
        }
        Ok(self.ranges[index as usize].end)
    }

    pub fn add(&mut self, start: f64, end: f64) -> Result<(), ()> {
        if start > end {
            eprintln!("Cannot add a range if the end is older than the start.");
            return Err(());
        }
        ...
        while idx < self.ranges.len() {
            ...
            } else if new_range < self.ranges[idx] && (idx == 0 || self.ranges[idx - 1] < new_range) {
...
#[allow(dead_code)]
impl TimeRanges {
    pub fn new_inherited() -> TimeRanges {
        Self {
