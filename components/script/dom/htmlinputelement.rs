...
use crate::textinput::{ByteOffset, Direction, SelectionDirection, TextInput, UTF16CodeUnits};
...
impl LayoutHTMLInputElementHelpers for LayoutDom<HTMLInputElement> {
    ...
    unsafe fn selection_for_layout(self) -> Option<Range<usize>> {
        ...
        match (*self.unsafe_get()).input_type() {
            InputType::Password => {
                let text = get_raw_textinput_value(self);
                let sel = ByteOffset::unwrap_range(textinput.sorted_selection_offsets_range());
            ...
            input_type if input_type.is_textual() => Some(ByteOffset::unwrap_range(
