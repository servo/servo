        ...
        match task {
            PaintWorkletTask::DrawAPaintImage(name, size, device_pixel_ratio, properties, sender) => {
                ...
                let result = if cache_hit {
                    ...
                    let result = self.draw_a_paint_image(&name, size, device_pixel_ratio, &*map);
            ...
            PaintWorkletTask::SpeculativelyDrawAPaintImage(name, properties) => {
    ...
    fn painter(&self, name: Atom) -> Box<Painter> {
        ...
        impl SpeculativePainter for WorkletPainter {
            fn speculatively_draw_a_paint_image(&self, properties: Vec<(Atom, String)>) {
                ...
                let task = PaintWorkletTask::SpeculativelyDrawAPaintImage(name, properties);
