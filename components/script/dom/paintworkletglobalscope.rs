...
impl PaintWorkletGlobalScope {
                               ...
                               sender: IpcSender<CanvasData>)
    {
        ...
        // TODO: Steps 1-2.1.
        // Step 3.
        debug!("Looking up paint definition {}.", name);
        ...
        match self.paint_class_instances.borrow_mut().entry(name.clone()) {
            ...
            Entry::Vacant(entry) => {
                debug!("Creating new paint instance.");
                // Step 5.1
        ...
        definition.context.set_bitmap_dimensions(size);
        ...
        let args = unsafe { HandleValueArray::from_rooted_slice(&[
            ObjectValue(definition.context.reflector().get_jsobject().get()),
        ...
        definition.context.send_data(sender);
    }
