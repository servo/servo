...
use script_task::{ScriptChan, ScriptMsg, MainThreadScriptMsg};
...
impl<'a> HTMLFormElementHelpers for JSRef<'a, HTMLFormElement> {
    fn submit(self, _submit_method_flag: SubmittedFrom, submitter: FormSubmitter) {
        ...
        win.r().script_chan().send(ScriptMsg::MainThreadMsg(MainThreadScriptMsg::TriggerLoad(win.r().pipeline(), load_data))).unwrap();
    }
