            ...
            fn filter<'a>(&self, elem: &'a Element<THH>, _root: &'a Node<THH>) -> bool {
                let form_owner = match elem.upcast::<Node<THH>>().type_id() {
                    NodeTypeId::Element(ElementTypeId::HTMLElement(t)) => match t {
                        HTMLElementTypeId::HTMLButtonElement => elem
                            .downcast::<HTMLButtonElement<THH>>()
                            .unwrap()
                            .form_owner(),
                        HTMLElementTypeId::HTMLFieldSetElement => elem
                            .downcast::<HTMLFieldSetElement<THH>>()
                            .unwrap()
                            .form_owner(),
                        ...
                        HTMLElementTypeId::HTMLObjectElement => elem
                            .downcast::<HTMLObjectElement<THH>>()
                            .unwrap()
                            .form_owner(),
                        HTMLElementTypeId::HTMLOutputElement => elem
                            .downcast::<HTMLOutputElement<THH>>()
                            .unwrap()
                            .form_owner(),
                        HTMLElementTypeId::HTMLSelectElement => elem
                            .downcast::<HTMLSelectElement<THH>>()
                            .unwrap()
                            .form_owner(),
                        HTMLElementTypeId::HTMLTextAreaElement => elem
                            .downcast::<HTMLTextAreaElement<THH>>()
                            .unwrap()
                            .form_owner(),
                        _ => {
                            debug_assert!(
                                !elem
                                    .downcast::<HTMLElement<THH>>()
                                    .unwrap()
                                    .is_listed_element() ||
...
pub enum FormSubmitter<'a, TH: TypeHolderTrait> {
    ...
    ButtonElement(&'a HTMLButtonElement<TH>), // TODO: image submit, etc etc
}
...
pub trait FormControl<TH: TypeHolderTrait>: DomObject {
    ...
    fn reset_form_owner(&self) {
        ...
        let new_owner = if self.is_listed() && has_form_id && elem.is_connected() {
            ...
            doc.GetElementById(form_id)
                .and_then(DomRoot::downcast::<HTMLFormElement<TH>>)
        } else {
            // Step 4
    ...
    where
        ...
        OwnerFn: Fn(&HTMLFormElement<TH>) -> DOMString,
        Self: Sized,
    {
    ...
    where
        ...
        OwnerFn: Fn(&HTMLFormElement<TH>) -> bool,
        Self: Sized,
    {
        if self.to_element().has_attribute(attr) {
            input(self)
...
impl<TH: TypeHolderTrait> FormControlElementHelpers<TH> for Element<TH> {
    fn as_maybe_form_control<'a>(&'a self) -> Option<&'a FormControl<TH, TypeHolder = TH>> {
        ...
        match node.type_id() {
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLButtonElement,
            )) => Some(self.downcast::<HTMLButtonElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLFieldSetElement,
            )) => Some(self.downcast::<HTMLFieldSetElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLImageElement,
            )) => Some(self.downcast::<HTMLImageElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLInputElement,
            )) => Some(self.downcast::<HTMLInputElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLLabelElement,
            )) => Some(self.downcast::<HTMLLabelElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLLegendElement,
            )) => Some(self.downcast::<HTMLLegendElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLObjectElement,
            )) => Some(self.downcast::<HTMLObjectElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLOutputElement,
            )) => Some(self.downcast::<HTMLOutputElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLSelectElement,
            )) => Some(self.downcast::<HTMLSelectElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            NodeTypeId::Element(ElementTypeId::HTMLElement(
                HTMLElementTypeId::HTMLTextAreaElement,
            )) => Some(self.downcast::<HTMLTextAreaElement<TH>>().unwrap()
                as &FormControl<TH, TypeHolder = TH>),
            _ => None,
        }
    }
...
pub fn encode_multipart_form_data<TH: TypeHolderTrait>(
    form_data: &mut Vec<FormDatum<TH>>,
    boundary: String,
    encoding: &'static Encoding,
) -> Vec<u8> {
