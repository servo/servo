...
impl HTMLFormElement {
    ...
    pub fn submit(&self, submit_method_flag: SubmittedFrom, submitter: FormSubmitter) {
        ...
        match (&*scheme, method) {
            ...
            ("http", FormMethod::FormGet) | ("https", FormMethod::FormGet) | ("data", FormMethod::FormGet) => {
                ...
                self.mutate_action_url(&mut form_data, &mut load_data.url);
                self.plan_to_navigate(load_data, &win);
            }
            // https://html.spec.whatwg.org/multipage/#submit-body
            ("http", FormMethod::FormPost) | ("https", FormMethod::FormPost) => {
                load_data.method = Method::Post;
                self.submit_as_entity_body(&mut load_data, &mut form_data, enctype);
                self.plan_to_navigate(load_data, &win);
            }
    ...
    // https://html.spec.whatwg.org/multipage/#submit-mutate-action
    fn mutate_action_url(&self, form_data: &mut Vec<FormDatum>, url: &mut Url) {
        let encoding = self.pick_encoding();
        let charset = &*encoding.whatwg_name().unwrap();
    ...
    // https://html.spec.whatwg.org/multipage/#submit-body
    fn submit_as_entity_body(&self, load_data: &mut LoadData,
                       form_data: &mut Vec<FormDatum>, enctype: FormEncType) {
        ...
        let bytes = match enctype {
            FormEncType::UrlEncoded => {
                ...
                self.mutate_action_url(form_data, &mut url);
                url.query().unwrap_or("").to_string().into_bytes()
            }
            FormEncType::FormDataEncoded => {
                self.encode_multipart_form_data(form_data, boundary)
            }
