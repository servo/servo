...
use crate::dom::readablestream::{ReadableStream, get_read_promise_bytes};
    ...
    pub(crate) fn chunk_steps(&self, chunk: RootedTraceableBox<Heap<JSVal>>, can_gc: CanGc) {
        match self {
            ...
            ReadRequest::ReadLoop {
                ...
                match get_read_promise_bytes(cx, &chunk.handle(), can_gc) {
                    Ok(vec) => {
...
impl ReadableStreamDefaultReader {
    ...
    pub(crate) fn read_all_bytes(
        ...
        // Spec read-loop: kick off by issuing ReadableStreamDefaultReaderRead with a custom ReadRequest.
