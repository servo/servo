    ...
    fn add_path_(&self, other: &Path2D, transform: &DOMMatrix2DInit){
        ...
        c.iter_mut().for_each(|segment| {
            match *segment {
                ...
                PathSegment::Bezier { cp1x, cp1y, cp2x, cp2y, x, y } => todo!(),
                PathSegment::ArcTo { cp1x, cp1y, cp2x, cp2y, radius } => todo!(),
                PathSegment::Ellipse { x, y, radius_x, radius_y, rotation, start_angle, end_angle, anticlockwise } => todo!(),
                PathSegment::SvgArc { radius_x, radius_y, rotation, large_arc, sweep, x, y } => todo!(),
            }
        });
