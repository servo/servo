...
impl HTMLCanvasElement {
    ...
    pub fn get_or_init_webgl_context(&self,
        ...
        if self.context.borrow().is_none() {
            ...
            let maybe_ctx = WebGLRenderingContext::new(&window, self, WebGLVersion::WebGL2, size, attrs);
    ...
    pub fn get_or_init_webgl2_context(&self,
                                      cx: *mut JSContext,
                                      attrs: Option<HandleValue>)
    ...
    fn get_gl_attributes(cx: *mut JSContext, attrs: Option<HandleValue>) -> Option<GLContextAttributes> {
        if let Some(webgl_attributes) = attrs {
            ...
            Some(GLContextAttributes::default())
        }
    }
