...
impl HTMLCollection {
    ...
    pub fn match_elements_by_qualified_name(window: &Window, root: &Node, mut qualified_name: DOMString)
        ...
        let ascii_lower_qualified_name = Atom::from(qualified_name); // FIXME(ajeffrey): don't clone atom if it was already lowercased.
    ...
    fn match_element(elem: &Element, qualified_name: Atom) -> bool {
        match *elem.prefix() {
            ...
            Some(ref prefix) => qualified_name.starts_with(prefix as &str) &&
                qualified_name.find(":") == Some((prefix as &str).len()) && // TODO(kevgs): replace find() with char access
