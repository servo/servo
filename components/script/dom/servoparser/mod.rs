    ...
    fn process_response(&mut self, _: RequestId, meta_result: Result<FetchMetadata, NetworkError>) {
        ...
        match (
            ...
            (mime::IMAGE, _, _) => {
                ...
                // Prevent special character escaping issues by encoding the image url
                // to base64 before doing the in-page replacement.
                let encoded_url = general_purpose::STANDARD_NO_PAD.encode(self.url.to_string());
                let page = page.replace("${imageUrl}", &encoded_url);
