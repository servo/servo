...
use crate::dom::linkprocessingoptions::{LinkHeader, LinkProcessingOptions, LinkProcessingPhase};
...
impl ParserContext {
    ...
    fn initialize_document_object(&self, document: &Document) {
        ...
        LinkProcessingOptions::process_link_headers(
        ...
        if !link_headers.is_empty() {
            ...
            window
                ...
                .queue(task!(unhandled_rejection_event: move || {
                    LinkProcessingOptions::process_link_headers(&link_headers, &document.root(), LinkProcessingPhase::Media);
                }));
        }
    }
    ...
    fn load_media_document(
        ...
        let link_headers = std::mem::take(&mut self.navigation_params.link_headers);
        LinkProcessingOptions::process_link_headers(&link_headers, doc, LinkProcessingPhase::Media);
    }
    ...
    fn process_response(&mut self, _: RequestId, meta_result: Result<FetchMetadata, NetworkError>) {
        ...
        let (policy_container, endpoints_list, link_headers) = match metadata.as_ref() {
            ...
            Some(metadata) => (
                ...
                LinkProcessingOptions::extract_links_from_headers(&metadata.headers),
            ),
        };
