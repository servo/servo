...
use dom::bindings::root::{DomRoot, Dom, MutNullableDom};
...
pub trait ServoParser<TH: TypeHolderTrait>:
    MutDomObject + MallocSizeOf + JSTraceable + IDLInterface + 'static
{
    fn new_inherited(
    ...
    where
        F: FnMut(&mut Tokenizer<TH>) -> Result<(), DomRoot<HTMLScriptElement<TH>>>;
    ...
    ///     insertion point
    /// ```
    fn resume_with_pending_parsing_blocking_script(
        &self,
    ...
    fn parse_html_document(document: &Document<TH>, input: DOMString, url: ServoUrl);

    fn parse_html_fragment(
        context: &Element<TH>,
        input: DOMString,
    ) -> Box<Iterator<Item = DomRoot<Node<TH>>>>
    where
        Self: Sized;

    fn parse_html_script_input(document: &Document<TH>, url: ServoUrl, type_: &str);

    fn parse_xml_document(document: &Document<TH>, input: DOMString, url: ServoUrl);
...
fn start_element<S: Serializer, TH: TypeHolderTrait>(
    node: &Element<TH>,
    serializer: &mut S,
) -> io::Result<()> {
...
fn end_element<S: Serializer, TH: TypeHolderTrait>(
    node: &Element<TH>,
    serializer: &mut S,
) -> io::Result<()> {
    let name = QualName::new(None, node.namespace().clone(), node.local_name().clone());
...
impl<TH: TypeHolderTrait> FetchResponseListener for ParserContext<TH> {
    ...
    fn process_response(&mut self, meta_result: Result<FetchMetadata, NetworkError>) {
        ...
        let parser: DomRoot<TH::ServoParser> =
            match ScriptThread::<TH>::page_headers_available(&self.id, metadata) {
                Some(parser) => parser,
                None => return,
            };
...
pub fn insert<TH: TypeHolderTrait>(
    parent: &Node<TH>,
    reference_child: Option<&Node<TH>>,
...
impl<TH: TypeHolderTrait> TreeSink for Sink<TH> {
    ...
    fn get_template_contents(&mut self, target: &Dom<Node<TH>>) -> Dom<Node<TH>> {
        let template = target
            .downcast::<HTMLTemplateElement<TH>>()
    ...
    fn associate_with_form(
        ...
        target: &Dom<Node<TH>>,
        form: &Dom<Node<TH>>,
        nodes: (&Dom<Node<TH>>, Option<&Dom<Node<TH>>>),
    ) {
...
#[allow(unsafe_code)]
unsafe impl<TH: TypeHolderTrait> JSTraceable
    for HtmlTokenizer<TreeBuilder<Dom<Node<TH>>, Sink<TH>>>
{
...
#[allow(unsafe_code)]
unsafe impl<TH: TypeHolderTrait> JSTraceable
    for XmlTokenizer<XmlTreeBuilder<Dom<Node<TH>>, Sink<TH>>>
{
    unsafe fn trace(&self, trc: *mut JSTracer) {
        struct Tracer<THH: TypeHolderTrait + 'static>(*mut JSTracer, PhantomData<THH>);
        let tracer = Tracer(trc, Default::default());
