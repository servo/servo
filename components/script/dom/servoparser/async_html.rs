...
pub struct FragmentContext<'a> {
    pub context_elem: &'a ParseNode,
    pub form_elem: Option<&'a ParseNode>,
}
...
enum ParseOperation {
    ...
    CreateElement(usize, QualName, Vec<Attribute>, u64),
...
impl Sink {
    ...
    fn new_parse_node(&self) -> ParseNode {
        ...
        // TODO: Default::default() for ParseNodeData
        let data = ParseNodeData {
            parent: None,
            qual_name: None,
            target: None,
            data: None
    ...
    fn root(&self, node: &ParseNode) -> usize {
        let mut id = node.id;
        while let Some(parent_id) = self.parse_node_data.borrow().get(&id).unwrap().parent {
            id = parent_id
        }
        id
    }
    ...
    fn process_operation(&self, op: ParseOperation) {
        let document = Root::from_ref(&**self.nodes.borrow().get(&0).unwrap());
        ...
        match op {
            ...
            ParseOperation::CreateElement(id, name, attrs, curr_line) => {
                let elem = Element::create(name, &*self.document,
                                           ElementCreator::ParserCreated(curr_line));
                for attr in attrs {
            ...
            ParseOperation::Insert(parent, reference_child, new_node) => {
                ...
                let node = match new_node {
                    ...
                    ParserNodeOrText::Text(ref string) => {
                        NodeOrText::AppendText(Tendril::from_str(string).unwrap())
                    }
                };
...
impl TreeSink for Sink {
    ...
    fn elem_name<'a>(&self, target: &'a Self::Handle) -> ExpandedName<'a> {
        let parse_node_data = self.parse_node_data.borrow();
        let data = parse_node_data.get(&target.id).unwrap();
        let qual_name = data.clone().qual_name.unwrap();
        ExpandedName {
            ns: &qual_name.ns,
    ...
    fn create_element(&mut self, name: QualName, attrs: Vec<Attribute>, _flags: ElementFlags)
        ...
        match self.parse_node_data.borrow_mut().get_mut(&node.id) {
            Some(ref mut node_data) => {
                node_data.qual_name = Some(name.clone());
                self.enqueue(ParseOperation::CreateElement(node.id, name, attrs, self.current_line));
                node
            },
    ...
    fn remove_from_parent(&mut self, target: &Self::Handle) {
        match self.parse_node_data.borrow_mut().get_mut(&target.id) {
            Some(ref mut data) => {
                data.parent = None;
