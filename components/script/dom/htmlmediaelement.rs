...
impl<TH: TypeHolderTrait> HTMLMediaElement<TH> {
    pub fn new_inherited(tag_name: LocalName, prefix: Option<Prefix>, document: &Document<TH>) -> Self {
        Self {
...
impl<TH: TypeHolderTrait> FetchResponseListener for HTMLMediaElementContext<TH> {
    ...
    fn process_response_eof(&mut self, status: Result<(), NetworkError>) {
        ...
        // => "Once the entire media resource has been fetched..."
            ...
            elem.upcast::<EventTarget<TH>>()
                .fire_event(atom!("progress"));

            elem.network_state.set(NetworkState::Idle);

            elem.upcast::<EventTarget<TH>>()
                .fire_event(atom!("suspend"));
        }
