...
use script_thread::{CommonScriptMsg, ScriptThreadEventCategory, Runnable, ScriptChan, ScriptThread};
use std::cell::Cell;
...
impl HTMLMediaElement {
    ...
    fn queue_fire_simple_event(&self, type_: &'static str) {
        ...
        let _ = win.dom_manipulation_thread_source().send(CommonScriptMsg::RunnableMsg(
            ScriptThreadEventCategory::DomEvent, box task));
    }
    ...
    fn resource_fetch_algorithm(&self, resource: Resource) {
        ...
        if let Resource::Url(url) = resource {
            ...
            let script_chan = window_from_node(self).networking_thread_source();
    ...
    fn queue_dedicated_media_source_failure_steps(&self) {
        let _ = window_from_node(self).dom_manipulation_thread_source().send(
            CommonScriptMsg::RunnableMsg(ScriptThreadEventCategory::DomEvent,
...
impl FireSimpleEventTask {
    fn new(target: &HTMLMediaElement, type_: &'static str) -> FireSimpleEventTask {
        ...
        let target_thread = MainThreadScriptChan(win.main_thread_script_chan().clone()).clone();
        FireSimpleEventTask {
            elem: Trusted::new(target, target_thread),
            type_: type_,
        }
    }
}
