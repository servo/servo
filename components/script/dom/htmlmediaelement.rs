...
impl HTMLMediaElement {
    ...
    fn queue_ratechange_event(&self) {
        let task_source = window_from_node(self)
            .task_manager()
            .media_element_task_source();
        task_source.queue_simple_event(self.upcast(), atom!("ratechange"));
    }
    ...
    fn seek(&self, time: f64, _approximate_for_speed: bool) {
        ...
        // Step 10.
        let task_source = self.global().task_manager().media_element_task_source();
        task_source.queue_simple_event(self.upcast(), atom!("seeking"));
    ...
    pub fn process_poster_image_loaded(&self, image: Arc<Image>) {
        ...
        if pref!(media.testing.enabled) {
            let task_source = self.global().task_manager().media_element_task_source();
            task_source.queue_simple_event(self.upcast(), atom!("postershown"));
        }
    }
                    ...
                    match self.direction_of_playback() {
                        PlaybackDirection::Forwards => {
                            ...
                            if self.Loop() {
                                ...
                                let _ = window_from_node(self).task_manager().media_element_task_source().queue(
            ...
            PlayerEvent::MetadataUpdated(ref metadata) => {
                ...
                if previous_duration != self.duration.get() {
                    let task_source = window_from_node(self)
                        .task_manager()
                        .media_element_task_source();
                    task_source.queue_simple_event(self.upcast(), atom!("durationchange"));
                }
