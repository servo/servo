...
impl WebSocket {
    ...
    pub fn new(global: GlobalRef, url: DOMString) -> Temporary<WebSocket> {
        ...
        */
        ...
        let parsed_url = Url::parse(ws_root.url.as_slice()).unwrap(); //Get url
        ...
        ws_root.ready_state.set(WebSocketRequestState::Open); //Set state to open
    ...
    fn Send(self, data: Option<DOMString>)-> Fallible<()>{
        ...
        */
        ...
        if self.sendCloseFrame.get() { //TODO: Also check if the buffer is full
            self.sendCloseFrame.set(false); //Deflag the close frame request
