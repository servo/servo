...
impl HTMLScriptElement {
    ...
    pub fn execute(&self) {
        ...
        if external {
            ...
            window.dom_manipulation_task_source().queue_simple_event(self.upcast(), atom!("load"), Some(window.get_runnable_wrapper()));
        }
    }

    pub fn queue_error_event(&self) {
        let window = window_from_node(self);
        window.dom_manipulation_task_source().queue_simple_event(self.upcast(), atom!("error"), Some(window.get_runnable_wrapper()));
    }
