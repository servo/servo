...
impl FileReaderEvent {
    pub fn new(file_reader_event: FileReaderEvent) -> FileReaderEvent {
        file_reader_event
    }

    pub fn handle(file_reader_event: FileReaderEvent) {
...
fn perform_annotated_read_operation(gen_id: GenerationId, read_data: ReadData,
    ...
    // Step 4
    let task = box FileReaderEvent::new(FileReaderEvent::ProcessRead(filereader.clone(), gen_id));
    chan.send(ScriptMsg::RunnableMsg(task)).unwrap();

    let task = box FileReaderEvent::new(FileReaderEvent::ProcessReadData(filereader.clone(),
    ...
    let output = match read_data.bytes.recv() {
        Ok(bytes) => bytes,
        Err(_) => {
            let task = box FileReaderEvent::new(FileReaderEvent::ProcessReadError(filereader,
    ...
    let task = box FileReaderEvent::new(FileReaderEvent::ProcessReadEOF(filereader,
        gen_id, blob_body));
    chan.send(ScriptMsg::RunnableMsg(task)).unwrap();
}
