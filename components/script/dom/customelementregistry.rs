...
impl CustomElementRegistry {
    ...
    fn get_callbacks(&self, prototype: HandleObject) -> Fallible<LifecycleCallbacks> {
        macro_rules! get_callback {
            ($cx:ident, $proto:ident, $callbacks:ident, $field:ident, $name:expr) => ({
                ...
                if !callback.is_undefined() {
                    ...
                    $callbacks.$field = Some(Function::new($cx, callback.to_object()));
                }
            })
        }

        let cx = self.window.get_cx();
        let mut callbacks = LifecycleCallbacks {
            connected_callback: None,
            disconnected_callback: None,
            adopted_callback: None,
            attribute_changed_callback: None,
        };

        // Step 4
        get_callback!(cx, prototype, callbacks, connected_callback, b"connectedCallback\0");
        get_callback!(cx, prototype, callbacks, disconnected_callback, b"disconnectedCallback\0");
        get_callback!(cx, prototype, callbacks, adopted_callback, b"adoptedCallback\0");
        get_callback!(cx, prototype, callbacks, attribute_changed_callback, b"attributeChangedCallback\0");
