...
impl WindowProxy {
    ...
    pub fn opener(&self, cx: *mut JSContext) -> JSVal {
        ...
        if let Some(opener_browsing_context_id) = self.opener {
            let rooted_opener_proxy = match ScriptThread::find_window_proxy(opener_browsing_context_id) {
                Some(window_proxy) => window_proxy,
                None => {
                    let sender_pipeline_id = self.currently_active().unwrap();
                    if let Some(top_id) = ScriptThread::get_top_level_for_browsing_context(sender_pipeline_id,
    ...
    pub fn open(&self,
        ...
        // TODO Step 4, tokenize features
        let (chosen, new)  = match self.choose_browsing_context(non_empty_target, false) {
