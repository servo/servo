...
use crate::textinput::{ByteOffset, SelectionDirection, SelectionState, TextInput};
...
impl<'a, E: TextControlElement> TextControlSelection<'a, E> {
    ...
    pub fn set_dom_range_text(
        ...
        let ByteOffset(content_length) = self.textinput.borrow().len();
    ...
    fn start(&self) -> u32 {
        let ByteOffset(offset) = self.textinput.borrow().selection_start_offset();
        offset as u32
    }

    fn end(&self) -> u32 {
        let ByteOffset(offset) = self.textinput.borrow().selection_end_offset();
        offset as u32
    }
