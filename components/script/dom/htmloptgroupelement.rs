...
impl<TH: TypeHolderTrait> HTMLOptGroupElement<TH> {
    ...
    pub fn new(
        ...
        prefix: Option<Prefix>,
        document: &Document<TH>,
    ) -> DomRoot<HTMLOptGroupElement<TH>> {
        ...
        match attr.local_name() {
            &local_name!("disabled") => {
                ...
                let options = el
                    .upcast::<Node<TH>>()
                    .children()
                    .filter(|child| child.is::<HTMLOptionElement<TH>>())
                    .map(|child| {
                        DomRoot::from_ref(child.downcast::<HTMLOptionElement<TH>>().unwrap())
                    });
                if disabled_state {
