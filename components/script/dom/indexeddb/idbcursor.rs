...
impl IDBCursorMethods<crate::DomTypeHolder> for IDBCursor {
    ...
    fn ContinuePrimaryKey(
        ...
        if self
            ...
            self.direction == IDBCursorDirection::Next
        {
            return Err(Error::Data(Some("Wrong cursor direction".into())));
        }
    ...
    fn Update(&self, cx: &mut JSContext, value: HandleValue) -> Fallible<DomRoot<IDBRequest>> {
        ...
        let Ok(serialized_value) = postcard::to_stdvec(&cloned_value) else {
            return Err(Error::InvalidState(Some(
                "Failed to serialize cloned value".into(),
            )));
        };
        ...
        if self.effective_object_store().uses_inline_keys() {
            ...
            match kpk {
                ExtractionResult::Failure | ExtractionResult::Invalid => {
                    return Err(Error::Data(Some(
                        "Failed to extract a key from a value".into(),
                    )));
                },
                ExtractionResult::Key(key) => {
                    ...
                    {
                        return Err(Error::Data(Some(
                            "Extracted key does not match the effective key".into(),
                        )));
                    }
                },
            }
    ...
    fn Delete(&self, cx: &mut JSContext) -> Fallible<DomRoot<IDBRequest>> {
        ...
        let effective_key = self
            .effective_key()
            .ok_or(Error::InvalidState(Some("Missing effective key".into())))?;
        IDBRequest::execute_async(
            cx,
            &self.effective_object_store(),
            |callback| {
...
pub(crate) fn iterate_cursor(
    ...
    if !cursor.key_only {
        ...
        postcard::from_bytes(&found_record.value)
            .map_err(|_| Error::Data(Some("Failed to deserialize cursor value".into())))
            .and_then(|data| {
