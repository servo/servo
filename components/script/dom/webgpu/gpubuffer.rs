...
impl DroppableGPUBuffer {
    ...
    fn pending_map(&self) -> &DomRefCell<Option<Rc<Promise>>> {
        &self.pending_map
    }

    fn mapping(&self) -> &DomRefCell<Option<ActiveBufferMapping>> {
        &self.mapping
    }
    ...
    fn unmap(&self) {
        ...
        if let Err(e) = self.channel.0.send(WebGPURequest::UnmapBuffer {
            ...
            warn!("Failed to send Buffer unmap ({:?}) ({})", self.id().0, e);
        }
    }
    ...
    fn destroy(&self) {
        ...
        if let Err(e) = self
            .channel
            .0
            .send(WebGPURequest::DestroyBuffer(self.id().0))
        {
            warn!(
