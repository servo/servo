...
use net::resource_task::{AsyncResponseListener, AsyncResponseTarget, AsyncResponseResult};
    ...
    fn fetch2(xhr: TrustedXHRAddress, script_chan: Box<ScriptChan+Send>,
        ...
        impl AsyncResponseListener for XHRContext {
            fn headers_available(&self, metadata: Metadata) -> AsyncResponseResult {
                ...
                let decision = xhr.r().process_headers_available(self.cors_request.clone(),
                                                                 self.gen_id,
                                                                 metadata);
                if decision.is_err() {
                    AsyncResponseResult::AbortResponse
                } else {
                    AsyncResponseResult::ContinueNormalResponse
                }
            }
