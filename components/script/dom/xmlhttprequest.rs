...
impl XMLHttpRequest {
    ...
    fn sync_in_window(&self) -> bool {
        match self.global() {
            GlobalRoot::Window(_) if self.sync.get() => false,
            _ => true
        }
    }
...
impl XMLHttpRequestMethods for XMLHttpRequest {
    ...
    fn Open(&self, method: ByteString, url: USVString) -> ErrorResult {
        ...
        match self.global() {
            ...
            GlobalRoot::Worker(_) => {} // FIXME: https://github.com/whatwg/xhr/issues/47
        }
    ...
    fn SetTimeout(&self, timeout: u32) -> ErrorResult {
        // Step 1
        if !self.sync_in_window() { return Err(Error::InvalidAccess); }
        // Step 2
    ...
    fn SetWithCredentials(&self, with_credentials: bool) -> ErrorResult {
        match self.ready_state.get() {
            ...
            _ => {
                // Step 3
                if !self.sync_in_window() { Err(Error::InvalidAccess) }
                // Step 4
                else {
