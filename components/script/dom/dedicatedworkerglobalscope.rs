...
impl DedicatedWorkerGlobalScope {
    ...
    pub fn run_worker_scope(init: WorkerGlobalScopeInit,
        ...
        spawn_named(format!("WebWorker for {}", serialized_worker_url), move || {
            ...
            {
                // Create a new scope here to release the lock after mutating main_thread_rt
                let mut guard = main_thread_rt.lock().unwrap();
                *guard = Some(SharedRt::new(&runtime));
            }
            ...
            unsafe {
                let guard = main_thread_rt.lock().unwrap();
                // Handle interrupt requests
                JS_SetInterruptCallback(guard.unwrap().rt(), Some(interrupt_callback));
            }
