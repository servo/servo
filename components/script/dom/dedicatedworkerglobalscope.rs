    ...
    pub fn run_worker_scope(worker_url: Url,
	    ...
	    loop {
                match global.receiver.recv_opt() {
                    ...
                    Ok(WorkerDispatchErrorEvent(addr, type_,bubbles, cancelable,
					        msg, file_name, line_num, col_num, error)) => {
                    	Worker::handle_error_message(addr, type_,bubbles, cancelable,
					        msg, file_name, line_num, col_num, error);
                    },
