...
impl DedicatedWorkerGlobalScope {
    fn new_inherited(init: WorkerGlobalScopeInit,
                     worker_url: Url,
                     id: PipelineId,
                     devtools_script_process_receiver: Receiver<DevtoolScriptControlMsg>,
                     runtime: Rc<Runtime>,
                     parent_sender: Box<ScriptChan+Send>,
                     own_sender: Sender<(TrustedWorkerAddress, WorkerScriptMsg)>,
                     receiver: Receiver<(TrustedWorkerAddress, WorkerScriptMsg)>)
        ...
        DedicatedWorkerGlobalScope {
            workerglobalscope: WorkerGlobalScope::new_inherited(
                ...
                runtime, devtools_script_process_receiver),
            id: id,
            receiver: receiver,
    ...
    pub fn new(init: WorkerGlobalScopeInit,
        ...
        let scope = box DedicatedWorkerGlobalScope::new_inherited(
            init, worker_url, id, devtools_script_process_receiver, runtime.clone(), parent_sender,
            own_sender, receiver);
...
impl DedicatedWorkerGlobalScope {
    pub fn run_worker_scope(init: WorkerGlobalScopeInit,
                            worker_url: Url,
                            id: PipelineId,
                            devtools_script_process_receiver: IpcReceiver<DevtoolScriptControlMsg>,
                            worker: TrustedWorkerAddress,
                            parent_sender: Box<ScriptChan+Send>,
                            own_sender: Sender<(TrustedWorkerAddress, WorkerScriptMsg)>,
                            receiver: Receiver<(TrustedWorkerAddress, WorkerScriptMsg)>) {
        ...
        spawn_named(format!("WebWorker for {}", serialized_worker_url), move || {
            ...
            ROUTER.route_ipc_receiver_to_mpsc_sender(devtools_script_process_receiver, devtools_mpsc_chan);
...
impl<'a> PrivateDedicatedWorkerGlobalScopeHelpers for &'a DedicatedWorkerGlobalScope {
    ...
    fn receive_event(self) -> Result<MixedMessage, RecvError> {
        ...
        let devtools_port = scope.devtools_script_receiver();
        ...
        unsafe {
            worker_handle.add();
            if scope.devtools_script_sender().is_some() {
                devtools_handle.add();
            }
        }
        let ret = sel.wait();
