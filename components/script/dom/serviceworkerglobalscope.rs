...
use dom::serviceworker::{SimpleServiceWorkerErrorHandler, SharedRt, TrustedServiceWorkerAddress};
use dom::workerglobalscope::WorkerGlobalScope;
use dom::workerglobalscope::WorkerGlobalScopeInit;
use ipc_channel::ipc::{self, IpcReceiver, IpcSender};
use ipc_channel::router::ROUTER;
use js::jsapi::{JS_SetInterruptCallback, JSAutoCompartment, JSAutoRequest, JSContext, RootedValue};
...
/// A ScriptChan that can be cloned freely and will silently send a TrustedServiceWorkerAddress with
/// common event loop messages. While this SendableServiceWorkerScriptChan is alive, the associated
/// Worker object will remain alive.
#[derive(JSTraceable, Clone)]
pub struct SendableServiceWorkerScriptChan {
    sender: Sender<(TrustedServiceWorkerAddress, CommonScriptMsg)>,
    worker: TrustedServiceWorkerAddress,
}

impl ScriptChan for SendableServiceWorkerScriptChan {
    fn send(&self, msg: CommonScriptMsg) -> Result<(), ()> {
        self.sender.send((self.worker.clone(), msg)).map_err(|_| ())
    }

    fn clone(&self) -> Box<ScriptChan + Send> {
        box SendableServiceWorkerScriptChan {
...
/// A ScriptChan that can be cloned freely and will silently send a TrustedServiceWorkerAddress with
/// worker event loop messages. While this SendableServiceWorkerScriptChan is alive, the associated
/// Worker object will remain alive.
#[derive(JSTraceable, Clone)]
pub struct ServiceWorkerThreadWorkerChan {
    sender: Sender<(TrustedServiceWorkerAddress, ServiceWorkerScriptMsg)>,
    worker: TrustedServiceWorkerAddress,
}

impl ScriptChan for ServiceWorkerThreadWorkerChan {
    fn send(&self, msg: CommonScriptMsg) -> Result<(), ()> {
        self.sender
            .send((self.worker.clone(), ServiceWorkerScriptMsg::Common(msg)))
            .map_err(|_| ())
    }

    fn clone(&self) -> Box<ScriptChan + Send> {
        box ServiceWorkerThreadWorkerChan {
            sender: self.sender.clone(),
            worker: self.worker.clone(),
        }
    }
...
impl ScriptPort for Receiver<(TrustedServiceWorkerAddress, ServiceWorkerScriptMsg)> {
    fn recv(&self) -> Result<CommonScriptMsg, ()> {
        match self.recv().map(|(_, msg)| msg) {
            Ok(ServiceWorkerScriptMsg::Common(script_msg)) => Ok(script_msg),
            Ok(ServiceWorkerScriptMsg::DOMMessage(_)) => panic!("unexpected service worker event message!"),
            Err(_) => Err(()),
        }
    }
}
...
impl<'a> AutoWorkerReset<'a> {
    fn new(workerscope: &'a ServiceWorkerGlobalScope,
        ...
        AutoWorkerReset {
            ...
            old_worker: replace(&mut *workerscope.service_worker.borrow_mut(), Some(worker)),
        }
    }
}
...
impl<'a> Drop for AutoWorkerReset<'a> {
    fn drop(&mut self) {
        *self.workerscope.service_worker.borrow_mut() = self.old_worker.clone();
    }
}

enum MixedMessage {
    FromServiceWorker((TrustedServiceWorkerAddress, ServiceWorkerScriptMsg)),
...
pub struct ServiceWorkerGlobalScope {
    ...
    receiver: Receiver<(TrustedServiceWorkerAddress, ServiceWorkerScriptMsg)>,
    #[ignore_heap_size_of = "Defined in std"]
    own_sender: Sender<(TrustedServiceWorkerAddress, ServiceWorkerScriptMsg)>,
    #[ignore_heap_size_of = "Defined in std"]
    ...
    service_worker: DOMRefCell<Option<TrustedServiceWorkerAddress>>,
...
impl ServiceWorkerGlobalScope {
    fn new_inherited(init: WorkerGlobalScopeInit,
        ...
        ServiceWorkerGlobalScope {
            ...
            service_worker: DOMRefCell::new(None),
        }
    }
        ...
        spawn_named(format!("ServiceWorker for {}", serialized_worker_url), move || {
            ...
            let (url, source) = match load_whole_resource(LoadContext::Script,
                ...
                Err(_) => {
                    ...
                    parent_sender.send(CommonScriptMsg::RunnableMsg(ServiceWorkerEvent,
                        box SimpleServiceWorkerErrorHandler::new(worker))).unwrap();
                    return;
                }
                Ok((metadata, bytes)) => {
            ...
            *main_thread_rt.lock().unwrap() = Some(SharedRt::new(&runtime));
            ...
            scope.mem_profiler_chan().run_with_memory_reporting(|| {
                ...
                let sw_lifetime_timeout = prefs::get_pref("dom.serviceworker.timeout").as_u64().unwrap();
    ...
    fn handle_event(&self, event: MixedMessage) {
        match event {
            ...
            MixedMessage::FromScheduler((linked_worker, timer_event)) => {
                match timer_event {
                    TimerEvent(TimerSource::FromServiceWorker, id) => {
    ...
    fn handle_script_event(&self, msg: ServiceWorkerScriptMsg) {
        match msg {
            ServiceWorkerScriptMsg::DOMMessage(data) => {
                let scope = self.upcast::<WorkerGlobalScope>();
            ...
            ServiceWorkerScriptMsg::Common(CommonScriptMsg::RunnableMsg(_, runnable)) => {
                runnable.handler()
            },
            ServiceWorkerScriptMsg::Common(CommonScriptMsg::RefcountCleanup(addr)) => {
                LiveDOMReferences::cleanup(addr);
            },
            ServiceWorkerScriptMsg::Common(CommonScriptMsg::CollectReports(reports_chan)) => {
                let scope = self.upcast::<WorkerGlobalScope>();
                let cx = scope.get_cx();
    ...
    pub fn script_chan(&self) -> Box<ScriptChan + Send> {
        box ServiceWorkerThreadWorkerChan {
            ...
            worker: self.service_worker.borrow().as_ref().unwrap().clone(),
        }
    }
    ...
    pub fn process_event(&self, msg: CommonScriptMsg) {
        self.handle_script_event(ServiceWorkerScriptMsg::Common(msg));
    }

    pub fn new_script_pair(&self) -> (Box<ScriptChan + Send>, Box<ScriptPort + Send>) {
        let (tx, rx) = channel();
        let chan = box SendableServiceWorkerScriptChan {
            sender: tx,
            worker: self.service_worker.borrow().as_ref().unwrap().clone(),
        };
        (chan, box rx)
