...
pub enum MixedMessage {
    ...
    FromTimeoutThread((IpcSender<ServiceWorkerMsg>, Url)),
}
...
pub struct ServiceWorkerGlobalScope {
    ...
    #[ignore_heap_size_of = "Defined in std"]
    timer_event_port: Receiver<(IpcSender<ServiceWorkerMsg>, Url)>,
...
impl ServiceWorkerGlobalScope {
    ...
    fn handle_event(&self, event: MixedMessage) {
        match event {
            ...
            MixedMessage::FromTimeoutThread((mgr_sender, scope_url)) => {
                let _ = mgr_sender.send(ServiceWorkerMsg::Timeout(scope_url));
            }
        }
    }
