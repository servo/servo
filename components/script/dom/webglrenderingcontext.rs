...
impl WebGLRenderingContext {
    ...
    fn validate_tex_image_2d_data(&self,
                                  ...
                                  expected_byte_length: &mut i32) -> bool {
        ...
        *expected_byte_length = width * height * element_size * components / components_per_element;
        return true;
    }
    ...
    fn tex_sub_image_2d(&self,
        ...
        if Some(format) != image_info.format() ||
            Some(data_type) != image_info.data_type() {
            self.webgl_error(InvalidOperation);
        }
...
impl WebGLRenderingContextMethods for WebGLRenderingContext {
    ...
    fn TexImage2D(&self,
        ...
        let mut expected_byte_length = 0;
        if !self.validate_tex_image_2d_data(width,
                                            height,
                                            format,
                                            data_type,
                                            data,
                                            &mut expected_byte_length) {
            return;
        }
    ...
    fn TexImage2D_(&self,
        ...
        let mut size = Size2D::new(0 as i32, 0 as i32);

        // Get pixels from image source
        if !self.get_image_pixels(source, &mut pixels, &mut size) {
            return;
        }
