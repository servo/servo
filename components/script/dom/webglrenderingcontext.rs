...
use dom::bindings::conversions::{ToJSValConvertible, array_buffer_view_to_vec_checked, array_buffer_view_to_vec, array_buffer_view_data_checked};
...
impl WebGLRenderingContext {
    ...
    fn validate_uniform_parameters<T>(&self,
                                   ...
                                   type_: UniformType,
                                   data: Option<&[T]>) -> bool {
        // If the passed location is null, data passed in will be silently
        ...
        // If the uniform location passed in does not belong to the current program,
        // or if it is null, the spec mandates we raise an InvalidOperation error and
        // do not change any uniform variables
        ...
        // If the data passed in is null, we raise an InvalidOperation error and
        // do not change any uniform variables
        let data = match data {
            Some(data) => data,
            None => {
        ...
        // If the uniform is not active, we raise an InvalidOperation
        ...
        // The length of the data passed in must be a multiple of the uniform's variable size,
        // and if it isn't, an InvalidOperation error is raised.
        ...
        // If the type of the uniform variable does not match the expected type,
        // raise an InvalidOperation
