...
impl WebGLRenderingContext {
    ...
    // Validate a given shader source. This method is basically the Rust version of
    // https://searchfox.org/mozilla-central/source/dom/canvas/WebGLValidateStrings.cpp#177 
    fn validate_glsl_string(&self, source: &DOMString) -> WebGLResult<()> {
        ...
        // Iterate over each character; if an invalid character is found, break out of the loop.
        let mut valid = false;
        ...
        // Error out if we saw an invalid character.
        if valid {
            Ok(())
        } else {
            Err(InvalidValue)
...
impl WebGLRenderingContextMethods for WebGLRenderingContext {
    ...
    fn ShaderSource(&self, shader: &WebGLShader, source: DOMString) {
        //let valid = self.validate_glsl_string(&source);
