...
macro_rules! bound_texture {
    ($self_:expr, $name:ident) => {
        $self_.bound_textures.borrow().get(&$self_.bound_texture_unit.get()).and_then(|item| {
            item.$name.get()
        })
    };
}
...
impl WebGLRenderingContext {
    ...
    pub fn recreate(&self, size: Size2D<i32>) {
        ...
        if let Some(texture) = bound_texture!(self, bound_texture_2d) {
    ...
    fn tex_parameter(&self, target: u32, name: u32, value: TexParameterValue) {
        let texture = match target {
            constants::TEXTURE_2D => bound_texture!(self, bound_texture_2d),
            constants::TEXTURE_CUBE_MAP => bound_texture!(self, bound_texture_cube_map),
    ...
    unsafe fn GetParameter(&self, cx: *mut JSContext, parameter: u32) -> JSVal {
        ...
        match parameter {
            ...
            constants::TEXTURE_BINDING_2D => {
                let texture = bound_texture!(self, bound_texture_2d);
                return optional_root_object_to_js_or_null!(cx, texture)
            },
            constants::TEXTURE_BINDING_CUBE_MAP => {
                let texture = bound_texture!(self, bound_texture_cube_map);
                return optional_root_object_to_js_or_null!(cx, texture)
            },
