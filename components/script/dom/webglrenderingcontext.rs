...
impl WebGLRenderingContext {
    ...
    fn validate_uniform_parameters<T>(&self,
        ...
        if data.len() % type_.element_size() != 0 ||
            (data.len() / type_.element_size() > active_uniform.Size() as usize) {
                self.webgl_error(InvalidOperation);
                return false;
        }

        if type_.to_constant() != active_uniform.Type() {
            self.webgl_error(InvalidOperation);
            return false;
        }
...
impl WebGLRenderingContextMethods for WebGLRenderingContext {
    ...
    fn Uniform1iv(&self,
        ...
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<i32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::Int, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform1iv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform1fv(&self,
                  ...
                  uniform: Option<&WebGLUniformLocation>,
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<f32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::Float, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform1fv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform2fv(&self,
                  ...
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<f32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::FloatVec2, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform2fv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform2iv(&self,
                  ...
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<i32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::IntVec2, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform2iv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform3fv(&self,
                  ...
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<f32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::FloatVec3, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform3fv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform3iv(&self,
                  ...
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<i32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::IntVec3, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform3iv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform4iv(&self,
                  ...
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<i32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::IntVec4, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform4iv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
    ...
    fn Uniform4fv(&self,
                  ...
                  data: Option<*mut JSObject>) {
        let data_slice = data.and_then(|d| array_buffer_view_to_vec::<f32>(d));
        if self.validate_uniform_parameters(uniform, UniformType::FloatVec4, data_slice.as_ref().map(Vec::as_slice)) {
            self.ipc_renderer
                .send(CanvasMsg::WebGL(WebGLCommand::Uniform4fv(uniform.unwrap().id(), data_slice.unwrap())))
                .unwrap()
        }
    }
