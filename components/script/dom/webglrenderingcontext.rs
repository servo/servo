...
pub struct WebGLRenderingContext<TH: TypeHolderTrait> {
    ...
    default_vao: DomOnceCell<WebGLVertexArrayObjectOES<TH>>,
    current_vao: MutNullableDom<WebGLVertexArrayObjectOES<TH>>,
    textures: Textures<TH>,
}
...
impl<TH: TypeHolderTrait> WebGLRenderingContext<TH> {
    ...
    fn current_vao(&self) -> DomRoot<WebGLVertexArrayObjectOES<TH>> {
    ...
    pub fn textures(&self) -> &Textures<TH> {
        &self.textures
    }
    ...
    pub fn array_buffer(&self) -> Option<DomRoot<WebGLBuffer<TH>>> {
    ...
    pub fn create_vertex_array(&self) -> Option<DomRoot<WebGLVertexArrayObjectOES<TH>>> {
        let (sender, receiver) = webgl_channel().unwrap();
    ...
    pub fn delete_vertex_array(&self, vao: Option<&WebGLVertexArrayObjectOES<TH>>) {
        if let Some(vao) = vao {
    ...
    pub fn is_vertex_array(&self, vao: Option<&WebGLVertexArrayObjectOES<TH>>) -> bool {
    ...
    pub fn bind_vertex_array(&self, vao: Option<&WebGLVertexArrayObjectOES<TH>>) {
        if let Some(vao) = vao {
            // The default vertex array has no id and should never be passed around.
            assert!(vao.id().is_some());
            handle_potential_webgl_error!(self, self.validate_ownership(vao), return);
    ...
    pub fn bound_framebuffer(&self) -> Option<DomRoot<WebGLFramebuffer<TH>>> {
...
impl<TH: TypeHolderTrait> WebGLRenderingContextMethods<TH> for WebGLRenderingContext<TH> {
    ...
    fn GetActiveUniform(
        ...
        index: u32,
    ) -> Option<DomRoot<WebGLActiveInfo<TH>>> {
    ...
    fn IsBuffer(&self, buffer: Option<&WebGLBuffer<TH>>) -> bool {
        buffer.map_or(false, |buf| {
            self.validate_ownership(buf).is_ok() &&
                buf.target().is_some() &&
                !buf.is_marked_for_deletion()
        })
    }
    ...
    // https://www.khronos.org/registry/webgl/specs/latest/1.0/#5.14.10
    fn Uniform1f(&self, location: Option<&WebGLUniformLocation<TH>>, val: f32) {
    ...
    // https://www.khronos.org/registry/webgl/specs/latest/1.0/#5.14.10
    fn Uniform1i(&self, location: Option<&WebGLUniformLocation<TH>>, val: i32) {
        self.with_location(location, |location| {
            match location.type_() {
    ...
    // https://www.khronos.org/registry/webgl/specs/latest/1.0/#5.14.10
    fn Uniform1iv(&self, location: Option<&WebGLUniformLocation<TH>>, val: Int32ArrayOrLongSequence) {
        self.with_location(location, |location| {
            match location.type_() {
                constants::BOOL |
                constants::INT |
    ...
    // https://www.khronos.org/registry/webgl/specs/latest/1.0/#5.14.10
    fn Uniform2iv(&self, location: Option<&WebGLUniformLocation<TH>>, val: Int32ArrayOrLongSequence) {
        self.with_location(location, |location| {
            match location.type_() {
                constants::BOOL_VEC2 | constants::INT_VEC2 => {},
                _ => return Err(InvalidOperation),
    ...
    // https://www.khronos.org/registry/webgl/specs/latest/1.0/#5.14.10
    fn Uniform3iv(&self, location: Option<&WebGLUniformLocation<TH>>, val: Int32ArrayOrLongSequence) {
        self.with_location(location, |location| {
            match location.type_() {
                constants::BOOL_VEC3 | constants::INT_VEC3 => {},
                _ => return Err(InvalidOperation),
    ...
    // https://www.khronos.org/registry/webgl/specs/latest/1.0/#5.14.10
    fn Uniform4iv(&self, location: Option<&WebGLUniformLocation<TH>>, val: Int32ArrayOrLongSequence) {
        self.with_location(location, |location| {
            match location.type_() {
                constants::BOOL_VEC4 | constants::INT_VEC4 => {},
                _ => return Err(InvalidOperation),
    ...
    fn GetAttachedShaders(
        &self,
        program: &WebGLProgram<TH>,
    ) -> Option<Vec<DomRoot<WebGLShader<TH>>>> {
        handle_potential_webgl_error!(self, self.validate_ownership(program), return None);
...
impl<TH: TypeHolderTrait> LayoutCanvasWebGLRenderingContextHelpers
    for LayoutDom<WebGLRenderingContext<TH>>
{
    #[allow(unsafe_code)]
    unsafe fn canvas_data_source(&self) -> HTMLCanvasDataSource {
...
#[must_root]
#[derive(JSTraceable, MallocSizeOf)]
pub struct Textures<TH: TypeHolderTrait> {
    active_unit: Cell<u32>,
    units: Box<[TextureUnit<TH>]>,
}

impl<TH: TypeHolderTrait> Textures<TH> {
    ...
    fn active_texture_slot(&self, target: u32) -> WebGLResult<&MutNullableDom<WebGLTexture<TH>>> {
    ...
    fn active_unit(&self) -> &TextureUnit<TH> {
        &self.units[self.active_unit.get() as usize]
    }

    fn iter(&self) -> impl Iterator<Item = (u32, &TextureUnit<TH>)> {
        self.units
            .iter()
            .enumerate()
...
#[must_root]
#[derive(Default, JSTraceable, MallocSizeOf)]
struct TextureUnit<TH: TypeHolderTrait> {
    tex_2d: MutNullableDom<WebGLTexture<TH>>,
    tex_cube_map: MutNullableDom<WebGLTexture<TH>>,
}

impl<TH: TypeHolderTrait> TextureUnit<TH> {
    fn unbind(&self, texture: &WebGLTexture<TH>) -> Option<u32> {
        let fields = [
