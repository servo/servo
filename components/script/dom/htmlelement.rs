...
impl HTMLElement {
    ...
    pub fn labels(&self) -> DomRoot<NodeList> {
        ...
        impl LiveListGenerator for LiveLabels {
            ...
            fn generate(&self) -> Ref<Vec<Dom<Node>>> {
                ...
                let ancestors = self
                    ...
                    .filter(move |elem| {
                        elem.first_labelable_descendant().deref() == Some(&*html_element)
                    })
        ...
        root.add_mutation_observer(RegisteredObserver::LiveDom(live_labels.clone()));
