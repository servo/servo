...
#[dom_struct]
pub struct HTMLElement<TH: TypeHolderTrait> {
    element: Element<TH>,
...
impl<TH: TypeHolderTrait> HTMLElementMethods<TH> for HTMLElement<TH> {
    ...
    fn SetInnerText(&self, input: DOMString) {
        ...
        // Step 7.
        Node::<TH>::replace_all(Some(fragment.upcast()), self.upcast::<Node<TH>>());
    }
}

fn append_text_node_to_fragment<TH: TypeHolderTrait>(document: &Document<TH>, fragment: &DocumentFragment<TH>, text: String) {
...
impl<TH: TypeHolderTrait> HTMLElement<TH> {
        ...
        match self.upcast::<Node<TH>>().type_id() {
            NodeTypeId::Element(ElementTypeId::HTMLElement(type_id)) => match type_id {
                HTMLElementTypeId::HTMLInputElement => {
                    self.downcast::<HTMLInputElement<TH>>()
                        .unwrap()
                        .input_type() !=
                        InputType::Hidden
                },
    ...
    pub fn labels(&self) -> DomRoot<NodeList<TH>> {
        ...
        let ancestors = self.upcast::<Node<TH>>()
                .ancestors()
