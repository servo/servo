...
impl MLGraphBuilder {
    ...
    fn create_element_wise_binary(
        ...
        debug_assert!(["add", "sub", "mul", "div", "max", "min", "pow"].contains(&operation));
        ...
        // Note: We don't keep an [[operator]] slot on the DOM MLOperand; the
        // operator and its connectivity are recorded on the backend via
        // channel.add_operator.
...
impl MLGraphBuilderMethods<crate::DomTypeHolder> for MLGraphBuilder {
    ...
    fn Input(
        ...
        // Note: The name is recorded on the backend via channel.add_input, not
        // on the DOM MLOperand.
    ...
    fn Build(
        ...
        // Step 4.3. If operand is in this's graph's inputs or constants,
        ...
        // Step 5. Let operands be a new empty set.
        // Step 6. Let operators be a new empty set.
        // Step 7. Let inputs be a new empty set.
        // Step 8. Let queue be a new queue containing outputs's values.
        // Step 9. While queue is not empty:
        // Step 9.1. Dequeue operand from queue.
        // Step 9.2. Append operand to operands.
        // Step 9.3. Append operand.[[operator]] to operators.
        // Step 9.4. If operand is in this's graph's inputs, then append operand
        // to inputs.
        // Step 9.5. For each input of operand.[[operator]]'s inputs, enqueue
        // input to queue.
        // Note: Steps 5-9 (graph traversal) are performed on the backend during
        // channel.build().
        ...
        // Step 14. For each operand in inputs:
        // Step 14.1. Set graph.[[inputDescriptors]][operand.[[name]]] to
        // operand.[[descriptor]].
        // Note: inputDescriptors are not stored on MLGraph; the backend derives
        // them from the graph during channel.build().
        ...
        // graph.
        // Note: Build runs asynchronously on the backend thread; the callback
        // resolves/rejects the promise via RoutedPromiseListener on MLGraph,
        // queued on the ml_task_source.
        ...
        // Note: Checked in MLGraph::handle_response when the async build result
        // arrives on the script thread.
