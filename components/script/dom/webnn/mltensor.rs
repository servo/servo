...
impl MLTensor {
    ...
    pub(crate) fn write_data(&self, source: &[u8]) {
        if let Some(ref mut buffer) = *self.data.borrow_mut() {
...
impl MLTensorMethods<crate::DomTypeHolder> for MLTensor {
    ...
    fn Destroy(&self) {
        ...
        // Note: We release the data synchronously instead of enqueueing on a
        // timeline.
