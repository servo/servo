...
/// <https://www.w3.org/TR/webnn/#appendices-mloperanddatatype-arraybufferview-compatibility>
fn data_type_matches_view_type(data_type: MLOperandDataType, view_type: Type) -> bool {
    match data_type {
        MLOperandDataType::Float32 => view_type == Type::Float32,
        MLOperandDataType::Float16 => view_type == Type::Float16,
        MLOperandDataType::Int32 => view_type == Type::Int32,
        MLOperandDataType::Uint32 => view_type == Type::Uint32,
        MLOperandDataType::Int64 => view_type == Type::BigInt64,
        MLOperandDataType::Uint64 => view_type == Type::BigUint64,
        MLOperandDataType::Int8 => view_type == Type::Int8,
        MLOperandDataType::Uint8 => view_type == Type::Uint8,
    }
}
...
pub(crate) fn validate_buffer_with_descriptor(
    ...
    let buffer_source_byte_length = match buffer {
        MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer::ArrayBufferView(view) => {
            get_buffer_source_copy(view.into()).len()
        },
        MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer::ArrayBuffer(buffer) => {
            get_buffer_source_copy(buffer.into()).len()
        },
    };
    if buffer_source_byte_length != operand_descriptor_byte_length(descriptor) {
        return false;
    }
    // Step 2. Switch on the type of bufferSource:
    // Step 2.1. If bufferSource is an ArrayBuffer, then return true.
    // Step 2.2. If bufferSource is a SharedArrayBuffer, then return true.
    // Note: Our BufferSource union has no SharedArrayBuffer variant, so Step 2.2
    // is unreachable.
    // Step 2.3. If bufferSource is an ArrayBufferView:
    // Step 2.3.1. If bufferSource is a Uint8Array object, then return true.
    // Step 2.3.2. If bufferSource matches descriptor's dataType according to the
    // table in §appendices-mloperanddatatype-arraybufferview-compatibility, then
    // return true.
    // Step 2.3.3. Return false.
    match buffer {
        MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer::ArrayBuffer(_) => true,
        MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer::ArrayBufferView(view) => {
            let view_type = view.get_array_type();
            view_type == Type::Uint8 || data_type_matches_view_type(descriptor.dataType, view_type)
        },
    }
}
...
pub(crate) struct MLContext {
    ...
    channel: webnn::WebNN,
    /// Identifier of this context on the shared WebNN backend thread.
    #[no_trace]
...
impl MLContext {
    pub(crate) fn new_inherited(options: &MLContextOptions) -> MLContext {
        let context_id = webnn::ContextId::new();
        let power_preference = match options.powerPreference {
            MLPowerPreference::Default => webnn::BackendPowerPreference::Default,
    ...
    pub(crate) fn new(
        ...
        reflect_weak_referenceable_dom_object(
            cx,
            Rc::new(MLContext::new_inherited(options)),
            global,
        )
    }
    ...
    /// Identifier of this context on the shared WebNN backend thread.
    pub(crate) fn context_id(&self) -> webnn::ContextId {
        self.context_id
    }
...
impl MLContextMethods<crate::DomTypeHolder> for MLContext {
    ...
    fn Dispatch(
        ...
        // TODO Step 6. If validating tensors with descriptors given inputs and
        // graph.[[inputDescriptors]] returns false, then throw a TypeError.
        ...
        // Note: We run compute on the shared WebNN backend thread instead of a
        // per-context timeline; the results are written back to the output
        // tensors via a task queued on the ml_task_source.
                ...
                match response.result {
                    Ok(result) => {
                        for (name, trusted_tensor) in output_tensors.iter() {
                            ...
                            let index = output_labels
                                .iter()
                                .position(|n| n == name.as_str())
                                .unwrap_or(0);
                            if let Some(data) = result.outputs.get(index) {
    ...
    fn CreateTensor(
        ...
        // Note: We allocate the data synchronously instead of enqueueing on a
        // timeline.
    ...
    fn CreateConstantTensor(
        ...
        // Note: We write the data synchronously instead of enqueueing on a
        // timeline.
    ...
    fn ReadTensor(&self, cx: &mut js::context::JSContext, tensor: &MLTensor) -> Rc<Promise> {
        ...
        // Note: We read the data synchronously instead of enqueueing on a
        // timeline.
        // Step 8.1. Run these steps, but abort when this is lost:
    ...
    fn ReadTensor_(
        ...
        // tensor.[[context]].[[timeline]]:
        // Note: We copy the data synchronously instead of enqueueing on a
        // timeline.
        ...
        let len = data.len();
        match output_data {
            MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer::ArrayBuffer(ref mut buffer) => {
                let slice = buffer.as_mut_slice_safe(cx.no_gc()).unwrap_or(&mut []);
            ...
            MaybeSharedArrayBufferViewOrMaybeSharedArrayBuffer::ArrayBufferView(ref mut view) => {
                let slice = view.as_mut_slice_safe(cx.no_gc()).unwrap_or(&mut []);
                let end = len.min(slice.len());
                slice[..end].copy_from_slice(&data[..end]);
            },
        }
    ...
    fn OpSupportLimits(&self) -> MLOpSupportLimits {
        // Note: The spec defines no normative algorithm steps for this method;
        // the returned limits are implementation-defined.
