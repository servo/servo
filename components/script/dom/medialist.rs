...
impl MediaListMethods for MediaList {
    ...
    fn AppendMedium(&self, medium: DOMString) {
        ...
        self.media_queries.read().media_queries.iter().map(|query| {
            if m_serialized == query.to_css_string() {
                return;
            }
        });
        // Step 4
    ...
    fn DeleteMedium(&self, medium: DOMString) {
        ...
        let queries = media_list.media_queries.clone();
        let iter = queries.iter().enumerate();
        for (i, query) in iter {
            if m_serialized == query.to_css_string() {
                media_list.media_queries.remove(i);
            }
        }
    }
}
