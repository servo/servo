...
#[inline]
fn to_hex(value: u8) -> Option<char> {
    match value {
...
fn serialize<W>(color: &RGBA, dest: &mut W) -> fmt::Result where W: fmt::Write {
    if color.alpha == 1f32 {
        ...
        write!(dest, "#{}{}{}{}{}{}",
             to_hex(red >> 4).unwrap(), to_hex(red & 0xF).unwrap(),
             to_hex(green >> 4).unwrap(), to_hex(green & 0xF).unwrap(),
             to_hex(blue >> 4).unwrap(), to_hex(blue & 0xF).unwrap())
    } else {
        write!(dest, "rgba({}, {}, {}, {})",
               (color.red * 255.).round(),
