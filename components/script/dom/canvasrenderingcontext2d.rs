...
impl<'a> CanvasRenderingContext2DMethods for &'a CanvasRenderingContext2D {
    ...
    fn SetFillStyle(self, value: StringOrCanvasGradientOrCanvasPattern) {
        match value {
            ...
            StringOrCanvasGradientOrCanvasPattern::eCanvasGradient(gradient) => {
                self.renderer.send(CanvasMsg::Canvas2d(Canvas2dMsg::SetFillStyle(gradient.r().to_fill_or_stroke_style()))).unwrap();
            }
            _ => {}
        }
    }
    ...
    fn GetImageData(self, sx: Finite<f64>, sy: Finite<f64>, sw: Finite<f64>, sh: Finite<f64>) -> Fallible<Root<ImageData>> {
        let sx = *sx;
        let sy = *sy;
        let sw = *sw;
        let sh = *sh;
    ...
    fn CreateRadialGradient(self, x0: Finite<f64>, y0: Finite<f64>, r0: Finite<f64>,
                                  x1: Finite<f64>, y1: Finite<f64>, r1: Finite<f64>) -> Fallible<Root<CanvasGradient>> {
        let x0 = *x0;
        let y0 = *y0;
        let r0 = *r0;
        let x1 = *x1;
