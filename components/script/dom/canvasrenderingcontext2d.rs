...
use dom::bindings::codegen::Bindings::HTMLImageElementBinding::HTMLImageElementBinding::HTMLImageElementMethods;
    ...
    fn is_origin_clean(&self,
        ...
        match image {
            ...
            HTMLImageElementOrHTMLCanvasElementOrCanvasRenderingContext2D::HTMLImageElement(image) =>
                match image.get_origin() {
                    None => {
                        let global_scope = GlobalScope::entry();
                        image.get_url().map(|url| url.origin() == global_scope.get_url().origin()).unwrap_or(true)
                    },
                    _ => {
                        let document = document_from_node(&*self.canvas);
                        let global_scope = GlobalScope::entry();
                        document.url().clone().origin() == global_scope.get_url().origin()
                    }
                }
        }
    }
