...
impl CanvasRenderingContext2D {
    fn new_inherited(global: GlobalRef,
        ...
        CanvasRenderingContext2D {
            ...
            origin_clean: DOMRefCell::new(true),
        }
    }
            ...
            HTMLImageElementOrHTMLCanvasElementOrCanvasRenderingContext2D::eHTMLImageElement(image) =>
                match image.get_url() {
                    ...
                    Some(url) => {
                        ...
                        // the entry settings object, but for now just compare the urls.
                        let node: &Node = &*self.canvas.upcast();
                        let doc = node.owner_doc();
                        let canvas_url = doc.url();
                        url.serialize_host() == canvas_url.serialize_host() &&
                            url.port() == canvas_url.port() &&
                            url.scheme == canvas_url.scheme
                    }
                }
        }
    }
    ...
    pub fn origin_is_clean(&self) -> bool {
        *self.origin_clean.borrow()
    }

    fn set_origin_unclean(&self) {
        *self.origin_clean.borrow_mut() = false
    }
}
