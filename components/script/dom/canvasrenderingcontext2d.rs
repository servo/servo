...
impl<TH: TypeHolderTrait> CanvasRenderingContext2D<TH> {
    ...
    pub fn new(
        ...
        size: Size2D<u32>,
    ) -> DomRoot<CanvasRenderingContext2D<TH>> {
    ...
    fn is_origin_clean(&self, image: CanvasImageSource<TH>) -> bool {
        match image {
            CanvasImageSource::HTMLCanvasElement(canvas) => canvas.origin_is_clean(),
            CanvasImageSource::HTMLImageElement(image) => {
                image.same_origin(GlobalScope::<TH>::entry().origin())
            },
...
impl<TH: TypeHolderTrait> LayoutCanvasRenderingContext2DHelpers
    for LayoutDom<CanvasRenderingContext2D<TH>>
{
    #[allow(unsafe_code)]
    unsafe fn get_ipc_renderer(&self) -> IpcSender<CanvasMsg> {
...
impl<TH: TypeHolderTrait> CanvasRenderingContext2DMethods<TH> for CanvasRenderingContext2D<TH> {
    ...
    // https://html.spec.whatwg.org/multipage/#dom-context-2d-drawimage
    fn DrawImage(&self, image: CanvasImageSource<TH>, dx: f64, dy: f64) -> ErrorResult {
