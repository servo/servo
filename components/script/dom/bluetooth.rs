...
impl<TH: TypeHolderTrait> Bluetooth<TH> {
    ...
    fn request_bluetooth_devices(
        ...
        if let PermissionState::Denied =
            get_descriptor_permission_state::<TH>(PermissionName::Bluetooth, None)
        {
...
where
    T: AsyncBluetoothListener<TH> + DomObject<TypeHolder = TH> + 'static,
...
fn canonicalize_filter<TH: TypeHolderTrait>(
    filter: &BluetoothLEScanFilterInit,
...
impl<TH: TypeHolderTrait> PermissionAlgorithm<TH> for Bluetooth<TH> {
    ...
    fn permission_query(
        ...
        // Step 2.
        status.set_state(get_descriptor_permission_state::<TH>(
            status.get_query(),
            None,
        ));
