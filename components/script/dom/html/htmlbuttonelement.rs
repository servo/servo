...
impl HTMLButtonElementMethods<crate::DomTypeHolder> for HTMLButtonElement {
    ...
    fn Command(&self) -> DOMString {
        ...
        match self.command_state() {
            ...
            CommandState::Custom => {
                self.upcast::<Element>().get_string_attribute(&local_name!("command"))
            },
...
impl HTMLButtonElement {
    ...
    fn compute_type(&self, value: DOMString, can_gc: CanGc) {
        let value = match value.to_ascii_lowercase().as_str() {
            ...
            _ => {
                if pref!(dom_command_invokers_enabled) {
                    ...
                    if el.has_attribute(&local_name!("command")) || el.has_attribute(&local_name!("commandfor")) {
                        ButtonType::Button
                    } else {
                        ButtonType::Submit
                    }
    ...
    fn command_for_element(&self) -> Option<DomRoot<Element>> {
        let command_for_attr = match self
            .upcast::<Element>()
        ...
        let command_for_value = command_for_attr.Value();
         ...
         self
            ...
            .find_map(|e| {
                if let Some(htmle) = e.downcast::<Element>() {
                    if htmle.Id() == command_for_value {
    ...
    fn command_state(&self) -> CommandState {
        let command = self.upcast::<Element>().get_string_attribute(&local_name!("command"));
        if command.starts_with_str("--") {
            return CommandState::Custom
        }
        let value: String = command.to_ascii_lowercase();
        if value == "close" {
...
impl VirtualMethods for HTMLButtonElement {
    ...
    fn attribute_mutated(&self, attr: &Attr, mutation: AttributeMutation, can_gc: CanGc) {
        ...
        match *attr.local_name() {
            ...
            local_name!("type") => {
                self.compute_type(attr.Value(), can_gc)
            },
            local_name!("command") => {
                let el = self.upcast::<Element>();
                self.compute_type(el.get_string_attribute(&local_name!("type")), can_gc)
            },
            local_name!("commandfor") => {
                let el = self.upcast::<Element>();
                self.compute_type(el.get_string_attribute(&local_name!("type")), can_gc)
            }
            local_name!("form") => {
...
impl Activatable for HTMLButtonElement {
    ...
    fn activation_behavior(&self, _event: &Event, target: &EventTarget, can_gc: CanGc) {
        ...
        if let Some(owner) = self.form_owner() {
            ...
            if ty == ButtonType::Button && self.upcast::<Element>().get_string_attribute(&local_name!("type")).to_ascii_lowercase() == "button" {
                return
            }
        }
        ...
        if let Some(target) = self.command_for_element() {
            ...
            let _ = vtable_for(target_node).command_steps(DomRoot::from_ref(self), self.command_state(), can_gc);
        }
