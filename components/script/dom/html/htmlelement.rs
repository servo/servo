...
impl HTMLElementMethods<crate::DomTypeHolder> for HTMLElement {
    ...
    /// <https://html.spec.whatwg.org/multipage/#set-the-inner-text-steps>
    fn SetInnerText(&self, cx: &mut js::context::JSContext, input: DOMString) {
        self.set_inner_text(input, CanGc::from_cx(cx));
    }
    ...
    fn SetOuterText(&self, cx: &mut js::context::JSContext, input: DOMString) -> Fallible<()> {
        ...
        if fragment.upcast::<Node>().children_count() == 0 {
            let text_node = Text::new(DOMString::from("".to_owned()), &document, CanGc::from_cx(cx));
        ...
        parent.ReplaceChild(fragment.upcast(), node, CanGc::from_cx(cx))?;
