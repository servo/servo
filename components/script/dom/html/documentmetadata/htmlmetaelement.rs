...
use crate::dom::element::attributes::storage::{AttrRef, AttrValueRef};
...
impl IsColorSchemeValue for Option<Atom> {
    fn is_color_scheme(self) -> bool {
        self.is_some_and(|name| *name.to_ascii_lowercase() == *"color-scheme")
    }
}

impl IsColorSchemeValue for Option<String> {
    fn is_color_scheme(self) -> bool {
        self.is_some_and(|name| name.to_ascii_lowercase() == "color-scheme")
    }
}

impl IsColorSchemeValue for Option<AttrValueRef<'_>> {
    fn is_color_scheme(self) -> bool {
        self.is_some_and(|name| name.to_ascii_lowercase() == "color-scheme")
    }
}
