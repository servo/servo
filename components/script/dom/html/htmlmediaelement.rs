...
impl HTMLMediaElement {
    ...
    fn playback_need_data(&self) {
        // The player needs more data. If we already have a valid fetch request, we do nothing.
        // Otherwise, if we have no request and the previous request was cancelled because we got an
        // EnoughData event, we restart fetching where we left.
        if let Some(ref mut current_fetch_context) = *self.current_fetch_context.borrow_mut() {
            if let Some(cancel_reason) = current_fetch_context.cancel_reason() {
                ...
                if *cancel_reason == CancelReason::Backoff {
                    self.seek(self.playback_position.get(), false);
                }
                return;
            }
