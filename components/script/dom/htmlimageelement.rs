...
impl Responder {
    ...
    fn run(self) {
        if let Ok(image_response) = self.receiver.recv() {
            self.script_chan
                .send(ScriptMsg::ImageResponseReceived(self.element, image_response))
                .unwrap()
        }
    }
...
pub fn handle_image_response(element: Trusted<HTMLImageElement>, image: ImageResponse) {
