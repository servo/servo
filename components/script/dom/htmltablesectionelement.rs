...
impl<TH: TypeHolderTrait> CollectionFilter<TH> for RowsFilter<TH> {
    fn filter(&self, elem: &Element<TH>, root: &Node<TH>) -> bool {
        elem.is::<HTMLTableRowElement<TH>>() && elem.upcast::<Node<TH>>().GetParentNode().r() == Some(root)
    }
}
...
impl<TH: TypeHolderTrait> HTMLTableSectionElementMethods<TH> for HTMLTableSectionElement<TH> {
    ...
    fn Rows(&self) -> DomRoot<HTMLCollection<TH>> {
        HTMLCollection::create(
            ...
            self.upcast(),
            Box::new(RowsFilter(Default::default())),
        )
    }
    ...
    fn DeleteRow(&self, index: i32) -> ErrorResult {
        let node = self.upcast::<Node<TH>>();
        node.delete_cell_or_row(index, || self.Rows(), |n| n.is::<HTMLTableRowElement<TH>>())
    }
}
...
#[allow(unsafe_code)]
impl<TH: TypeHolderTrait> HTMLTableSectionElementLayoutHelpers
    for LayoutDom<HTMLTableSectionElement<TH>>
{
    fn get_background_color(&self) -> Option<RGBA> {
        unsafe {
