    ...
    fn InsertRow(&self, index: i32) -> Fallible<Root<HTMLElement>> {
        ...
        let strategy = if index == -1 {
            ...
            match self.Rows()
                      ...
                      .map(Strategy::InsertBefore)
                      .chain(iter::once(Strategy::AppendChild))
                      .nth(index as usize) {
...

        {
            ...
            try!(match strategy {
                Strategy::AppendChild => self_node.AppendChild(tr_node),
                Strategy::InsertBefore(node) => self_node.InsertBefore(tr_node, Some(node).r()),
            });
        }
