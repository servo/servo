...
pub enum ChildrenMutation<'a> {
    Append(&'a Node, &'a [&'a Node]),
    Insert(&'a Node, &'a [&'a Node], &'a Node),
    Prepend(&'a [&'a Node], &'a Node),
    Replace(Option<&'a Node>, &'a Node, &'a [&'a Node], Option<&'a Node>),
    ReplaceAll(&'a [&'a Node], &'a [&'a Node]),
}

impl<'a> ChildrenMutation<'a> {
    fn insert(previous_sibling: Option<&'a Node>,
              added_nodes: &'a [&'a Node],
              next_sibling: Option<&'a Node>)
              -> ChildrenMutation<'a> {
        match (previous_sibling, next_sibling) {
            (None, None) => ChildrenMutation::ReplaceAll(&[], added_nodes),
            (Some(prev), None) => ChildrenMutation::Append(prev, added_nodes),
            (None, Some(next)) => ChildrenMutation::Prepend(added_nodes, next),
            (Some(prev), Some(next)) => ChildrenMutation::Insert(prev, added_nodes, next),
        }
    }

    fn replace(previous_sibling: Option<&'a Node>,
               removed_node: &'a &'a Node, added_nodes: &'a [&'a Node],
               next_sibling: Option<&'a Node>)
               -> ChildrenMutation<'a> {
        if let (None, None) = (previous_sibling, next_sibling) {
            ChildrenMutation::ReplaceAll(ref_slice(removed_node), added_nodes)
        } else {
            ChildrenMutation::Replace(previous_sibling,
                                      *removed_node, added_nodes,
                                      next_sibling)
        }
    }

    fn replace_all(removed_nodes: &'a [&'a Node], added_nodes: &'a [&'a Node])
                   -> ChildrenMutation<'a> {
        ChildrenMutation::ReplaceAll(removed_nodes, added_nodes)
    }
}
