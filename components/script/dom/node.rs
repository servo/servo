...
impl<TH: TypeHolderTrait> Node<TH> {
    ...
    pub fn type_id(&self) -> NodeTypeId {
        ...
        match *self.eventtarget.type_id() {
            ...
            EventTargetTypeId::BluetoothRemoteGATTService => panic!("5"),
            EventTargetTypeId::EventSource => panic!("6"),
            EventTargetTypeId::FileReader => panic!("7"),
            EventTargetTypeId::GlobalScope(_) => panic!("8"),
            EventTargetTypeId::MediaQueryList => panic!("9"),
            EventTargetTypeId::PermissionStatus(_) => panic!("10"),
            EventTargetTypeId::ServiceWorker => panic!("11"),
            EventTargetTypeId::ServiceWorkerContainer => panic!("12"),
            EventTargetTypeId::ServiceWorkerRegistration => panic!("13"),
            EventTargetTypeId::VRDisplay => panic!("14"),
            EventTargetTypeId::WebSocket => panic!("15"),
            EventTargetTypeId::Worker => panic!("16"),
            EventTargetTypeId::XMLHttpRequestEventTarget(_) => panic!("17"),
            EventTargetTypeId::AudioNode(_) => panic!("18"),
            EventTargetTypeId::BaseAudioContext(_) => panic!("19"),
            _ => panic!("20"),
        }
    }
...
impl<TH: TypeHolderTrait> FollowingNodeIterator<TH> {
    ...
    fn next_skipping_children_impl(&mut self, current: DomRoot<Node<TH>>) -> Option<DomRoot<Node<TH>>> {
...
impl<TH: TypeHolderTrait> Node<TH> {
    ...
    pub fn pre_insert(node: &Node<TH>, parent: &Node<TH>, child: Option<&Node<TH>>)
        ...
        // Step 5.
        Node::<TH>::insert(node, parent, reference_child, SuppressObserver::Unsuppressed);
    ...
    pub fn clone(node: &Node<TH>, maybe_doc: Option<&Document<TH>>,
                 clone_children: CloneChildrenFlag) -> DomRoot<Node<TH>> {
        // Step 1.
        ...
        if clone_children == CloneChildrenFlag::CloneChildren {
            for child in node.children() {
                let child_copy = Node::<TH>::clone(&child, Some(&document),
                                             clone_children);
...
impl<TH: TypeHolderTrait> NodeMethods<TH> for Node<TH> {
    ...
    fn ReplaceChild(&self, node: &Node<TH>, child: &Node<TH>) -> Fallible<DomRoot<Node<TH>>> {
        ...
        match node.type_id() {
            NodeTypeId::CharacterData(CharacterDataTypeId::Text) if self.is::<Document<TH>>() =>
                return Err(Error::HierarchyRequest),
            NodeTypeId::DocumentType if !self.is::<Document<TH>>() => return Err(Error::HierarchyRequest),
            NodeTypeId::Document(_) => return Err(Error::HierarchyRequest),
            _ => ()
        }
            ...
            match node.type_id() {
                ...
                NodeTypeId::DocumentFragment => {
                    ...
                    match node.child_elements().count() {
                        ...
                        // Step 6.1.2
                        1 => {
                            if self.child_elements().any(|c| c.upcast::<Node<TH>>() != child) {
                                return Err(Error::HierarchyRequest);
                            }
    ...
    fn CloneNode(&self, deep: bool) -> DomRoot<Node<TH>> {
        Node::<TH>::clone(self, None, if deep {
...
pub enum ChildrenMutation<'a, TH: TypeHolderTrait> {
    ...
    ReplaceAll { removed: &'a [&'a Node<TH>], added: &'a [&'a Node<TH>] },
