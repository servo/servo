...
pub trait NodeHelpers {
    ...
    fn following_node(self, root: JSRef<Node>) -> FollowingNodeIterator;
    fn preceding_node(self, root: JSRef<Node>) -> PrecedingNodeIterator;
    fn last_child_node(self) -> LastChildIterator;
...
impl Iterator for PrecedingNodeIterator {
    ...
    fn next(&mut self) -> Option<Temporary<Node>> {
        ...
        if let Some(previous_sibling) = node.r().GetPreviousSibling() {
            ...
            if let Some(last_child) = previous_sibling.root().r().last_child_node().last() {
                self.current = Some(last_child);
                return previous_sibling.root().r().last_child_node().last()
            }
