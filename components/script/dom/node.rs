...
impl Node {
    ...
    pub fn following_siblings<T>(&self) -> SiblingIterator<T> where T: DerivedFrom<Node> {
        let current = match self.GetNextSibling() {
            Some(current) => Root::downcast::<T>(current),
            None => None,
        };
        SiblingIterator::<T> {
            current: current,
    ...
    pub fn preceding_siblings<T>(&self) -> ReverseSiblingIterator<T> where T: DerivedFrom<Node> {
        let current = match self.GetPreviousSibling() {
            Some(current) => Root::downcast::<T>(current),
            None => None,
        };
        ReverseSiblingIterator::<T> {
    ...
    pub fn descending_last_children<T>(&self) -> LastChildIterator<T> where T: DerivedFrom<Node> {
        let current = match self.GetLastChild() {
            Some(current) => Root::downcast::<T>(current),
            None => None,
        };
        LastChildIterator {
    ...
    pub fn query_selector(&self, selectors: DOMString) -> Fallible<Option<Root<Element>>> {
        ...
        match SelectorParser::parse_author_origin_no_namespace(&selectors) {
            ...
            // Step 3.
            Ok(selectors) => {
                Ok(self.traverse_preorder().find(|element| {
    ...
    pub fn children<T>(&self) -> SiblingIterator<T> where T: DerivedFrom<Node> {
        let current = match self.GetFirstChild() {
            Some(current) => Root::downcast::<T>(current),
            None => None,
        };
        SiblingIterator::<T> {
...
pub struct SiblingIterator<T: DerivedFrom<Node>> {
    current: Option<Root<T>>,
    phantom: PhantomData<T>,
}

fn get_next_sibling<T>(start: &Node) -> Option<Root<T>>
where T: DerivedFrom<Node> {
    let mut current = Root::from_ref(start);
    while let Some(next) = current.GetNextSibling() {
        if let Some(derived) = Root::downcast(next.clone()) {
            return Some(derived);
        }
        current = next;
    }
...
fn get_previous_sibling<T>(start: &Node) -> Option<Root<T>>
where T: DerivedFrom<Node> {
    let mut current = Root::from_ref(start);
    while let Some(previous) = current.GetPreviousSibling() {
        if let Some(derived) = Root::downcast(previous.clone()) {
            return Some(derived);
        }
        current = previous;
    }
...
fn get_parent_node<T>(start: &Node) -> Option<Root<T>>
where T: DerivedFrom<Node> {
    let mut current = Root::from_ref(start);
    //Should while loop here should probably be an if block
    //or should we go through all the parents?
    while let Some(parent_node) = current.GetParentNode() {
        if let Some(derived) = Root::downcast(parent_node.clone()) {
            return Some(derived);
        }
        current = parent_node;
    }
...
where T: DerivedFrom<Node> {
    ...
    while let Some(first_child) = current.GetFirstChild() {
        if let Some(derived) = Root::downcast(first_child.clone()) {
            return Some(derived);
        }
        current = first_child;
    }
...
where T: DerivedFrom<Node> {
    let mut current = Root::from_ref(start);
    //The while loop here should probably just be an if block
    while let Some(last_child) = current.GetLastChild() {
        if let Some(derived) = Root::downcast(last_child.clone()) {
            return Some(derived);
        }
        current = last_child;
    }
...
impl<T> Iterator for SiblingIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.current = get_next_sibling(&current.upcast());
        Some(current)
    }
}
...
impl<T> Iterator for ReverseSiblingIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.current = get_previous_sibling(&current.upcast());
        Some(current)
    }
}
...
impl<T> FollowingIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next_skipping_children_impl(&mut self, current: Root<T>) -> Option<Root<T>> {
        if self.root == Root::upcast(current.clone()) {
            self.current = None;
            return None;
        }

        if let Some(next_sibling) = get_next_sibling::<T>(&current.clone().upcast()) {
            self.current = Some(next_sibling);
            return get_next_sibling(&current.upcast())
        }

        for ancestor in current.upcast().inclusive_ancestors() {
            if self.root == ancestor {
                break;
            }
            if let Some(next_sibling) = get_next_sibling::<T>(&ancestor.upcast()) {
                self.current = Some(next_sibling);
                return get_next_sibling(&ancestor.upcast())
            }
        }
        self.current = None;
        None
...
impl<T> Iterator for FollowingIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next(&mut self) -> Option<Root<T>> {
        ...
        if let Some(first_child) = get_first_child::<T>(&current.upcast()) {
            self.current = Some(first_child);
            return get_first_child(current.upcast())
        }
...
impl<T> Iterator for PrecedingIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next(&mut self) -> Option<Root<T>> {
        ...
        self.current = if self.root == Root::upcast(current.clone()) {
            None
        } else if let Some(previous_sibling) = get_previous_sibling::<T>(&current.upcast()) {
            if self.root == Root::upcast(previous_sibling.clone()) {
                None
            } else if let Some(last_child) = previous_sibling.upcast().descending_last_children::<T>().last() {
                Some(last_child)
            } else {
                Some(previous_sibling)
            }
        } else {
            get_parent_node(&current.upcast())
        };
        self.current.clone()
    }
}
...
impl<T> Iterator for LastChildIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next(&mut self) -> Option<Root<T>> {
        ...
        self.current = get_last_child(&current.upcast());
        Some(current)
    }
}
...
impl<T> Iterator for AncestorIterator<T> where T: DerivedFrom<Node> {
    ...
    fn next(&mut self) -> Option<Root<T>> {
        ...
        self.current = get_parent_node(&current.upcast());
        Some(current)
    }
}
...
impl<T> TreeIterator<T> where T: DerivedFrom<Node> {
    fn new(root: &Node) -> TreeIterator<T> {
        TreeIterator::<T> {
            current: Root::downcast::<T>(Root::from_ref(root)),
            depth: 0,
            phantom: PhantomData,
        }
    }
    ...
    fn next_skipping_children_impl(&mut self, current: Root<T>) -> Option<Root<T>> {
        for ancestor in current.upcast().inclusive_ancestors() {
            ...
            if let Some(next_sibling) = get_previous_sibling::<T>(&ancestor.upcast()) {
                self.current = Some(next_sibling);
                return Some(current);
            }
            self.depth -= 1;
