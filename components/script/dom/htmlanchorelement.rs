...
impl HTMLAnchorElement {
    fn new_inherited(localName: DOMString, prefix: Option<DOMString>, document: &Document) -> HTMLAnchorElement {
        HTMLAnchorElement {
    ...
    pub fn new(localName: DOMString, prefix: Option<DOMString>, document: &Document) -> Root<HTMLAnchorElement> {
    ...
    fn activation_behavior(&self, event: &Event, target: &EventTarget) {
        ...
        if let Some(element) = ElementCast::to_ref(target) {
            if target.is_htmlimageelement() && element.has_attribute(&atom!("ismap")) {
                ...
                let rect = window_from_node(target_node).r().content_box_query(target_node.to_trusted_node_address());
                ismap_suffix = Some(
