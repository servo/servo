        ...
        if !event.DefaultPrevented() {
            ...
            match clipboard_event.clipboard_event_type() {
                ClipboardEventType::Copy => {
                    ...
                    if let Some(selection) = editing_context.selection(cx) {
...
impl EditingContext {
    ...
    pub(crate) fn selection(&self, cx: &mut JSContext) -> Option<String> {
        match self {
            EditingContext::TextControl(element) => {
                element.text_control_element().text_input().selection()
            },
