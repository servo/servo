...
In the remainder of this document, it is assumed that there are two relevant
low-level objects when considering a JavaScript object implementing a DOM
interface. The first is an object in the JavaScript virtual machine, which we
will call a "reflector". (In SpiderMonkey, this would be an instance of the C++
`JSObject` class.) The second is the object that implements a particular DOM
interface (for example, `XMLHttpRequest`) and stores any internal state. We
will call that object a "DOM object".
...
underlying low-level DOM objects. When JavaScript works with such a DOM object
(retrieving it through [`getElementById`][gEBI] for example), the browser
builds a reflector that holds a reference to the DOM object. If the JavaScript
garbage collector determines that a reflector is no longer reachable, it
destroys the reflector and decrements the reference count on the underlying
object.
...
soon as they are no longer needed. Unfortunately, the cross-language reference
counting scheme introduces a major complication.

Consider a C++ `Element` DOM object which holds a reference-counted pointer to
an `Event` DOM object:
...
When the event fires, the handler adds a property on the `Event` reflector
which points back to the `Element` reflector. We now have a cross-language
reference cycle, with an `Element` DOM object pointing to an `Event` DOM object
within C++, and an `Event` reflector pointing to the `Element` reflector in
JavaScript. The C++ reference counting will never destroy a cycle, and the
JavaScript garbage collector can't trace through the C++ pointers, so these
...
A `Root<T>` can be constructed from a reference (with the
[`Root::from_ref` associated function][from-ref], in case we need to use the
DOM object longer than the reference allows us to) or from a `JS<T>` field
(by calling its [`root`][js-root] method). It is also returned from
[`T::new` functions][new] when creating a DOM object.
...
`JS<T>` from appearing at all. Because lint plugins are part of the usual
...
[experiments with garbage collecting DOM objects][blink-gc], but they don't
