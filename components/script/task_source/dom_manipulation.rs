...
pub enum DOMManipulationTask {
    ...
    FireToggleEvent(Trusted<HTMLDetailsElement>, u32),
...
impl DOMManipulationTask {
    pub fn handle_msg(self, script_thread: &ScriptThread) {
        ...
        match self {
            ...
            FireToggleEvent(details, toggle_number) => {
                let details = details.root();
                let details = details.r();

                if details.check_toggle_count(toggle_number) {
                    details.upcast::<EventTarget>().fire_simple_event("toggle");
                }
            }
