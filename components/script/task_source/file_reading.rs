...
impl<TH: TypeHolderTrait> FileReadingTask<TH> {
    pub fn handle_task(self) {
        ...
        match self {
            ProcessRead(reader, gen_id) => FileReader::<TH>::process_read(reader, gen_id),
            ProcessReadData(reader, gen_id) => FileReader::<TH>::process_read_data(reader, gen_id),
