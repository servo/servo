...
impl<'dom> style::dom::TElement for ServoLayoutElement<'dom> {
    ...
    fn compute_layout_damage(old: &ComputedValues, new: &ComputedValues) -> RestyleDamage {
        ...
        let text_shaping_needs_recollect = || {
            if old.clone_white_space_collapse() != new.clone_white_space_collapse() ||
                old.clone_text_transform() != new.clone_text_transform() ||
            ...
            let new_text = old.get_inherited_text().clone();
            if old_text.word_break != new_text.word_break ||
