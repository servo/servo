...
use net_traits::ControlMsg::{GetCookiesByNameForUrl, SetCookiesForUrlWithData};
...
pub fn handle_get_cookie(page: &Rc<Page>,
                         _pipeline: PipelineId,
                         name: String,
    ...
    let _ = document.DefaultView().resource_thread().send(
        GetCookiesByNameForUrl(url.clone(), name, sender, NonHTTP)
        );
    let cookies = receiver.recv().unwrap();
    reply.send(cookies).unwrap();
}

pub fn handle_add_cookie(page: &Rc<Page>,
                         _pipeline: PipelineId,
                         cookie: CookieData,
    ...
    reply.send(match (document.is_cookie_averse(), cookie.domain.clone()) {
        ...
        (false, Some(ref domain)) if url.domain().map(|x| { x == domain }).unwrap_or(false) => {
            let _ = document.DefaultView().resource_thread().send(SetCookiesForUrlWithData(url.clone(), cookie, method));
            Ok(())
        },
        (false, None) => {
            let _ = document.DefaultView().resource_thread().send(SetCookiesForUrlWithData(url.clone(), cookie, method));
