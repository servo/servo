...
use cookie::Cookie;
use embedder_traits::{
    JSValue, JavaScriptEvaluationError, JavaScriptSerializationError, WebDriverFrameId,
...
pub(crate) fn jsval_to_webdriver(
    ...
    if result.is_err() {
        report_pending_exception(cx, ReportExceptionFlags::DispatchEvent, realm, can_gc);
    }
    result
}
    ...
    if val.get().is_undefined() {
        ...
        if let Ok(element) = root_from_object::<Element>(*object, cx) {
            ...
            if JS_CallFunctionName(
                ...
                Err(JavaScriptEvaluationError::SerializationError(
                    JavaScriptSerializationError::Generic,
                ))
            }
        } else {
            clone_an_object(
