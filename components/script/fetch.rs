...
use net_traits::{FilteredMetadata, FetchMetadata};
...
impl FetchResponseListener for FetchContext {
    ...
    #[allow(unrooted_must_root)]
    fn process_response(&mut self, fetchmetadata: Result<FetchMetadata, NetworkError>) {
        ...
        match fetchmetadata {
            ...
            // Step 4.2
            Ok(fetchmetadata_) => {
                match fetchmetadata_ {
                    FetchMetadata::Unfiltered(metadata) => {
                        self.response_object.root().set_headers(metadata.headers);
                        self.response_object.root().set_raw_status(metadata.status);
                        self.response_object.root().set_final_url(metadata.final_url);
                    },
                    FetchMetadata::Filtered {
                        filtered: FilteredMetadata::Transparent(metadata),
                        unsafe_: _,
                    } => {
                        self.response_object.root().set_headers(metadata.headers);
                        self.response_object.root().set_raw_status(metadata.status);
