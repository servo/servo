...
pub fn parse_html(document: &Document,
    ...
    let parser = match fragment_context {
        None => ServoHTMLParser::new(Some(url.clone()), document),
        Some(fc) => ServoHTMLParser::new_for_fragment(Some(url.clone()), document, fc),
    };
    let parser = parser.r();
    ...
    fn parse_progress(document: &Document, parser: &ServoHTMLParser, url: &Url, load_response: &LoadResponse) {
