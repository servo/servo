...
pub fn heap_size_of_self_and_children<T: Reflectable + HeapSizeOf>(obj: &T) -> usize {
    unsafe {
        ...
        (class.heap_size_of)(obj as *const T as *const libc::c_void)
    }
}

/// Used by codegen to include the pointer to the HeapSizeOf implementation of each IDL
/// interface. This way we don't have to find the most-derived interface of DOM objects
/// by hand in code.
#[allow(unsafe_code)]
pub unsafe fn heap_size_of_raw_self_and_children<T>(obj: *const libc::c_void) -> usize
                                                    where T: HeapSizeOf {
