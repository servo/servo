...
impl RcCompositorLayer for Rc<Layer<CompositorData>> {
    fn collect_old_layers<Window>(&self,
        ...
        fn collect_old_layers_for_pipeline<Window>(layer: &Layer<CompositorData>,
                                                   compositor: &mut IOCompositor<Window>,
                                                   pipeline_id: PipelineId,
                                                   new_layers: &Vec<LayerProperties>)
                                                   where Window: WindowMethods {
            ...
            for kid in &*layer.children() {
                collect_old_layers_for_pipeline(kid, compositor, pipeline_id, new_layers);
            }
        ...
        collect_old_layers_for_pipeline(&root_layer, compositor, pipeline_id, new_layers);
    }
}
