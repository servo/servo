...
impl<Window: WindowMethods> IOCompositor<Window> {
    ...
    fn send_updated_display_ports_to_layout(&mut self) {
        ...
        let dppx = (self.page_zoom * self.device_pixels_per_screen_px()).get();
        let window_size = self.window_size.as_f32().to_untyped();
        let window_size = Size2D(window_size.width / dppx, window_size.height / dppx);
        let window_size = TypedSize2D::from_untyped(&window_size);
