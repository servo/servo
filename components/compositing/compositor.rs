...
impl<Window: WindowMethods> IOCompositor<Window> {
    ...
    fn create_descendant_layer(&mut self,
        ...
        if let Some(parent_layer) = self.find_layer_with_pipeline_and_layer_id(pipeline_id,
            ...
            if let Some(ref subpage_layer_info) = layer_properties.subpage_layer_info {
                ...
                // information, so go ask it.
                //
                // FIXME(pcwalton): I haven't observed any deadlocks here, but I have not convinced
                // myself that they cannot happen.
                let (sender, receiver) = ipc::channel().unwrap();
                self.constellation_chan
                    .0
                    .send(ConstellationMsg::GetPipelineIdForSubpage(pipeline_id,
                                                                    subpage_layer_info.subpage_id,
                                                                    sender))
                    .unwrap();

                // It's possible that the pipeline has shut down by the time we receive the
                // response to our query. In that case we will return `None`, and it should be OK
                // to render nothing.
                if let Some(subpage_pipeline_id) = receiver.recv().unwrap() {
    ...
    fn does_layer_have_outstanding_paint_messages(&self, layer: &Rc<Layer<CompositorData>>)
        ...
        let current_epoch = self.pipeline_details
                                .get(&layer_data.pipeline_id)
                                .unwrap()
                                .current_epoch;
