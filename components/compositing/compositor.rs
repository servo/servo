...
use embedder_traits::{
    ...
    ShutdownState, TouchEvent, TouchEventType, TouchId, ScrollEvent as EmbedderScrollEvent,
};
...
impl IOCompositor {
    ...
    fn scroll_node_at_device_point(
        ...
        for hit_test_result in hit_test_results.iter() {
            if previous_pipeline_id.replace(hit_test_result.pipeline_id) != Some(hit_test_result.pipeline_id) {
