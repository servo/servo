...
enum ScrollZoomEvent {
    ...
    PinchZoom(f32),
    /// A zoom event that magnifies the view by the Initial Scale factor parsed from meta tag.
    ViewportInitialZoom(f32),
...
impl WebViewRenderer {
    ...
    pub(crate) fn process_pending_scroll_and_pinch_zoom_events(
        ...
        for scroll_event in self.pending_scroll_zoom_events.drain(..) {
            match scroll_event {
                ...
                ScrollZoomEvent::ViewportInitialZoom(magnification) => {
                    combined_magnification = magnification
                },
    ...
    pub fn set_viewport_description(&mut self, viewport_description: ViewportDescription) {
        self.pending_scroll_zoom_events
            .push(ScrollZoomEvent::ViewportInitialZoom(
