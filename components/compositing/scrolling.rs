...
impl ScrollingTimer {
    pub fn run(&mut self) {
        while let Ok(ToScrollingTimerMsg::ScrollEventProcessedMsg(timestamp)) = self.receiver.recv() {
            ...
            let delta_secs = delta_ns / NANOS_PER_SEC;
            let delta_nanos = delta_ns % NANOS_PER_SEC;
            assert!(delta_nanos <= u32::MAX as u64);
            thread::sleep(Duration::new(delta_secs, delta_nanos as u32));
