...
impl<LTF: LayoutThreadFactory, STF: ScriptThreadFactory> Constellation<LTF, STF> {
    ...
    fn handle_init_load(&mut self, url: Url) {
        ...
        self.new_pipeline(root_pipeline_id, None, Some(window_size), None, None,
    ...
    fn handle_script_loaded_url_in_iframe_msg(&mut self, load_info: IFrameLoadInfo) {
        let (old_pipeline_id, sandbox, new_url, source_script_chan, replacement_script_chan) =
            match load_info.load_type {
                IFrameLoadType::Async(ref async_load) => {
                    let old_pipeline_id = async_load.old_subpage_id.and_then(|old_subpage_id| {
                        self.subpage_map.get(&(load_info.containing_pipeline_id, old_subpage_id))
                    }).cloned();

                    // If no url is specified, reload.
                    let new_url = match (old_pipeline_id, &async_load.url) {
                        (_, &Some(ref url)) => url.clone(),
                        (Some(old_pipeline_id), &None) => self.pipelines.get(&old_pipeline_id).unwrap().url.clone(),
                        (None, &None) => Url::parse("about:blank").expect("infallible"),
                    };
                ...
                IFrameLoadType::Sync((sync_script_chan, script_chan)) => {
                    (None,
                     ...
                     Url::parse("about:blank").expect("infallible"),
        ...
        let (script_chan, replacement_script_chan, window_size) = {
            ...
            (script_chan, replacement_script_chan, window_size)
        };
