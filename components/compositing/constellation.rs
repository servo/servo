...
impl<LTF: LayoutThreadFactory, STF: ScriptThreadFactory> Constellation<LTF, STF> {
    ...
    fn handle_subframe_loaded(&mut self, pipeline_id: PipelineId) {
        let subframe_pipeline = match self.pipelines.get(&pipeline_id) {
        ...
        let parent_pipeline = match self.pipelines.get(&subframe_parent.0) {
            Some(pipeline) => pipeline,
            None => return debug!("Pipeline {:?} subframe loaded after closure.", subframe_parent.0),
        };
    ...
    fn handle_tick_animation(&mut self, pipeline_id: PipelineId, tick_type: AnimationTickType) {
        // If the pipeline lookup fails, then we have torn down the pipeline,
        // so it is reasonable to silently ignore the tick event.
        match self.pipelines.get(&pipeline_id) {
            Some(pipeline) => match tick_type {
            ...
            None => debug!("Pipeline {:?} got animation tick after closure.", pipeline_id),
        }
    }
    ...
    fn load_url(&mut self, source_id: PipelineId, load_data: LoadData) -> Option<PipelineId> {
        ...
        match parent_info {
            Some((parent_pipeline_id, subpage_id)) => {
                ...
                // If the parent pipeline lookup fails, then we have torn down the pipeline,
                // so it is reasonable to silently ignore the load.
                match self.pipelines.get(&parent_pipeline_id) {
                    ...
                    None => {
                        debug!("Pipeline {:?} got load after parent's closure.", source_id);
                        None
                    },
                }
            }
    ...
    fn handle_navigate_msg(&mut self,
        ...
        if let Some((parent_pipeline_id, subpage_id)) = pipeline_info {
            let script_chan = match self.pipelines.get(&parent_pipeline_id) {
                ...
                Some(pipeline) => &pipeline.script_chan,
            };
