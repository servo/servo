...
impl<LTF: LayoutTaskFactory, STF: ScriptTaskFactory> Constellation<LTF, STF> {
    ...
    fn handle_script_loaded_url_in_iframe_msg(&mut self,
        ...
        let script_chan = {
            ...
            if same_script {
                debug!("Constellation: loading same-origin iframe, parent url {:?}, iframe url {:?}", source_url, url);
                Some(source_pipeline.script_chan.clone())
            } else {
                debug!("Constellation: loading cross-origin iframe, parent url {:?}, iframe url {:?}", source_url, url);
                None
            }
        };
