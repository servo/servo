        ...
        let request = {
            ...
            select! {
                msg = receiver_from_script.recv() =>
                    Request::Script(msg.expect("Unexpected script panic in constellation")),
                msg = receiver_from_compositor.recv() =>
                    Request::Compositor(msg.expect("Unexpected compositor panic in constellation")),
                msg = receiver_from_layout.recv() =>
                    Request::Layout(msg.expect("Unexpected layout panic in constellation")),
                msg = receiver_from_panic.recv() =>
                    Request::Panic(msg.expect("Unexpected panic panic in constellation"))
            }
        };
