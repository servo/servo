...
impl NetworkEventActor {
    ...
    pub fn event_actor(&self) -> EventActor {
        ...
        EventActor {
            ...
            startedDateTime: self.request.startedDateTime.strftime("%m/%d/%Y %H:%M:%S").unwrap().to_string(),
            isXHR: false,
            private: false,
        }
    }
    ...
    pub fn response_start(&self) -> ResponseStartMsg {
        ...
        let mut hSize = 0;
        if let Some(hSize2) = hSizeOption {
            hSize = hSize2;
        }
    ...
    pub fn request_cookies(&self) -> RequestCookiesMsg {
        ...
        let cookies_size = match list.get() {
            Some(&Cookie(ref cookie)) =>  cookie.len(),
            None => 0
        };
