...
impl Rope {
    ...
    fn delete_slice(&mut self, slice: Range<RopeIndex>) {
        assert!(slice.start <= slice.end);

        if slice.start.line == slice.end.line {
            self.line_for_index_mut(slice.start)
                .replace_range(slice.start.code_point..slice.end.code_point, "");
            return;
        }

        self.line_for_index_mut(slice.start)
            .truncate(slice.start.code_point);
        self.line_for_index_mut(slice.end)
            .replace_range(0..slice.end.code_point, "");

        if slice.end.line > slice.start.line + 1 {
            self.lines.splice(slice.start.line + 1..slice.end.line, []);
        }
    }
    ...
    pub fn clamp_index(&self, rope_index: RopeIndex) -> RopeIndex {
        ...
        // that is still valid.
        let line_index = rope_index.line.min(self.lines.len().saturating_sub(1));
        let line_length_utf8 = self.lines[line_index].len();
    ...
    pub fn utf8_offset_to_rope_index(&self, utf8_offset: Utf8CodeUnitLength) -> RopeIndex {
        ...
        for (line_index, line) in self.lines.iter().enumerate() {
            if current_utf8_offset.0 == 0 || current_utf8_offset.0 < line.len() {
                return RopeIndex::new(line_index, current_utf8_offset.0);
            }
            current_utf8_offset -= Utf8CodeUnitLength(line.len());
        }
        self.last_index()
    }
...
/// index into a [`Rope`]. As [`Rope`] always uses Rust strings interally, code point
...
/// different. When moving forward, the end of the unit of movement is returning and when
/// moving backward the start of the unit of movement is returned. This matches the
/// expeceted behavior when interactively moving through editable text.
