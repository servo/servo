...
/// A thread to handle fetches in a Servo process. This thread is resonsible for listener
/// for new fetch requets as well as listening for updeates on those operations and forwarding
...
struct FetchThread {
    ...
    /// An [`IpcSender`] that's send with every fetch request and leading back to our
    /// router proxy.
