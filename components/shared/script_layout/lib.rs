...
impl ImageAnimationState {
    pub fn new(image: Arc<Image>, now: f64) -> Self {
        Self {
            image,
            active_frame: 0,
    ...
    pub fn time_to_next_frame(&self, now: f64) -> f64 {
        (now - self.last_update_time).max(0.0)
    }

    pub fn update(&mut self, now: f64) -> bool {
        assert!(self.image.frames.len() > 1);
        ...
        match self.active_frame != next_active_frame_id {
            true => {
