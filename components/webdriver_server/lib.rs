...
fn nullable_date_to_option(val: &Nullable<Date>) -> Option<u64> {
    match *val {
...
impl Handler {
    ...
    fn handle_add_cookie(&self, params: &AddCookieParameters) -> WebDriverResult<WebDriverResponse> {
        ...
        let cookie = CookieData {
            ...
            expiry: nullable_date_to_option(&params.expiry),
            max_age: nullable_date_to_option(&params.maxAge),
...
impl WebDriverHandler<ServoExtensionRoute> for Handler {
    fn handle_command(&mut self,
        ...
        match msg.command {
            ...
            WebDriverCommand::AddCookie(ref parameters) => {
                println!("params: \nexpiry: {:?}\tmax-age: {:?}",
                         parameters.expiry, parameters.maxAge);
                self.handle_add_cookie(parameters)
            },
