...
fn nullable_to_option<T: rustc_serialize::json::ToJson>(val: Nullable<T>) -> Option<T> {
    match val {
...
impl Handler {
    ...
    fn handle_get_cookie(&self, name: &str) -> WebDriverResult<WebDriverResponse> {
        ...
        let response = cookies.into_iter().map(|cookie| {
            Cookie::new(cookie.name, cookie.value, cookie.path.map(Nullable::Value).unwrap_or(Nullable::Null),
                        Nullable::Null, Nullable::Null, Nullable::Null, false, false)
        }).collect::<Vec<Cookie>>();
    ...
    fn handle_add_cookie(&self, params: &AddCookieParameters) -> WebDriverResult<WebDriverResponse> {
        ...
        let cookie = CookieData {
            ...
            path: nullable_to_option(params.path.to_owned()),
            domain: nullable_to_option(params.domain.to_owned()),
            // TODO(dlr): Once webdriver-rust supports accessing
            // AddCookieParameters Date value, this should be
            // changed to accept a possible given expiry
            expiry: None,
