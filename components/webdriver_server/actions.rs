...
impl Handler {
    ...
    pub(crate) fn dispatch_actions(
        ...
        session.actions_queue.borrow_mut().push_back(new_token);

        if new_token != *session.actions_queue.borrow().front().unwrap() {
        ...
        session.actions_queue.borrow_mut().pop_front();
    ...
    fn dispatch_actions_inner(
        ...
        for tick_actions in actions_by_tick.iter() {
            ...
            match self
                ...
                .try_recv_timeout(Duration::from_millis(3000))
            {
                Ok(response) => {
    ...
    fn perform_pointer_move(
        ...
        loop {
            ...
            if x != current_x || y != current_y {
                // Step 7.2
                let msg_id = *session.actions_queue.borrow().front().unwrap();
