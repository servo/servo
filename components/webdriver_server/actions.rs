...
impl Handler {
            ...
            match self.webdriver_response_receiver.recv() {
                Ok(response) => {
                    let response_id = match response {
                        WebDriverCommandResponse::DispatchSucceed(response_id) => response_id,
                        WebDriverCommandResponse::DispatchFailed(event) => {
                            input_to_retry.push(event);
                            continue;
                        },
                    };
    ...
    fn redispatch_failed_input_event(&self) -> Result<(), ErrorStatus> {
...

        {
            ...
            self.num_pending_actions.set(input_to_retry.len() as u32);
            input_to_retry.clear();
        }

        self.wait_for_user_agent_handling_complete()?;

        let mut input_to_retry = self.input_to_retry.borrow_mut();
        if !input_to_retry.is_empty() {
            warn!(
                "There are {} events failed to be dispatched (after retry)",
                input_to_retry.len()
            );
        }
        input_to_retry.clear();
