        ...
        for reporter in self.reporters.values() {
            ...
            if let Ok(reports) = port.recv() {
                ...
                for report in reports.iter() {
                    let mut path = report.path.clone();
                    ...
                    match report.kind {
                        ...
                        ReportKind::ExplicitUnknownLocationSize =>
                            path.insert(0, String::from("explicit")),
                    ...
                    if report.path.len() == 1 {
                        if report.path[0].as_str() == JEMALLOC_HEAP_ALLOCATED_STR {
                            assert!(jemalloc_heap_allocated_size == None);
                            jemalloc_heap_allocated_size = Some(report.size);
                        } else if report.path[0].as_str() == SYSTEM_HEAP_ALLOCATED_STR {
                            assert!(system_heap_allocated_size == None);
                            system_heap_allocated_size = Some(report.size);
                        }
                    }

                    // Insert the report.
                    forest.insert(&path, report.size);
                }
            }
        }
