...
use js::jsapi::{
    ...
    JSAtom, JSContext, JSObject, JS_GetElement, NewArrayObject1, ObjectOpResult,
    ...
    fn define_property(
        ...
        if id.get() == VoidId() {
            ...
            if desc.hasValue_() {
                ...
                return self.set_length_value(cx, proxy, backing_list_obj.handle_mut(), desc, result);
            }
            result.succeed();
            return Ok(());
        }
    ...
    fn get(
        ...
        unsafe {
            ...
            if let Some(index) = index {
                ...
                if !JS_GetElement(cx, backing_list_obj.handle().into_handle(), index, vp.into_handle_mut()) {
                    return Err(());
                }
                return Ok(());
            }
    ...
    fn set_length(
        ...
        unsafe {
            ...
            for _ in length..old_len {
                ...
                if self
                    .handler
                    .on_delete_item(cx, proxy, value.handle(), index_to_delete).is_err()
                {
                    ok = false;
                    break;
                }
