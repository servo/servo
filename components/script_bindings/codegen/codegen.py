...
from webidl_type_wrapper import IDLTypeTags
...
def genericsForType(t: IDLType) -> tuple[str, str]:
    if containsDomInterface(t):
        return ("<D: DomTypes>", "<D>")
    return ("", "")


def isDomInterface(t: IDLType, logging: bool = False) -> bool:
...
def containsDomInterface(t: IDLType, logging: bool = False) -> bool:
...
builtinNames = {
    IDLTypeTags.bool: 'bool',
    IDLTypeTags.int8: 'i8',
    IDLTypeTags.int16: 'i16',
    IDLTypeTags.int32: 'i32',
    IDLTypeTags.int64: 'i64',
    IDLTypeTags.uint8: 'u8',
    IDLTypeTags.uint16: 'u16',
    IDLTypeTags.uint32: 'u32',
    IDLTypeTags.uint64: 'u64',
    IDLTypeTags.unrestricted_float: 'f32',
    IDLTypeTags.float: 'Finite<f32>',
    IDLTypeTags.unrestricted_double: 'f64',
    IDLTypeTags.double: 'Finite<f64>',
    IDLTypeTags.int8array: 'Int8Array',
    IDLTypeTags.uint8array: 'Uint8Array',
    IDLTypeTags.int16array: 'Int16Array',
    IDLTypeTags.uint16array: 'Uint16Array',
    IDLTypeTags.int32array: 'Int32Array',
    IDLTypeTags.uint32array: 'Uint32Array',
    IDLTypeTags.float32array: 'Float32Array',
    IDLTypeTags.float64array: 'Float64Array',
    IDLTypeTags.arrayBuffer: 'ArrayBuffer',
    IDLTypeTags.arrayBufferView: 'ArrayBufferView',
    IDLTypeTags.uint8clampedarray: 'Uint8ClampedArray',
}

numericTags = [
    IDLTypeTags.int8, IDLTypeTags.uint8,
    IDLTypeTags.int16, IDLTypeTags.uint16,
    IDLTypeTags.int32, IDLTypeTags.uint32,
    IDLTypeTags.int64, IDLTypeTags.uint64,
    IDLTypeTags.unrestricted_float,
    IDLTypeTags.unrestricted_double
]
...
"""
    ...
    if type.isDOMString():
        ...
        if defaultValue is None:
            ...
            assert defaultValue.type.tag() == IDLTypeTags.domstring
    ...
    if type.isUSVString():
        ...
        if defaultValue is None:
            ...
            assert defaultValue.type.tag() in (IDLTypeTags.domstring, IDLTypeTags.usvstring)
    ...
    if type.isByteString():
        ...
        if defaultValue is None:
            ...
            assert defaultValue.type.tag() in (IDLTypeTags.domstring, IDLTypeTags.bytestring)
...
def convertConstIDLValueToJSVal(value):
    ...
    tag = value.type.tag()
    if tag in [IDLTypeTags.int8, IDLTypeTags.uint8, IDLTypeTags.int16,
               IDLTypeTags.uint16, IDLTypeTags.int32]:
        return f"ConstantVal::Int({value.value})"
    if tag == IDLTypeTags.uint32:
        return f"ConstantVal::Uint({value.value})"
    if tag in [IDLTypeTags.int64, IDLTypeTags.uint64]:
        return f"ConstantVal::Double({value.value} as f64)"
    if tag == IDLTypeTags.bool:
        return "ConstantVal::Bool(true)" if value.value else "ConstantVal::BoolVal(false)"
    if tag in [IDLTypeTags.unrestricted_float, IDLTypeTags.float,
               IDLTypeTags.unrestricted_double, IDLTypeTags.double]:
        return f"ConstantVal::Double({value.value} as f64)"
...
pub(crate) fn init_{infoName}<D: DomTypes>() {{
    ...
    def getJSReturnTypeTag(t):
        ...
        if tag in [IDLTypeTags.int8, IDLTypeTags.uint8,
                   IDLTypeTags.int16, IDLTypeTags.uint16,
                   IDLTypeTags.int32]:
            return "JSVAL_TYPE_INT32"
        if tag in [IDLTypeTags.int64, IDLTypeTags.uint64,
                   IDLTypeTags.unrestricted_float, IDLTypeTags.float,
                   IDLTypeTags.unrestricted_double, IDLTypeTags.double]:
...
class CGConstant(CGThing):
    ...
    def define(self):
        ...
        if tag == IDLTypeTags.float:
            const_type = "f32"
        elif tag == IDLTypeTags.double:
            const_type = "f64"
