...
class CGIDLInterfaceBindingRoot(CGThing):
    ...
    root: CGThing | None
    def __init__(self, config: Configuration, prefix: str, webIDLFile: str, only_interfaces:set[str]) -> None:
        ...
        cgthings = []
        #print("\nINTERFACES\n\n|\n|\n|\n|\n|\n|\n----------------------------------------------------------------------------------------------------------------------------------------------------------------------")
        #for i in only_interfaces:
        #    print(i + "\n")
        #print("\nEND|\n|\n|\n|\n|\n|\n|\n|\n------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------")

        for d in descriptors:
            #print("D: " + str(d.interface.identifier.name))

            current_set = set()
            current_set.add(d.interface.identifier.name)
            #print("S: " + str(current_set))
            #print("Disjoingness " + str(current_set.isdisjoint(only_interfaces)))
            if (
                not current_set.isdisjoint(only_interfaces) and
        ...
        curr = CGImports(curr, descriptors=[], callbacks=[],
                         ...
                         config=config)
        self.root = curr
        #self.root = CGWrapper(curr, pre=f"{AUTOGENERATED_WARNING_COMMENT}{ALLOWED_WARNINGS}")
...
class CGConcreteBindingRoot(CGThing):
    ...
    def __init__(self, config: Configuration, prefix: str, webIDLFile: str, only_interfaces: set[str]) -> None:
        ...
        for d in descriptors:
            cgthings = self._make_descriptor(d, descriptors, originalBinding, cgthings, only_interfaces)
    ...
    def _make_descriptor(self, d: Descriptor, descriptors: list[Descriptor], originalBinding: str, cgthings: list[Any], only_interfaces: set[str]) -> list[Any]:
        ...
        if d.concrete and not set([ifaceName]).isdisjoint(only_interfaces):
        ...
        if (
            ...
            not d.allowDropImpl
        ) and (set([ifaceName]).isdisjoint(only_interfaces)):
...
class GlobalGenRoots():
    ...
    @staticmethod
    def BindingsIDL(config: Configuration) -> CGThing:
    ...
    @staticmethod
    def ConcreteInheritTypes(config: Configuration, skip:list[str] = [], inverse: bool = False) -> CGThing:
        ...
        for descriptor in descriptors:
            ...
            if (not inverse and name in skip) or (inverse and name not in skip):
                continue
            chain = descriptor.prototypeChain
            upcast = descriptor.hasDescendants()
