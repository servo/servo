...
use script::dom::bindings::codegen::Bindings::BlobBinding::BlobBinding::BlobMethods;
use script::dom::bindings::codegen::Bindings::WindowBinding::WindowMethods;
use script::dom::bindings::codegen::Bindings::XMLHttpRequestBinding;
use script::dom::bindings::codegen::Bindings::XMLHttpRequestBinding::BodyInit;
use script::dom::bindings::codegen::Bindings::XMLHttpRequestBinding::XMLHttpRequestMethods;
use script::dom::bindings::codegen::Bindings::XMLHttpRequestBinding::XMLHttpRequestResponseType;
use script::dom::bindings::codegen::UnionTypes::DocumentOrBodyInit;
use script::dom::bindings::conversions::ToJSValConvertible;
use script::dom::bindings::error::{Error, ErrorResult, Fallible};
use script::dom::bindings::inheritance::Castable;
use script::dom::bindings::refcounted::Trusted;
use script::dom::bindings::reflector::{DomObject, reflect_dom_object};
use script::dom::bindings::root::{Dom, DomRoot, MutNullableDom};
use script::dom::bindings::str::{ByteString, DOMString, USVString, is_token};
use script::dom::blob::{Blob, BlobImpl};
use script::dom::document::{Document, HasBrowsingContext, IsHTMLDocument};
use script::dom::document::DocumentSource;
use script::dom::event::{Event, EventBubbles, EventCancelable};
use script::dom::eventtarget::EventTarget;
use script::dom::formdata::FormData;
use script::dom::globalscope::GlobalScope;
use script::dom::headers::is_forbidden_header_name;
use script::dom::htmlformelement::{encode_multipart_form_data, generate_boundary};
use script::dom::node::Node;
use script::dom::progressevent::ProgressEvent;
use script::dom::servoparser::ServoParser;
use script::dom::urlsearchparams::URLSearchParams;
use script::dom::window::Window;
use script::dom::workerglobalscope::WorkerGlobalScope;
use script::dom::xmlhttprequesteventtarget::XMLHttpRequestEventTarget;
use script::dom::xmlhttprequestupload::XMLHttpRequestUpload;
use encoding_rs::{Encoding, UTF_8};
use euclid::Length;
use script::fetch::FetchCanceller;
...
use script::network_listener::{NetworkListener, PreInvoke};
...
use script::task_source::networking::NetworkingTaskSource;
use script::task_source::TaskSourceName;
use time;
use script::timers::{OneshotTimerCallback, OneshotTimerHandle};
use url::Position;
use TypeHolder;
use script;
use script::dom::xmlhttprequest::XMLHttpRequestTrait;
use script::dom::bindings::utils::DOMClass;
use script::dom::bindings::conversions::IDLInterface;
use script::dom::bindings::conversions::DerivedFrom;
...
pub struct XMLHttpRequest {
    ...
    upload: Dom<XMLHttpRequestUpload<TypeHolder>>,
    response_url: DomRefCell<String>,
    status: Cell<u16>,
    status_text: DomRefCell<ByteString>,
    response: DomRefCell<ByteString>,
    response_type: Cell<XMLHttpRequestResponseType>,
    response_xml: MutNullableDom<Document<TypeHolder>>,
    response_blob: MutNullableDom<Blob<TypeHolder>>,
...
impl DerivedFrom<XMLHttpRequestEventTarget<TypeHolder>> for XMLHttpRequest {} 


impl XMLHttpRequestTrait<TypeHolder> for XMLHttpRequest {
    // https://xhr.spec.whatwg.org/#constructors
    fn Constructor(global: &GlobalScope<TypeHolder>) -> Fallible<DomRoot<XMLHttpRequest>> {
...
impl XMLHttpRequest {
    fn new_inherited(global: &GlobalScope<TypeHolder>) -> XMLHttpRequest {
        //TODO - update this when referrer policy implemented for workers
        let (referrer_url, referrer_policy) = if let Some(window) = global.downcast::<Window<TypeHolder>>() {
    ...
    fn new(global: &GlobalScope<TypeHolder>) -> DomRoot<XMLHttpRequest> {
    ...
    fn sync_in_window(&self) -> bool {
        self.sync.get() && self.global().is::<Window<TypeHolder>>()
    }

    fn initiate_async_xhr(context: Arc<Mutex<XHRContext>>,
                          task_source: NetworkingTaskSource<TypeHolder>,
                          global: &GlobalScope<TypeHolder>,
                          init: RequestInit,
...
impl XMLHttpRequestMethods<TypeHolder> for XMLHttpRequest{
    ...
    fn Open_(&self, method: ByteString, url: USVString, async: bool,
        ...
        // Step 1
        if let Some(window) = DomRoot::downcast::<Window<TypeHolder>>(self.global()) {
    ...
    fn Upload(&self) -> DomRoot<XMLHttpRequestUpload<TypeHolder>> {
        DomRoot::from_ref(&*self.upload)
    }

    // https://xhr.spec.whatwg.org/#the-send()-method
    fn Send(&self, data: Option<DocumentOrBodyInit<TypeHolder>>) -> ErrorResult {
        // Step 1, 2
        if self.ready_state.get() != XMLHttpRequestState::Opened || self.send_flag.get() {
            return Err(Error::InvalidState);
        }
        ...
        let has_handlers = self.upload.upcast::<EventTarget<TypeHolder>>().has_handlers();
    ...
    fn SetResponseType(&self, response_type: XMLHttpRequestResponseType) -> ErrorResult {
        // Step 1
        if self.global().is::<WorkerGlobalScope<TypeHolder>>() && response_type == XMLHttpRequestResponseType::Document {
            return Ok(());
        }
        match self.ready_state.get() {
            // Step 2
    ...
    fn GetResponseXML(&self) -> Fallible<Option<DomRoot<Document<TypeHolder>>>> {
        // TODO(#2823): Until [Exposed] is implemented, this attribute needs to return null
        //              explicitly in the worker scope.
        if self.global().is::<WorkerGlobalScope<TypeHolder>>() {
            return Ok(None);
        }
...
type TrustedXHRAddress = Trusted<XMLHttpRequest>;
...
impl XMLHttpRequest {
    ...
    fn dispatch_progress_event(&self, upload: bool, type_: Atom, loaded: u64, total: Option<u64>) {
        ...
        progressevent.upcast::<Event<TypeHolder>>().fire(target);
    }
    ...
    fn blob_response(&self) -> DomRoot<Blob<TypeHolder>> {
        // Step 1
    ...
    fn document_response(&self) -> Option<DomRoot<Document<TypeHolder>>> {
        // Step 1
        ...
        let charset = self.final_charset().unwrap_or(UTF_8);
        let temp_doc: DomRoot<Document<TypeHolder>>;
        match mime_type {
    ...
    fn document_text_html(&self) -> DomRoot<Document<TypeHolder>> {
        let charset = self.final_charset().unwrap_or(UTF_8);
        let wr = self.global();
        let response = self.response.borrow();
        ...
        <TypeHolder as TypeHolderTrait>::ServoParser::parse_html_document(
            &document,
    ...
    fn handle_xml(&self) -> DomRoot<Document<TypeHolder>> {
        let charset = self.final_charset().unwrap_or(UTF_8);
        let wr = self.global();
        let response = self.response.borrow();
        let (decoded, _, _) = charset.decode(&response);
        ...
        // TODO: Disable scripting while parsing
        <TypeHolder as TypeHolderTrait>::ServoParser::parse_xml_document(
            &document,
            DOMString::from(decoded),
            wr.get_url());
        document
    ...
    fn new_doc(&self, is_html_document: IsHTMLDocument) -> DomRoot<Document<TypeHolder>> {
        let wr = self.global();
    ...
    fn fetch(&self,
              init: RequestInit,
              global: &GlobalScope<TypeHolder>) -> ErrorResult {
        ...
        let (task_source, script_port) = if self.sync.get() {
            ...
            (NetworkingTaskSource(tx, global.pipeline_id(), Default::default()), Some(rx))
        } else {
...
fn is_field_value(slice: &[u8]) -> bool {
...
impl XHRTimeoutCallbackTrait<TypeHolder> for XHRTimeoutCallback {
    fn invoke(self) {
        let xhr = self.xhr.root();
...
fn serialize_document(doc: &Document<TypeHolder>) -> Fallible<DOMString> { 
    let mut writer = vec![]; 
    match serialize(&mut writer, &doc.upcast::<Node<TypeHolder>>(), SerializeOpts::default()) { 
