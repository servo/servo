...
#[macro_use] extern crate domobject_derive;
...
extern crate msg;
#[macro_use] extern crate mozjs as js;
...
impl ServoParserTrait<TypeHolder> for ServoParser {
    ...
    fn parse_html_document(document: &Document<TypeHolder>, input: DOMString, url: ServoUrl) {
        let parser = if PREFS.get("dom.servoparser.async_html_tokenizer.enabled").as_boolean().unwrap() {
            ServoParser::new(document,
                                Tokenizer::AsyncHtml(async_html::Tokenizer::new(
                                        document,
                                        url,
                                        None,
    ...
    fn parse_html_fragment(
        context: &Element<TypeHolder>,
        input: DOMString
    ) -> Box<Iterator<Item=DomRoot<Node<TypeHolder>>>> {
    ...
    fn parse_xml_document(document: &Document<TypeHolder>, input: DOMString, url: ServoUrl) {
        let parser = ServoParser::new(document,
                                        Tokenizer::Xml(xml::Tokenizer::new(
                                                document,
                                                url,
                                                None,
                                                ParsingAlgorithm::Normal
    ...
    fn resume_with_pending_parsing_blocking_script(
        &self,
        script: &HTMLScriptElement<TypeHolder>,
        result: ScriptResult
    ) {
