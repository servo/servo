...
fn create_constellation(opts: opts::Opts,
    ...
    let image_cache_thread = new_image_cache_thread(resource_threads.resource_thread.clone(),
                                                    webrender_api_sender.as_ref().map(|wr| wr.create_api()));
    let font_cache_thread = FontCacheThread::new(resource_threads.resource_thread.clone(),
                                                 webrender_api_sender.as_ref().map(|wr| wr.create_api()));
