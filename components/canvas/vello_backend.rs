...
impl GenericDrawTarget<VelloBackend> for DrawTarget {
    ...
    fn create_source_surface_from_data(&self, data: &[u8]) -> Option<Vec<u8>> {
        ...
        pixels::generic_transform_inplace::<2, true, false>(&mut data);
        Some(data)
    }
        ...
        self.with_draw_options(draw_options, move |self_| {
            self_.scene.fill(
                ...
                &peniko::Image {
                    ...
                    x_extend: peniko::Extend::Pad,
                    y_extend: peniko::Extend::Pad,
                    quality: filter.convert(),
                    alpha: draw_options.alpha,
                },
                Some(
                    kurbo::Affine::translate((dest.origin.x, dest.origin.y))
    ...
    fn fill(&mut self, path: &kurbo::BezPath, pattern: &peniko::Brush, draw_options: &DrawOptions) {
        self.with_draw_options(draw_options, |self_| {
            self_.scene.fill(
                peniko::Fill::NonZero,
                self_.transform,
                &pattern.clone().multiply_alpha(draw_options.alpha),
                None,
                path,
            );
        })
    ...
    fn fill_text(
        ...
        let pattern = pattern.clone().multiply_alpha(draw_options.alpha);
        self.with_draw_options(draw_options, |self_| {
            let mut advance = 0.;
    ...
    fn fill_rect(&mut self, rect: &Rect<f32>, pattern: &peniko::Brush, draw_options: &DrawOptions) {
        let pattern = pattern.clone().multiply_alpha(draw_options.alpha);
        self.with_draw_options(draw_options, |self_| {
            self_.scene.fill(
                peniko::Fill::NonZero,
    ...
    fn push_clip(&mut self, path: &kurbo::BezPath) {
        self.scene
            .push_layer(peniko::Mix::Clip, 1.0, self.transform, path);
    }
    ...
    fn bytes(&self) -> Cow<[u8]> {
        ...
        let result = {
            let size: usize = (self.size.width * self.size.height * 4).try_into().unwrap();
                ...
                vello::util::block_on_wgpu(&self.device, receiver.receive()).unwrap()
            {
                ...
                return Cow::Owned(vec![0; size]);
            }
            let data = buf_slice.get_mapped_range();
            let mut result_unpadded = Vec::<u8>::with_capacity(size);
            ...
            // swap RB
            pixels::generic_transform_inplace::<1, true, false>(&mut result_unpadded);
            result_unpadded
        };
        buffer.unmap();
        Cow::Owned(result)
...
impl GenericPathBuilder<VelloBackend> for kurbo::BezPath {
    ...
    fn get_current_point(&mut self) -> Option<Point2D<f32>> {
        fn first_point(path: &kurbo::PathEl) -> Option<kurbo::Point> {
            match path {
                kurbo::PathEl::MoveTo(p) => Some(*p),
                kurbo::PathEl::LineTo(p1) => Some(*p1),
                kurbo::PathEl::QuadTo(p1, _) => Some(*p1),
                kurbo::PathEl::CurveTo(p1, _, _) => Some(*p1),
        ...
        self.elements()
            ...
            .or_else(|| {
                self.elements()
                    .first()
                    .and_then(|first| first_point(first).map(Convert::convert))
            })
    }
...
impl PathHelpers<VelloBackend> for kurbo::BezPath {
    ...
    fn contains_point(&self, x: f64, y: f64, path_transform: &Transform2D<f32>) -> bool {
        ...
        path.apply_affine(path_transform.convert());
        path.close_path();
        path.contains(point) // TODO: impl on edge detection (preferably in kurbo)
    }
...
impl PatternHelpers for peniko::Brush {
    ...
    fn draw_rect(&self, rect: &Rect<f32>) -> Rect<f32> {
        match self {
            Self::Image(image) => *rect, // TODO: impl
            Self::Gradient(_) | Self::Solid(_) => *rect,
        }
    }
}
    ...
    fn convert(self) -> peniko::Brush {
        ...
        match self {
            ...
            Surface(surface_style) => {
                let mut data = surface_style.surface_data.into_vec();
                pixels::generic_transform_inplace::<2, true, false>(&mut data);
                peniko::Brush::Image(peniko::Image {
                    ...
                    /*
                    This is problematic, because vello does not know concept of no repeat
                    it will just pad the image with edge bytes, but we would want to simply clip the shape
                    so we will either need another clip layer or fix this in vello
                    */
