...
use canvas_traits::{CanvasMsg, CanvasWebGLMsg, CanvasCommonMsg, FromLayoutMsg};
...
impl WebGLPaintTask {
    ...
    pub fn start(size: Size2D<i32>, attrs: GLContextAttributes)
        ...
        let in_process_chan_for_router = in_process_chan.clone();
        ROUTER.add_route(out_of_process_port.to_opaque(), Box::new(move |message| {
            drop(in_process_chan_for_router.send(message.to().unwrap()))
        }));
                ...
                match in_process_port.recv().unwrap() {
                    ...
                    CanvasMsg::FromLayout(message) => {
                        match message {
                            ...
                            FromLayoutMsg::SendNativeSurface(chan) =>
