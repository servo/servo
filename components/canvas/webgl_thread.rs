...
impl WebGLImpl {
    ...
    fn get_uniform(
        ...
        let uniform = match location as u32 {
            ...
            gl::FLOAT_MAT2 |
            gl::FLOAT_MAT3 |
            gl::FLOAT_MAT4 => Ok(WebGLParameter::FloatArray(
                            gl.get_uniform_fv(program, location))),
            gl::INT_VEC2 |
            gl::INT_VEC3 |
            gl::INT_VEC4 => Ok(WebGLParameter::IntArray(
                            gl.get_uniform_iv(program, location))),
            gl::BOOL_VEC2 |
            gl::BOOL_VEC3 |
            gl::BOOL_VEC4 => {
                let result = gl.get_uniform_iv(program, location)
                    ...
                    .map(|el| *el == 1)
                    .collect::<Vec<_>>();
                Ok(WebGLParameter::BoolArray(result))
            },
            gl::INT |
            gl::SAMPLER_2D |
            gl::SAMPLER_CUBE => {
