...
use fnv::FnvHashMap;
use glow::{
    self as gl, Context as Gl, HasContext, NativeFramebuffer, NativeTransformFeedback,
    NativeUniformLocation, NativeVertexArray, TransformFeedback,
};
use half::f16;
...
impl WebGLThread {
    ...
    fn create_webgl_context(
        ...
        unsafe {
            gl.bind_framebuffer(
                gl::FRAMEBUFFER,
                NonZeroU32::new(framebuffer).map(|fb| NativeFramebuffer(fb)),
            );
        ...
        {
            WebGLImpl::bind_vertex_array(
                &gl,
                Some(vao.glow()),
                use_apple_vertex_array,
                webgl_version,
            );
            Some(vao.glow())
        } else {
...
impl WebGLImpl {
    ...
    pub fn apply(
        ...
        match command {
            ...
            WebGLCommand::CreateTransformFeedback(ref sender) => {
                let value = unsafe { gl.create_transform_feedback().unwrap() };
                sender.send(value.0.get()).unwrap()
            },
            ...
            WebGLCommand::GetTransformFeedbackVarying(program, index, ref sender) => {
                let mut active_transform_feedback =
                ...
                let name = active_transform_feedback.name.split_off(2);
                sender
                    .send((
                        active_transform_feedback.size,
                        active_transform_feedback.tftype,
                        name,
                    ))
                    .unwrap();
            },
            WebGLCommand::TransformFeedbackVaryings(program, varyings, buffer_mode) => {
                let varyings: Vec<&str> = varyings.iter().map(String::as_ref).collect();
                unsafe {
                    gl.transform_feedback_varyings(program.glow(), varyings.as_slice(), buffer_mode)
                };
            },
            ...
            WebGLCommand::TexImage2DPBO {
                ...
                unsafe {
                    ...
                    gl.tex_image_2d(
                        ...
                        format.as_gl_constant(),
                        effective_data_type,
                        // TODO(safe_gl): Fix this in glow
                        Some(slice::from_raw_parts(offset as _, 0)),
                    );
                }
            },
            ...
            WebGLCommand::FenceSync(ref sender) => {
                ...
                sender
                    .send(unsafe { WebGLSyncId::from_glow(value) })
                    .unwrap();
            },
            ...
            WebGLCommand::GetUniformIndices(program_id, ref names, ref sender) => {
                ...
                let indices = gl
                    .get_uniform_indices(program_id.glow(), &name_strs)
                    .iter()
            ...
            WebGLCommand::GetActiveUniforms(program_id, ref indices, pname, ref sender) => {
                let results = gl.get_active_uniforms_iv(program_id.glow(), indices, pname);
            ...
            WebGLCommand::GetActiveUniformBlockParameter(
                ...
                let results = gl.get_active_uniform_block_iv(program_id.glow(), block_idx, pname);
                sender.send(results).unwrap();
            },
            ...
            WebGLCommand::GenerateQuery(ref sender) => {
                ...
                sender.send(unsafe { WebGLQueryId::from_glow(id) }).unwrap()
            },
            WebGLCommand::GetQueryState(ref sender, query_id, pname) => {
                // TODO(gl_dev): glow does not have EXT for gles2
                let value = unsafe { gl.get_query_parameter_u32(query_id.get(), pname) };
                sender.send(value).unwrap()
            },
            WebGLCommand::GenerateSampler(ref sender) => {
                let id = unsafe { gl.create_sampler().unwrap() };
                sender
                    .send(unsafe { WebGLSamplerId::from_glow(id) })
                    .unwrap()
            },
        ...
        #[cfg(debug_assertions)]
        {
            ...
            if error != gl::NO_ERROR {
                ...
                // (servo#30568) revert to panic!() once underlying bug is fixed
                log::warn!(
    ...
    fn link_program(gl: &Gl, program: WebGLProgramId) -> ProgramLinkInfo {
        ...
        let active_uniforms = (0..num_active_uniforms as u32)
            .map(|i| {
                ...
                let mut active_uniform =
                    unsafe { gl.get_active_uniform(program.glow(), i) }.unwrap();
                let is_array = active_uniform.name.ends_with("[0]");
                if is_array {
                    // FIXME(nox): NLL
                    let len = active_uniform.name.len();
                    active_uniform.name.truncate(len - 3);
                }
                ActiveUniformInfo {
                    base_name: from_name_in_compiled_shader(&active_uniform.name).into(),
                    size: if is_array {
        ...
        ProgramLinkInfo {
            ...
            transform_feedback_length: transform_feedback_length,
            transform_feedback_mode: transform_feedback_mode,
        }
    }
    ...
    fn create_vertex_array(
        ...
        let vao = match gl {
            ...
            _ => unsafe { gl.create_vertex_array() }.ok(),
        }
    ...
    fn bind_vertex_array(
        ...
        match gl {
            ...
            _ => unsafe { gl.bind_vertex_array(vao) },
        }
        debug_assert_eq!(unsafe { gl.get_error() }, gl::NO_ERROR);
    }
    ...
    fn delete_vertex_array(
        ...
        match gl {
            ...
            _ => unsafe { gl.delete_vertex_array(vao.glow()) },
        }
        debug_assert_eq!(unsafe { gl.get_error() }, gl::NO_ERROR);
    }
