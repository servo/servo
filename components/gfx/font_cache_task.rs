...
pub enum Command {
    ...
    AddWebFont(Atom, Source, Sender<WebFontLoadResult>),
    AddDownloadedWebFont(LowercaseString, Url, Vec<u8>, Sender<WebFontLoadResult>),
    Exit(Sender<()>),
}
        ...
        loop {
            ...
            match msg {
                ...
                Command::AddWebFont(family_name, src, result) => {
                    ...
                    match src {
                        Source::Url(ref url_source) => {
                            ...
                            ROUTER.add_route(data_receiver.to_opaque(), box move |message| {
                                ...
                                match response {
                                    ...
                                    ResponseAction::ResponseComplete(_) => {
                        ...
                        Source::Local(ref local_family_name) => {
                            ...
                            result.send(WebFontLoadResult::NoNewFontLoaded).unwrap();
                        }
                    }
                }
                Command::AddDownloadedWebFont(family_name, url, bytes, result) => {
                    ...
                    drop(result.send(WebFontLoadResult::NewFontLoaded));
                }
                Command::Exit(result) => {
                    result.send(()).unwrap();
                    break;
...
impl FontCacheTask {
    ...
    pub fn add_web_font(&self, family: Atom, src: Source, sender: Sender<WebFontLoadResult>) {
