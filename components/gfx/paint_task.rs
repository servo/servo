    ...
    fn start(&mut self) {
        ...
        loop {
            let message = {
                ...
                let mut layout_to_paint_port = select.handle(&self.layout_to_paint_port);
                let mut chrome_to_paint_port = select.handle(&self.chrome_to_paint_port);
                unsafe {
                    layout_to_paint_port.add();
                    chrome_to_paint_port.add();
                }
                let result = select.wait();
                if result == layout_to_paint_port.id() {
                    Msg::FromLayout(self.layout_to_paint_port.recv().unwrap())
                } else if result == chrome_to_paint_port.id() {
