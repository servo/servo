        ...
        loop {
            match self.port.recv().unwrap() {
                ...
                Msg::Paint(requests, frame_tree_id) => {
                    ...
                    for PaintRequest { buffer_requests, scale, layer_id, epoch, is_3d }
                          in requests.into_iter() {
                        if self.current_epoch == Some(epoch) {
                            self.paint(&mut replies, buffer_requests, scale, layer_id, is_3d);
                        } else {
...
impl WorkerThreadProxy {
    ...
    fn paint_tile(&mut self,
                  ...
                  scale: f32,
                  is_3d: bool) {
        let msg = MsgToWorkerThread::PaintTile(thread_id, tile, layer_buffer, stacking_context, scale, is_3d);
    ...
    fn main(&mut self) {
        loop {
            match self.receiver.recv().unwrap() {
                ...
                MsgToWorkerThread::PaintTile(thread_id, tile, layer_buffer, stacking_context, scale, is_3d) => {
                    let draw_target = self.optimize_and_paint_tile(thread_id, &tile, stacking_context, scale, is_3d);
...
enum MsgToWorkerThread {
    Exit,
    PaintTile(usize, BufferRequest, Option<Box<LayerBuffer>>, Arc<StackingContext>, f32, bool),
}
