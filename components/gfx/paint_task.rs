        ...
        spawn_named_with_send_on_failure(format!("PaintTask {:?}", id), task_state::PAINT, move || {
            {
                ...
                let (memory_reporter_sender, memory_reporter_receiver) =
                    ipc::channel::<ReporterRequest>().unwrap();
                let paint_chan_for_memory_reporter = chan.clone();
                ROUTER.add_route(memory_reporter_receiver.to_opaque(), box move |message| {
                    // Just injects an appropriate event into the paint task's queue.
                    let request: ReporterRequest = message.to().unwrap();
                    paint_chan_for_memory_reporter.0.send(Msg::CollectReports(
