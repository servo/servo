...
fn paint_reporter_thread(receiver: IpcReceiver<ReporterRequest>, sender: PaintChan) {
        ...
        spawn_named_with_send_on_failure(format!("PaintTask {:?}", id), task_state::PAINT, move || {
            {
                ...
                let paint_chan_for_memory_reporter = chan.clone();
                spawn_named(reporter_name.clone(), move || {
                    paint_reporter_thread(memory_reporter_receiver, paint_chan_for_memory_reporter)
                });
