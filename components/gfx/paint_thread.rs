...
impl PaintLayer {
    fn new_from_stacking_context(layer_info: &LayerInfo,
        ...
        let transform = transform.post_mul(&stacking_context.transform);
        let perspective = perspective.post_mul(&stacking_context.perspective);
...
impl LayerCreator {
    ...
    fn create_layers_for_stacking_context<'a>(&mut self,
        ...
        self.process_stacking_context_items(stacking_context,
                                            ...
                                            &transform.post_mul(&stacking_context.transform),
                                            &perspective.post_mul(&stacking_context.perspective));
    }
...
impl WorkerThread {
    ...
    fn optimize_and_paint_tile(&mut self,
...

        {
            ...
            let matrix = matrix.post_scaled(scale as AzFloat, scale as AzFloat, 1.0);
            let tile_bounds = tile.page_rect.translate(&paint_layer.display_list_origin);
            let matrix = matrix.post_translated(-tile_bounds.origin.x as AzFloat,
