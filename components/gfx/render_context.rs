...
impl<'a> RenderContext<'a>  {
    ...
    pub fn draw_border(&self,
        ...
        static DIRECTIONS: [Direction, ..4] = [ Top, Right, Bottom, Left ];
        for &direction in DIRECTIONS.iter() {
            self.draw_border_segment(direction, bounds, border, &radius, color, style);
        }
    }
    ...
    fn draw_border_path(&self,
                        ...
                        color:     Color) {
        let box_NW = bounds.origin;
        let box_NE = box_NW + Point2D(bounds.size.width, 0.0);
        let box_SW = box_NW + Point2D(0.0, bounds.size.height);
        let box_SE = box_NW + Point2D(bounds.size.width, bounds.size.height);

        let draw_opts    = DrawOptions::new(1.0, 0);
        let path_builder = self.draw_target.create_path_builder();

        let rad_E: AzFloat = 0.;
        let rad_SE = rad_E  + Float::frac_pi_4();
        let rad_S  = rad_SE + Float::frac_pi_4();
        let rad_SW = rad_S  + Float::frac_pi_4();
        let rad_W  = rad_SW + Float::frac_pi_4();
        let rad_NW = rad_W  + Float::frac_pi_4();
        let rad_N  = rad_NW + Float::frac_pi_4();
        let rad_NE = rad_N  + Float::frac_pi_4();

        match direction {
            Top    => {
                let edge_NW = box_NW  + Point2D(radius.top_left.max(border.left), 0.);
                let edge_NE = box_NE  + Point2D(-radius.top_right.max(border.right), 0.);
                let edge_SE = edge_NE + Point2D(0., border.top);
                let edge_SW = edge_NW + Point2D(0., border.top);

                path_builder.move_to(edge_NW);
                path_builder.line_to(edge_NE);

                let origin = edge_NE + Point2D((border.right - radius.top_right).max(0.), radius.top_right);
                let distance_to_elbow = (radius.top_right - border.top).max(0.);

                path_builder.arc(origin, radius.top_right,  rad_N, rad_NE, false);
                path_builder.arc(origin, distance_to_elbow, rad_NE, rad_N, true);

                path_builder.line_to(edge_SE);
                path_builder.line_to(edge_SW);

                let origin = edge_NW + Point2D(-(border.left - radius.top_left).max(0.), radius.top_left);
                let distance_to_elbow = (radius.top_left - border.top).max(0.);

                path_builder.arc(origin, distance_to_elbow, rad_N, rad_NW, true);
                path_builder.arc(origin, radius.top_left,   rad_NW, rad_N, false);
            }
            Left   => {
                let edge_NW = box_NW  + Point2D(0., radius.top_left.max(border.top));
                let edge_SW = box_SW  + Point2D(0., -radius.bottom_left.max(border.bottom));
                let edge_NE = edge_NW + Point2D(border.left, 0.);
                let edge_SE = edge_SW + Point2D(border.left, 0.);

                path_builder.move_to(edge_SW);
                path_builder.line_to(edge_NW);

                let origin = edge_NW + Point2D(radius.top_left, -(border.top - radius.top_left).max(0.));
                let distance_to_elbow = (radius.top_left - border.left).max(0.);

                path_builder.arc(origin, radius.top_left,   rad_W, rad_NW, false);
                path_builder.arc(origin, distance_to_elbow, rad_NW, rad_W, true);

                path_builder.line_to(edge_NE);
                path_builder.line_to(edge_SE);

                let origin = edge_SW + Point2D(radius.bottom_left, (border.bottom - radius.bottom_left).max(0.));
                let distance_to_elbow = (radius.bottom_left - border.left).max(0.);

                path_builder.arc(origin, distance_to_elbow,  rad_W, rad_SW, true);
                path_builder.arc(origin, radius.bottom_left, rad_SW, rad_W, false);
            }
            Right  => {
                let edge_NE = box_NE  + Point2D(0., radius.top_right.max(border.top));
                let edge_SE = box_SE  + Point2D(0., -radius.bottom_right.max(border.bottom));
                let edge_NW = edge_NE + Point2D(-border.right, 0.);
                let edge_SW = edge_SE + Point2D(-border.right, 0.);

                path_builder.move_to(edge_SW);
                path_builder.line_to(edge_NW);

                let origin = edge_NE + Point2D(-radius.top_right, -(border.top - radius.top_right).max(0.));
                let distance_to_elbow = (radius.top_right - border.right).max(0.);

                path_builder.arc(origin, distance_to_elbow, rad_E, rad_NE, true);
                path_builder.arc(origin, radius.top_right,  rad_NE, rad_E, false);

                path_builder.line_to(edge_NE);
                path_builder.line_to(edge_SE);

                let origin = edge_SE + Point2D(-radius.bottom_right, (border.bottom - radius.bottom_right).max(0.));
                let distance_to_elbow = (radius.bottom_right - border.right).max(0.);

                path_builder.arc(origin, radius.bottom_right, rad_E, rad_SE, false);
                path_builder.arc(origin, distance_to_elbow,   rad_SE, rad_E, true);
            }
            Bottom => {
                let edge_SW = box_SW  + Point2D(radius.bottom_left.max(border.left), 0.);
                let edge_SE = box_SE  + Point2D(-radius.bottom_right.max(border.right), 0.);
                let edge_NW = edge_SW + Point2D(0., -border.bottom);
                let edge_NE = edge_SE + Point2D(0., -border.bottom);

                path_builder.move_to(edge_NW);
                path_builder.line_to(edge_NE);

                let origin = edge_SE + Point2D((border.right - radius.bottom_right).max(0.), -radius.bottom_right);
                let distance_to_elbow = (radius.bottom_right - border.bottom).max(0.);

                path_builder.arc(origin, distance_to_elbow,   rad_S, rad_SE, true);
                path_builder.arc(origin, radius.bottom_right, rad_SE, rad_S, false);

                path_builder.line_to(edge_SE);
                path_builder.line_to(edge_SW);

                let origin = edge_SW - Point2D((border.left - radius.bottom_left).max(0.), radius.bottom_left);
                let distance_to_elbow = (radius.bottom_left - border.bottom).max(0.);

                path_builder.arc(origin, radius.bottom_left, rad_S, rad_SW, false);
                path_builder.arc(origin, distance_to_elbow,  rad_SW, rad_S, true);
            }
        }
