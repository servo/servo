...
impl<'a> PaintContext<'a> {
    ...
    pub fn draw_image(&self,
        ...
        let matrix = Matrix2D::identity().post_translated(dest_rect.origin.x, dest_rect.origin.y);
        ...
        let baseline_origin = match text.orientation {
            ...
            SidewaysLeft => {
                ...
                self.draw_target.set_transform(
                    &draw_target_transform.post_mul(&Matrix2D::row_major(0., -1., 1., 0., x, y)));
                Point2D::zero()
            }
            SidewaysRight => {
                let x = origin.x.to_f32_px();
    ...
    pub fn get_or_create_temporary_draw_target(&mut self,
        ...
        if accum_blur > Au(0) {
            ...
            matrix = Matrix2D::identity()
                .post_translated(-temporary_draw_target_bounds.origin.x as AzFloat,
                                 -temporary_draw_target_bounds.origin.y as AzFloat)
                .post_mul(&old_transform);
        }
...
impl TemporaryDrawTarget {
    ...
    fn from_bounds(main_draw_target: &DrawTarget, bounds: &Rect<Au>) -> TemporaryDrawTarget {
        ...
        let matrix = Matrix2D::identity()
            .post_translated(-temporary_draw_target_bounds.origin.x as AzFloat,
                             -temporary_draw_target_bounds.origin.y as AzFloat)
            .post_mul(&draw_target_transform);
        temporary_draw_target.set_transform(&matrix);
        TemporaryDrawTarget {
