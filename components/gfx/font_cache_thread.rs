...
impl FontCache {
    ...
    fn handle_add_web_font(&mut self,
                           ...
                           sources: Vec<Source>,
                           sender: IpcSender<()>) {
        let src = if let Some(src) = sources.first() {
            src.clone()
        } else {
            sender.send(()).unwrap();
            return;
                    ...
                    match response {
                        ...
                        ResponseAction::ResponseComplete(response) => {
                            if response.is_err() || !*response_valid.lock().unwrap() {
                                let rest = sources[1..].iter().cloned().collect();
                                let msg = Command::AddWebFont(family_name.clone(), rest, sender.clone());
...
impl FontCacheThread {
    ...
    pub fn add_web_font(&self, family: FontFamily, sources: Vec<Source>, sender: IpcSender<()>) {
