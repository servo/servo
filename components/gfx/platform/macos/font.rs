        ...
        const FORMAT_0_HEADER_LEN: usize = 8;
        {
            ...
            for _ in 0..num_subtables {
                ...
                if cov == KERN_COVERAGE_HORIZONTAL_FORMAT_0 {
                    ...
                    result.scale = self.ctfont.pt_size() as f64 /
...
impl CachedKernTable {
    fn pair_data(&self) -> &[u8] {
        &self.font_table.buffer()[self.pair_data_range.clone()]
    }
}
...
impl FontHandleMethods for FontHandle {
    ...
    fn glyph_h_kerning(&self, first_glyph: GlyphId, second_glyph: GlyphId) -> FractionalPixel {
        let table = match self.h_kern_subtable {
            Some(ref table) => table,
            None => return 0.0
        };

        // Binary search.
        let pairs = table.pair_data();
        ...
        while start < end {
            ...
            if key > query {
                ...
                let value = BigEndian::read_i16(&pairs[i * KERN_PAIR_LEN + 4..]);
                trace!("Kerning for {}/{}: {:?}", first_glyph, second_glyph, value);
                return value as f64 * table.scale;
            }
        }
        trace!("No kerning info for {}/{}", first_glyph, second_glyph);
        0.0
    }
