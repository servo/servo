...
impl WebrenderExternalImageApi for WGPUExternalImages {
    ...
    fn unlock(&mut self, id: u64) {
        ...
        if let Some(context_data) = webgpu_contexts.get_mut(&id) {
            context_data.put_presentation(presentation);
        } else {
...
impl PresentationStagingBuffer {
    fn destroy(self, context_data: &mut ContextData) {
        if let Some(mut staging_buffer) = Arc::into_inner(self.staging_buffer) {
            staging_buffer.unmap();
            context_data.staging_buffers.push(staging_buffer);
        }
    }
...
impl ContextData {
    ...
    /// Returns none if no staging buffer available or failure when making it available
        ...
        self.staging_buffers
            ...
            .or_else(|| {
                if self.staging_buffers.is_empty() {
                    ...
                    Some(self.staging_buffers.len())
                }
            })
            .and_then(|pos| {
                let mut staging_buffer = self.staging_buffers.remove(pos);
    ...
    fn put_presentation(&mut self, presentation: PresentationStagingBuffer) {
        ...
        if let Some(stale_presentation) = stale_presentation {
            stale_presentation.destroy(self);
        }
    }
}
...
impl crate::WGPU {
    ...
    pub(crate) fn get_image(
        ...
        if let Some((texture_id, encoder_id, config)) = pending_texture {
            let Some(staging_buffer) = context_data.get_available_buffer(&config) else {
                ...
                self.texture_download(
                    ...
                    move |staging_buffer| {
                        ...
                        if staging_buffer.is_mapped() {
                            context_data.put_presentation(PresentationStagingBuffer::new(
                                epoch,
                                staging_buffer,
                            ));
                        } else {
            ...
            if let Err((staging_buffer, _)) = result {
                context_data.staging_buffers.push(staging_buffer);
                sender
                    .send(Snapshot::cleared(config.size).as_ipc())
                    .unwrap();
            } // ok is handled via callback
        } else {
            sender
                .send(
                    context_data
    ...
    pub(crate) fn present(
        ...
        let mut webgpu_contexts = self.wgpu_image_map.lock().unwrap();
        let context_data = webgpu_contexts.get_mut(&context_id).unwrap();
        let Some(staging_buffer) = context_data.get_available_buffer(&configuration) else {
            warn!("Failure obtaining available staging buffer");
            return;
        };
        let epoch = context_data.next_epoch();
        ...
        let result = self.texture_download(
            texture_id,
            encoder_id,
            staging_buffer,
            configuration,
            move |staging_buffer| {
                ...
                if staging_buffer.is_mapped() {
                    context_data
                        .put_presentation(PresentationStagingBuffer::new(epoch, staging_buffer));
                } else {
                    context_data.staging_buffers.push(staging_buffer);
                }
                compositor_api.update_image(
                    image_key,
                    image_desc(&configuration),
                    SerializableImageData::External(image_data(context_id)),
                );
                done.send(()).unwrap();
            },
    ...
    /// Copies data async from provided texture using encoder_id to provided staging buffer
    fn texture_download(
        &self,
        texture_id: TextureId,
        encoder_id: CommandEncoderId,
    ...
    ) -> Result<(), (StagingBuffer, Box<dyn std::error::Error>)> {
        let command_buffer_id = match staging_buffer
            .prepare_load_texture_command_buffer(texture_id, encoder_id, &config)
        {
            Ok(id) => id,
            Err(error) => return Err((staging_buffer, error)),
        };
        ...
        let buffer_size = buffer.size;
        {
            ...
            if let Err((_, error)) = result {
                return Err((staging_buffer, error.into()));
            }
        }
