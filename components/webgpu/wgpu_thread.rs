...
impl WGPU {
    ...
    /// Dispatches error (if there is any) using simple
    /// heuristic to determine error type
    fn maybe_dispatch_error<T: std::fmt::Display>(
        &mut self,
        ...
        if let Some(error) = error {
            ...
            let error = if err.contains("OutOfMemory") {
