...
pub struct RelativePos {
    /// Relative offset from first byte, wrapping back from end if negative
    pub start: i64,
    /// Relative offset from one past last byte, wrapping back from start if negative
    pub end: i64,
}
...
impl RelativePos {
    ...
    pub fn from_abs_range(range: Range<usize>, size: usize) -> RelativePos {
        RelativePos {
            ...
            end: size as i64 - range.end as i64,
        }
    }
}
