...
impl<'a> ServoAnimationTrait<'a> for GenericImageDecoder<'a> {
...
#[typetag::serde]
impl ServoImageEncoderDecoderFactory for DefaultImageEncoderDecoderFactory {
...
impl<'a, T: AnimationDecoder<'a>> ServoAnimationTrait<'a> for T {
    fn boxed_into_frames(self: Box<Self>) -> Frames<'a> {
...
pub(crate) fn decode_animated_image(
    cors_status: CorsStatus,
    animation_decoder: Box<dyn ServoAnimationTrait<'_> + '_>,
) -> Option<RasterImage> {
    let mut width = 0;
    let mut height = 0;
