...
impl HstsList {
    ...
    pub fn from_preload(preload_content: &[u8]) -> Option<HstsList> {
        ...
        if hsts_entries.is_some() {
            let mut hsts_list: HstsList = HstsList::new();

            for hsts_entry in hsts_entries.unwrap().entries {
    ...
    pub fn is_host_secure(&self, host: &str) -> bool {
        let base_domain = reg_host(host);

        match self.entries_map.get(&base_domain) {
            Some(entries) => {
                entries.iter().any(|e| {
    ...
    fn has_domain(&self, host: &str, base_domain: &str) -> bool {
        match self.entries_map.get(base_domain) {
            Some(entries) => entries.iter().any(|e| e.matches_domain(&host)),
            _ => false,
        }
    }
    ...
    fn has_subdomain(&self, host: &str, base_domain: &str) -> bool {
        match self.entries_map.get(base_domain) {
            Some(entries) => entries.iter().any(|e| e.matches_subdomain(host)),
            _ => false,
        }
    }

    pub fn push(&mut self, entry: HstsEntry) {
        let base_domain = reg_host(&entry.host);
        let have_domain = self.has_domain(&entry.host, &base_domain);
        let have_subdomain = self.has_subdomain(&entry.host, &base_domain);

        let entries = self.entries_map.entry(base_domain).or_insert(vec![]);
