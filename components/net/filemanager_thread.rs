...
impl FileManager {
    fn select_file(&mut self, filter: Vec<FilterPattern>, sender: IpcSender<FileManagerResult<SelectedFile>>) {
        match tinyfiledialogs::open_file_dialog("Pick a file", "", None) {
            Some(s) => {
    ...
    fn select_files(&mut self, filter: Vec<FilterPattern>, sender: IpcSender<FileManagerResult<Vec<SelectedFile>>>) {
        match tinyfiledialogs::open_file_dialog_multi("Pick files", "", None) {
            Some(v) => {
