            ...
            match self.receiver.recv().unwrap() {
                ...
                FileManagerThreadMsg::ReadFile(sender, id) => {
                    match self.try_read_file(id) {
                        ...
                        Err(_) => sender.send(Err(FileManagerThreadError::ReadFileError)).unwrap()
                    }
                }
...
impl FileManager {
    fn select_file(&mut self, sender: IpcSender<FileManagerResult<SelectedFile>>) {
        ...
        match self.create_entry(selected_path) {
            Some(triple) => sender.send(Ok(triple)).unwrap(),
            None => sender.send(Err(FileManagerThreadError::InvalidSelection)).unwrap(),
        };
    }
