...
fn cors_preflight_fetch(request: Rc<Request>, cache: Option<BasicCORSCache>) -> Response {
    ...
    *preflight.method.borrow_mut() = Method::Options;
    ...
    // Step 3, 4
    let mut value: Vec<UniCase<String>> =
        request.headers.borrow().iter().map(|ref view| UniCase(view.name().to_owned())).collect();
    value.sort();
    ...
    // Step 7
    if cors_check(preflight, &response).is_ok()
       && response.status.map_or(false, |status| { status.is_success() }) {
        // Substep 4
        let methods = if !response.headers.has::<AccessControlAllowMethods>() && request.use_cors_preflight {
            vec![request.method.borrow().clone()]
        } else {
            // Substep 1
        ...
        // Substep 2
        let header_names = match response.headers.get::<AccessControlAllowHeaders>() {
...
fn cors_check(request: Rc<Request>, response: &Response) -> Result<(), ()> {
    ...
    // strings are already utf-8 encoded, so there's need to re-encode origin for this step
