...
fn should_request_be_blocked_as_mixed_content(client: Option<&Client>,
                                              ...
                                              -> bool {
    // Step 1.1.
    if !client.map_or(false, |client| client.prohibit_mixed_security_contexts) {
        return false;
    }
    ...
    if destination == Destination::Document
            && client.map_or(true, |client| !client.target_browsing_context_has_parent_browsing_context) {
        return false;
    }
...
fn should_response_be_blocked_as_mixed_content(response_https_state: HttpsState,
                                               ...
                                               -> bool {
    // Step 1.1.
    if !request_client.map_or(false, |client| client.prohibit_mixed_security_contexts) {
        return false;
    }
    ...
    // Step 1.4.
    if destination == Destination::Document
            && request_client.map_or(true, |client| !client.target_browsing_context_has_parent_browsing_context) {
        return false;
    }
