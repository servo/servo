...
use net_traits::request::{CacheMode, CredentialsMode, FetchTaskTarget};
...
fn http_network_or_cache_fetch(request: Rc<Request>,
                               ...
                               authentication_fetch_flag: bool,
                               done_sender: Option<Sender<()>>) -> Response {
    ...
    // Step 18
    if response.is_none() {
        response = Some(http_network_fetch(request.clone(), http_request.clone(), credentials_flag, done_sender));
    }
...
fn cors_preflight_fetch(request: Rc<Request>, cache: &mut CORSCache) -> Response {
    ...
    *preflight.method.borrow_mut() = Method::Options;
    ...
    let response = http_network_or_cache_fetch(preflight.clone(), false, false, None);
