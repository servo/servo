...
/// We won't require any of the optional values if we don't wanna cancel.
pub struct CancellationListener {
    ...
    resource_task: Option<ResourceTask>,
}

impl CancellationListener {
    pub fn new(receiver: Option<Receiver<CancelLoad>>,
               res_id: Option<ResourceId>,
               resource_task: Option<ResourceTask>) -> CancellationListener {
...
impl Drop for CancellationListener {
    fn drop(&mut self) {
        if let Some(ref sender) = self.resource_task {
            // we just wanna make sure that the ResourceId (which is no longer necessary)
            // is removed from the resource manager's hashmap
            let _ = sender.send(ControlMsg::Cancel(self.resource_id.unwrap()));
        }
    }
}
...
impl ResourceManager {
    ...
    fn load(&mut self,
        ...
        let cancel_listener = id_sender.map_or_else(|| {
            ...
            CancellationListener::new(Some(cancel_receiver), Some(resource_id), Some(resource_task))
        });
