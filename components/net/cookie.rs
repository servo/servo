...
pub struct Cookie {
    ...
    pub expiry_time: Option<Serde<DateTime<Utc>>>,
}
...
impl Cookie {
    ...
    pub fn new_wrapped(
        ...
        let (persistent, expiry_time) = match (cookie.max_age(), cookie.expires()) {
            (Some(max_age), _) => (
                true,
                Some(Utc::now() + Duration::seconds(max_age.num_seconds())),
            ),
            (_, Some(expires)) => (
                true,
                Some(
                    match Utc
                        .timestamp_opt(expires.to_timespec().sec, expires.to_timespec().nsec as u32)
                    {
                        LocalResult::Single(date_time) => date_time,
                        LocalResult::Ambiguous(date_time, _) => date_time,
                        LocalResult::None => {
                            warn!("Out-of-range number of seconds and/or invalid nanosecond.");
                            Utc::now()
                        },
                    },
                ),
    ...
    pub fn set_expiry_time_negative(&mut self) {
        self.expiry_time = Some(Serde(Utc::now() - Duration::seconds(1)));
    }
