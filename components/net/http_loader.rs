...
fn set_request_id(msg: ChromeToDevtoolsControlMsg, id: &String) -> ChromeToDevtoolsControlMsg {
    match msg {
        ChromeToDevtoolsControlMsg::NetworkEvent(_, event) =>
            ChromeToDevtoolsControlMsg::NetworkEvent(id.clone(), event),
        _ => msg
    }
}
    ...
    loop {
        ...
        if response.status().class() == StatusClass::Redirection {
            if let Some(&Location(ref new_url)) = response.headers().get::<Location>() {
                ...
                let request_id = uuid::Uuid::new_v4().simple().to_string();
                if let Some(m) = msg {
                    let new_msg = set_request_id(m, &request_id);
