...
use resource_task::{start_sending_opt, start_sending_sniffed_opt, replace_hosts};
...
reason: \"certificate verify failed\" }]))";
        ...
        let (tx, rx) = channel();
        cookies_chan.send(ControlMsg::GetCookiesForUrl(doc_url.clone(), tx, CookieSource::HTTP)).unwrap();
        ...
        if let Some(cookies) = response.headers.get_raw("set-cookie") {
            for cookie in cookies.iter() {
                if let Ok(cookies) = String::from_utf8(cookie.clone()) {
                    cookies_chan.send(ControlMsg::SetCookiesForUrl(doc_url.clone(),
                                                                   cookies,
