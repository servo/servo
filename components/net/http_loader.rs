...
pub fn load<A>(load_data: LoadData,
    ...
    loop {
        ...
        modify_request_headers(&mut request_headers, doc_url.clone(), user_agent.clone(), &cookie_jar);
        ...
        process_response_headers(&response, &url, doc_url.clone(), &cookie_jar, &hsts_list);
