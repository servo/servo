...
use net_traits::{RequestSource, HttpResponse};
...
impl StreamedResponse {
    ...
    fn custom_response(response: Box<HttpResponse>, m: Metadata) -> StreamedResponse {
...
pub fn load<A, B>(load_data: &LoadData,
    ...
    // Cases when the client wants to send custom response
    // if its controlled by a service worker.
    ...
    match load_data.source {
        RequestSource::Window(ref sender) => {
            ...
            let received_response = msg_receiver.recv().unwrap();
            let metadata = Metadata::default(doc_url.clone());
            return Ok(StreamedResponse::custom_response(box received_response.unwrap(), metadata));
        }
