...
pub fn load<A>(load_data: LoadData,
    ...
    if cancel_listener.is_cancelled() {
        return Err(LoadError::new(url, LoadErrorType::Cancelled, "HTTP load cancelled".to_owned()));
    }
            ...
            *req.headers_mut() = request_headers.clone();
            ...
            response = match maybe_response {
                ...
                Err(e) => match e.error_type {
                    LoadErrorType::ConnectionAborted => {
        ...
        if response.status().class() == StatusClass::Redirection {
            if let Some(&Location(ref new_url)) = response.headers().get::<Location>() {
                ...
                if let Some(ref c) = load_data.cors {
                    if c.preflight {
                        return Err(LoadError::new(url, LoadErrorType::Cors,
...
fn send_data<R: Read>(reader: &mut R,
    ...
    let load_consumer = start_chan.clone();
    ...
    loop {
        if cancel_listener.is_cancelled() {
            return send_error(LoadError::new(url, LoadErrorType::Cancelled, "HTTP load cancelled".to_owned()),
                              load_consumer);
        }
