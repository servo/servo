...
use crate::methods::write_html_safe;
use crate::{
...
fn test_local_directory_listing() {
    let path = Path::new("../../tests/wpt/directory-listing-example/")
        .canonicalize()
        .unwrap();
    let url = ServoUrl::from_file_path(path.clone()).unwrap();
    ...
    let full_path = path
        ...
        .to_str()
        .map(str::to_string)
        .unwrap_or("<invalid path>".to_string());
    expected.push_str(
        "<!DOCTYPE html>\
<html lang=\"en\">\
<head><title>Directory listing: ",
    );
    write_html_safe(&full_path, &mut expected);
    ...
    write_html_safe(&full_path, &mut expected);
    expected.push_str(
        "</span></h1></header><div class=\"directory_info\">\
        <div class=\"parent_link\"><a href=\"../\">Up to parent directory</a></div>\
        <div class=\"listing\"><div class=\"row file\">\
        <div class=\"name\"><a href=\"README.txt\">README.txt</a></div>\
        <div class=\"size\">375 <abbr title=\"bytes\">B</abbr></div>\
        <div class=\"modified\"></div></div><div class=\"row directory\">\
        <div class=\"name\"><a href=\"html&lt;unsafe&amp;weird&gt;directoryname/\">\
        html&lt;unsafe&amp;weird&gt;directoryname/</a></div><div class=\"size\">-</div>\
        <div class=\"modified\"></div></div><div class=\"row file\">\
        <div class=\"name\"><a href=\"html&lt;unsafe&amp;weird&gt;filename.html\">\
        html&lt;unsafe&amp;weird&gt;filename.html</a></div>\
        <div class=\"size\">88 <abbr title=\"bytes\">B</abbr></div>\
        <div class=\"modified\"></div></div><div class=\"row symlink\">\
        <div class=\"name\">\
        <a href=\"soft_symlink_example.html\">soft_symlink_example.html</a></div>\
        <div class=\"size\">19 <abbr title=\"bytes\">B</abbr></div>\
        <div class=\"modified\"></div></div><div class=\"row file\">\
        <div class=\"name\"><a href=\"some_file_name.html\">some_file_name.html</a></div>\
        <div class=\"size\">102 <abbr title=\"bytes\">B</abbr></div>\
        <div class=\"modified\"></div></div><div class=\"row directory\">\
        <div class=\"name\">\
        <a href=\"sub-directory-example/\">sub-directory-example/</a></div>\
        <div class=\"size\">-</div><div class=\"modified\"></div></div></div></div>\
    ...
    match *resp_body {
        ResponseBody::Done(ref val) => {
            ...
            // We can't rely on the last-modification datetimes remaining the same,
            // so strip out all of the last-modification content to allow the rest of
            // the HTML to be simply compared.
            let (redacted_html, last_mod_values) = remove_file_modification_datetimes(string_val);
            assert_eq!(&redacted_html, &expected);
            // Now check that the datetime content appears to have the expected structure.
            for last_mod in last_mod_values {
                // TODO: Consider whether it's worth adding more rigorous checks for time.
                assert!(last_mod.starts_with("<time datetime=\"20"));
                assert!(last_mod.ends_with("</span></span></time>"))
            }
        },
        _ => panic!(),
    }
...
fn remove_file_modification_datetimes(string_val: String) -> (String, Vec<String>) {
    let mut output = String::with_capacity(string_val.len());
    let mut discards = Vec::new();
    let start_tag = "<div class=\"modified\">";
    let start_tag_length = start_tag.len();
    let end_tag = "</div>";
    let end_tag_length = end_tag.len();
    let mut from_index = 0;
    while let Some(mod_start_index) = string_val[from_index..].find(start_tag) {
        let mod_start_index = from_index + mod_start_index;
        println!("from_index is {}", from_index);
        println!("Found mod_start_index: {}", mod_start_index);
        output.push_str(&string_val[from_index..(mod_start_index + start_tag_length)]);
        from_index = mod_start_index + start_tag_length;
        println!("Incremented from_index to {}", from_index);
        if let Some(mod_end_index) = string_val[from_index..].find(end_tag) {
            let mod_end_index = from_index + mod_end_index;
            println!("Found mod_end_index {}", mod_end_index);
            let discard = string_val[from_index..mod_end_index].to_string();
            discards.push(discard);
            output.push_str(end_tag);
            from_index = mod_end_index + end_tag_length;
            println!("Incremented from_index to {}", from_index);
        }
    }
    println!("Exited loop!");
    output.push_str(&string_val[from_index..]);
    (output, discards)
}

// Yes, this is a unit test for a method required only by the unit test for directory listing.
#[test]
fn test_remove_file_modification_datetimes() {
    let input = "<html><p><div class=\"modified\">some text we don't want</div></p></body>";
    let expected = "<html><p><div class=\"modified\"></div></p></body>";
    let (redacted, discards) = remove_file_modification_datetimes(input.to_string());
    assert_eq!(&redacted, &expected);
    assert_eq!(1, discards.len());
    assert_eq!("some text we don't want", discards.get(0).unwrap());
}

// This is also a unit test for that method required by the unit test for directory listing.
#[test]
fn test_remove_file_modification_datetimes_multiple() {
    let input =
        "<html><p><div class=\"modified\">some text we don't want</div><div>we want this</div>\
<div class=\"modified\">more <span class=\"demo\">text</span> we don't want</div>\
<div class=\"modified\">and <em>more</em></div></p></body>";
    let expected = "<html><p><div class=\"modified\"></div><div>we want this</div>\
<div class=\"modified\"></div>\
<div class=\"modified\"></div></p></body>";
    let (redacted, discards) = remove_file_modification_datetimes(input.to_string());
    assert_eq!(&redacted, &expected);
    assert_eq!(3, discards.len());
    assert_eq!("some text we don't want", discards.get(0).unwrap());
    assert_eq!(
        "more <span class=\"demo\">text</span> we don't want",
        discards.get(1).unwrap()
    );
    assert_eq!("and <em>more</em>", discards.get(2).unwrap());
}
