...
use net_traits::{AsyncResponseTarget, Metadata, ProgressMsg, CoreResourceThread, ResponseAction};
...
pub fn new_resource_threads(user_agent: String,
    ...
    ResourceThreads::new(new_core_resource_thread(user_agent, devtools_chan),
...
pub fn new_core_resource_thread(user_agent: String,
                                devtools_chan: Option<Sender<DevtoolsControlMsg>>) -> CoreResourceThread {
    let hsts_preload = HstsList::from_servo_preload();
    let (setup_chan, setup_port) = ipc::channel().unwrap();
    let setup_chan_clone = setup_chan.clone();
    spawn_named("CoreResourceManager".to_owned(), move || {
...
impl CoreResourceManager {
    pub fn new(user_agent: String,
               ...
               devtools_channel: Option<Sender<DevtoolsControlMsg>>) -> CoreResourceManager {
