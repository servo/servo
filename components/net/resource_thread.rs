...
impl ResourceChannelManager {
    ...
    fn process_msg(&mut self,
        ...
        match msg {
            ...
            CoreResourceMsg::SetCookieHeaderForUrl(request, cookie_list, source) =>
                self.resource_manager.set_cookie_header_for_url(request, cookie_list, source, group),
            CoreResourceMsg::SetCookiesForUrl(request, cookies, source) =>
                self.resource_manager.set_cookies_for_url(request, cookies, source, group),
...
impl CoreResourceManager {
    ...
    fn set_cookie_header_for_url(&mut self,
                           request: ServoUrl,
                           cookie_list: String,
                           source: CookieSource,
    ...
    fn set_cookies_for_url(&mut self, request: ServoUrl,
                           cookies: Vec<Serde<cookie_rs::Cookie<'static>>>,
                           source: CookieSource,
                           resource_group: &ResourceGroup) {
        for cookie in cookies.into_iter() {
            if let Some(cookie) = cookie::Cookie::new_wrapped(cookie.into_inner(), &request, source) {
                let mut cookie_jar = resource_group.cookie_jar.write().unwrap();
                cookie_jar.push(cookie, &request, source)
            }
        }
