    ...
    fn process_msg(&mut self,
        ...
        match msg {
            CoreResourceMsg::Fetch(req_init, sender) =>
                self.resource_manager.fetch(req_init, ResponseInit::default(), false, sender, group),
            CoreResourceMsg::FetchRedirect(req_init, res_init, sender) =>
                self.resource_manager.fetch(req_init, res_init, true, sender, group),
...
impl CoreResourceManager {
    ...
    fn fetch(&self,
        ...
        thread::Builder::new().name(format!("fetch thread for {}", req_init.url)).spawn(move || {
            let request = Rc::new(Request::from_init(req_init));
            ...
            if redirect_flag {
                http_redirect_fetch(request, &mut CorsCache::new(), response, true, &mut sender, &mut None, &context);
            } else {
