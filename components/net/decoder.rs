...
impl Decoder {
    ...
    #[inline]
    fn plain_text(body: Body, url: ServoUrl) -> Decoder {
        Decoder {
            url: url,
    ...
    #[inline]
    fn pending(body: Body, type_: DecoderType, url: ServoUrl) -> Decoder {
        Decoder {
            inner: Inner::Pending(Pending {
                body: ReadableChunks::new(body, url.clone()),
                type_: type_,
            }),
            url: url,
        }
    ...
    pub fn detect(response: Response<Body>, url: ServoUrl) -> Response<Decoder> {
        let values = response
            .headers()
        ...
        match decoder {
            Some(type_) => response.map(|r| Decoder::pending(r, type_, url)),
            None => response.map(|r| Decoder::plain_text(r, url)),
        }
    }
}
...
impl Future for Pending {
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        let url = self.body.url.clone();
        let body = mem::replace(&mut self.body, ReadableChunks::new(Body::empty(), url));
...
impl Gzip {
    fn new(stream: ReadableChunks<Body>) -> Self {
        ...
        Gzip {
            ...
            inner: Box::new(gzip::Decoder::new(Peeked::new(stream, url.clone()))),
            url: url,
        }
    }
}
...
impl Stream for Gzip {
    ...
    fn poll(&mut self) -> Poll<Option<Self::Item>, Self::Error> {
        poll_with_read(&mut self.inner, &mut self.buf, &self.url)
    }
}
...
impl Brotli {
    fn new(stream: ReadableChunks<Body>) -> Self {
        ...
        Self {
            buf: BytesMut::with_capacity(INIT_BUFFER_SIZE),
            inner: Box::new(Decompressor::new(
                Peeked::new(stream, url.clone()),
                BUF_SIZE,
            )),
            url: url,
        }
...
impl Deflate {
    fn new(stream: ReadableChunks<Body>) -> Self {
        ...
        Self {
            buf: BytesMut::with_capacity(INIT_BUFFER_SIZE),
            inner: Box::new(DeflateDecoder::new(Peeked::new(stream, url.clone()))),
            url: url,
        }
    }
}
...
impl<R> Peeked<R> {
    #[inline]
    fn new(inner: R, url: ServoUrl) -> Self {
        Peeked {
...
impl<S> ReadableChunks<S> {
    #[inline]
    fn new(stream: S, url: ServoUrl) -> Self {
        ReadableChunks {
