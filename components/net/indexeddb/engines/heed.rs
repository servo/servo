...
impl HeedEngine {
    pub fn new(base_dir: &Path, db_file_name: &Path) -> Self {
        ...
        let heed_handle = Manager::singleton()
            .write()
            .expect("Could not get write lock")
            .get_or_create(db_dir.as_path(), Heed::new)
            .expect("Could not create database with this origin");

        // TODO: What is a reasonable number of threads?
        Self {
            heed_handle,
...
impl KvsEngine for HeedEngine {
    fn create_store(&self, store_name: SanitizedName, auto_increment: bool) {
        let heed = self.heed_handle.read().unwrap();
        let new_store = heed
    ...
    fn process_transaction(
        ...
        let db_handle = self.heed_handle.clone();
                    ...
                    match request.operation {
                        AsyncOperation::PutItem(key, value, overwrite) => {
                            ...
                            if overwrite {
                                ...
                                // FIXME:(arihant2math) We should be able to set some flags in
                                // `heed` in order to do this without running a get request first
                                if store
                                    .inner
