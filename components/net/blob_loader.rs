...
pub fn load_blob(load_data: &LoadData, start_chan: LoadConsumer,
    ...
    let (start, end) = compute_slice_pos(Some(rel_pos.start), rel_pos.end, entry.size as usize);
    let new_size = end - start;

    headers.set(ContentType(content_type.clone()));
    headers.set(ContentLength(new_size as u64));
    ...
    if let Ok(chan) =
        start_sending_sniffed_opt(start_chan, metadata, classifier,
                                  &entry.bytes[start..end], load_data.context.clone()) {
