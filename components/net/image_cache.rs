...
impl CompletedLoad {
    fn new(image_response: ImageResponse, id: PendingImageId, final_url: Option<ServoUrl>) -> CompletedLoad {
        CompletedLoad {
...
impl ImageCacheStore {
    ...
    fn complete_load(&mut self, key: LoadKey, mut load_result: LoadResult) {
        ...
        let completed_load = CompletedLoad::new(image_response.clone(), key, Some(url));
            ...
            match result {
                CacheResult::Hit(key, pl) => match (&pl.result, &pl.metadata) {
                    ...
                    (&None, &Some(ref meta)) => {
                        ...
                        return Ok(ImageOrMetadataAvailable::MetadataAvailable(meta.clone(), url.clone()))
                    }
    ...
    fn add_listener(&self, id: PendingImageId, listener: ImageResponder) {
        ...
        if let Some(load) = store.pending_loads.get_by_key_mut(&id) {
            if let Some(ref metadata) = load.metadata {
                listener.respond(ImageResponse::MetadataLoaded(metadata.clone(), load.url.clone()));
            }
    ...
    fn notify_pending_response(&self, id: PendingImageId, action: FetchResponseMsg) {
        match (action, id) {
            ...
            (FetchResponseMsg::ProcessResponse(_response), _) => {
                ...
                let final_url = metadata.unwrap().final_url;
                pending_load.url = final_url;
            }
            (FetchResponseMsg::ProcessResponseChunk(data), _) => {
                ...
                if let None = pending_load.metadata {
                    if let Ok(metadata) = load_from_buf(&pending_load.bytes.as_slice()) {
                        ...
                        for listener in &pending_load.listeners {
                            listener.respond(
                                ImageResponse::MetadataLoaded(img_metadata.clone(), pending_load.url.clone()));
                        }
