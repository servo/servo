...
impl MIMEClassifier {
    ...
    fn is_xml(tp: &str, sub_tp: &str) -> bool {
        let suffix = &sub_tp[(max((sub_tp.len() as isize) - ("+xml".len() as isize), 0isize) as usize)..];
...
impl ByteMatcher {
    fn matches(&self, data: &Vec<u8>) -> Option<usize> {
        ...
        for j in range(0usize,self.pattern.len()) {
...
impl Mp4Matcher {
    fn matches(&self,data: &Vec<u8>) -> bool {
        ...
        for i in range(4usize,8usize) {
            if data[i] != ftyp[i - 4] {
                return false;
            }
        }
        let mut all_match = true;
        for i in range(8usize,11usize) {
            if data[i]!=mp4[i - 8usize] {
                all_match = false;
                break;
            }
        }
        ...
        let mut bytes_read = 16usize;

        while bytes_read < box_size {
            all_match = true;
            for i in range(0usize,3usize) {
