...
pub fn decode(url: Url) -> Result<DecodeData, DecodeError> {
    ...
    let mut scheme_data = match url.scheme_data {
        ...
        _ => return Err(DecodeError::NonRelativeScheme),
    };
    match url.query {
        Some(ref query) => {
...
pub fn load(load_data: LoadData,
    ...
    match decode(url.clone()) {
        ...
        Err(err_type) => { match err_type {
            DecodeError::InvalidDataUri => send_error(url, "invalid data uri".to_owned(), start_chan),
            DecodeError::NonRelativeScheme => send_error(url, "non-relative scheme URL".to_owned(), start_chan),
            DecodeError::NonBase64DataUri => send_error(url, "non-base64 data uri".to_owned(), start_chan),
            }
        }
    }
}
