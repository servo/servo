...
pub fn load(load_data: LoadData,
    ...
    if parts.len() != 2 {
        send_error(LoadError::DataURI(url, "invalid data uri".to_owned()), start_chan);
        return
    }
    ...
    let bytes = if is_base64 {
        ...
        match bytes.from_base64() {
            Err(..) => return send_error(LoadError::DataURI(url, "non-base64 data uri".to_owned()), start_chan),
            Ok(data) => data,
        }
    } else {
        bytes
