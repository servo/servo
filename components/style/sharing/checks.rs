...
use sharing::ElementSharingChecker;
...
pub fn have_same_presentational_hints<E>(
    ...
    where E: TElement,
{
    element.pres_hints() == candidate.pres_hints()
}
...
pub fn have_same_class<E>(
    ...
    where E: TElement,
{
    element.class_list() == candidate.class_list()
}
...
#[inline]
pub fn revalidate<E>(checker: &mut ElementSharingChecker<E>,
                     candidate: &mut StyleSharingCandidate<E>,
    ...
    where E: TElement,
{
    ...
    let for_element =
        checker.revalidation_match_results(stylist, bloom, selector_flags_map);
