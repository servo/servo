...
pub trait TNode<'ln> : Sized + Copy + Clone {
    ...
    /// Returns true if the style and layout data has been initialized.
    ...
    #[inline(always)]
    unsafe fn borrow_data_unchecked(&self) -> *const PrivateStyleData;

    unsafe fn maybe_borrow_data_unchecked(&self) -> Option<*const PrivateStyleData> {
        if self.has_data() {
            Some(self.borrow_data_unchecked())
        } else {
            None
        }
    }
    ...
    #[inline(always)]
    fn borrow_data(&self) -> Ref<PrivateStyleData>;

    fn maybe_borrow_data(&self) -> Option<Ref<PrivateStyleData>> {
        if self.has_data() {
            Some(self.borrow_data())
        } else {
            None
        }
    }
    ...
    #[inline(always)]
    fn mutate_data(&self) -> RefMut<PrivateStyleData>;
