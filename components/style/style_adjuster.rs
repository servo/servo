...
use properties::longhands::overflow_y::computed_value as overflow_y;
...
impl<'a> StyleAdjuster<'a> {
    ...
    fn adjust_for_contain(&mut self) {
        ...
        if self.style.get_box().clone_contain().contains(contain::PAINT) {
            if self.style.get_box().clone_overflow_x() == overflow::visible {
                ...
                box_style.set_overflow_x(overflow::clip);
                box_style.set_overflow_y(overflow_y::T(overflow::clip));
            }
        }
    }
    ...
    fn adjust_for_overflow(&mut self) {
        ...
        let original_overflow_y = self.style.get_box().clone_overflow_y().0;
