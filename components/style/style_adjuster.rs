...
impl<'a, 'b: 'a> StyleAdjuster<'a, 'b> {
    ...
    /// Whether we should skip display item-based display fixups.
    fn skip_item_based_display_fixup(&self) -> bool {
        // Pseudos never get their display fixed up, except ::before or ::after.
        self.style.pseudo.as_ref().map_or(false, |p| !p.is_before_or_after())
    }
