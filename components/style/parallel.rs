          ...
          D: DomTraversal<N>,
{
    let mut discovered_child_nodes = vec![];
    {
        ...
        if tls_borrow.is_none() {
            *tls_borrow = Some(traversal.create_thread_local_context());
        }
        let tlc = tls_borrow.as_mut().unwrap();
        ...
        for unsafe_node in unsafe_nodes {
            ...
            traversal.process_preorder(&mut traversal_data, tlc, node);
            ...
            if D::needs_postorder_traversal() {
                if children_to_process == 0 {
                    ...
                    bottom_up_dom(traversal, tlc, root, *unsafe_node)
                } else {
