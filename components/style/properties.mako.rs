...
pub fn cascade(viewport_size: Size2D<Au>,
    ...
    if positioned || floated || is_root_element {
        ...
        let specified_display = context.style.box_.display;
        match specified_display {
            T::inline_table => {
                Arc::make_mut(&mut context.style.box_)._servo_display_for_hypothetical_box =
                    longhands::_servo_display_for_hypothetical_box::derive_from_display(
                            specified_display,
                            &context);
                Arc::make_mut(&mut context.style.box_).display = T::table;
            }
            ...
            T::table_caption => {
                Arc::make_mut(&mut context.style.box_)._servo_display_for_hypothetical_box =
                    longhands::_servo_display_for_hypothetical_box::derive_from_display(
                            specified_display,
                            &context);
                Arc::make_mut(&mut context.style.box_).display = T::block;
            }
            _ => {}
        }
    }
