...
//            }
    ...
    <%self:longhand name="transform-origin">
        ...
        pub fn parse(context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue,()> {
            let result = super::parse_origin(context, input);
            match result {
                Ok(value) => {
                    Ok(SpecifiedValue {
                        horizontal: value.horizontal.unwrap_or(LengthOrPercentage::Percentage(0.5)),
                        vertical: value.vertical.unwrap_or(LengthOrPercentage::Percentage(0.5)),
                        depth: value.depth.unwrap_or(Length::Absolute(Au(0))),
                    })
                },
                _ => Err(())
            }
