                ...
                % for value in values[:-1]:
                    "${value}" => {
                        % if value in experimental_values:
                            if !::util::prefs::get_pref("layout.${value}.enabled").and_then(|x| x.as_boolean()).unwrap_or(false) {
                                return Err(())
                            }
                        % endif
                ...
                % if property.derived_from is None:
                    "${property.name}" => {
                        % if property.experimental:
                            if !::util::prefs::get_pref("${property.experimental}").and_then(|x| x.as_boolean()).unwrap_or(false) {
            ...
            % for shorthand in SHORTHANDS:
                "${shorthand.name}" => {
                    % if shorthand.experimental:
                        if !::util::prefs::get_pref("${shorthand.experimental}").and_then(|x| x.as_boolean()).unwrap_or(false) {
                            return PropertyDeclarationParseResult::ExperimentalProperty
                        }
                    % endif
                    match input.try(CSSWideKeyword::parse) {
