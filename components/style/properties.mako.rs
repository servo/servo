...
pub mod longhands {
    ...
    //            }
                ...
                let floated = context.style.box_.float != longhands::float::SpecifiedValue::none;
                if positioned || floated || context.is_root_element {
                    match *self {
                            ...
                            PropertyDeclaration::${property.camel_case}(ref
                                ...
                                % if property.name in DERIVED_LONGHANDS:
                                    % if not style_struct.inherited:
                                        ...
                                        let computed_value = context.style.${style_struct.ident}
...
pub fn cascade(viewport_size: Size2D<Au>,
    ...
    if !seen.get_display() {
        let computed_value = context.style.box_.display.to_computed_value(&context);
        Arc::make_mut(&mut context.style.box_).display = computed_value;
        Arc::make_mut(&mut context.style.box_)._servo_display_for_hypothetical_box =
            longhands::_servo_display_for_hypothetical_box::derive_from_display(
                    computed_value,
                    &context);
