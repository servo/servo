...
pub fn take_thread_local_bloom_filter<E>(parent_element: Option<E>,
    ...
    STYLE_BLOOM.with(|style_bloom| {
        match (parent_element, style_bloom.borrow_mut().take()) {
            ...
            (None,     _  ) => {
                trace!("bloom: [{}] No parent, but new bloom filter!", tid());
            ...
            (Some(parent), Some((mut bloom_filter, old_node, old_generation))) => {
                if old_node == parent.as_node().to_unsafe() &&
                    ...
                    trace!("bloom: [{}] Parent matches (={}). Reusing bloom filter.", tid(), old_node.0);
                } else {
...
fn insert_ancestors_into_bloom_filter<E>(bf: &mut Box<BloomFilter>,
                                         ...
                                         root: OpaqueNode)
                                         where E: TElement {
    trace!("bloom: [{}] Inserting ancestors.", tid());
    let mut ancestors = 0;
    loop {
        ancestors += 1;
    ...
    trace!("bloom: [{}] Inserted {} ancestors.", tid(), ancestors);
}
          ...
          C: StyleContext<'a>
{
    ...
    match node.layout_parent_element(root) {
        None => {
            trace!("bloom: [{}] - {:X}, and deleting BF.", tid(), unsafe_layout_node.0);
          ...
          D: DomTraversalContext<E::ConcreteNode>
{
    ...
    // filter.
    trace!("bloom: [{}] + {:X}", tid(), unsafe_layout_node.0);
