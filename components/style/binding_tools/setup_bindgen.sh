...
if [[ -x "$(command -v multirust)" ]]; then
  echo "Overriding multirust to use nightly"
  multirust override nightly
elif [[ -x "$(command -v rustup)" ]]; then
  echo "Overriding rustup to use nightly"
  rustup override set nightly
else
  echo "Couldn't locate rustup or multirust." \
       "Please make sure that you're using nightly Rust."
