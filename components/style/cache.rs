...
impl<T, A: Array<Item=Entry<T>>> LRUCache<T, A> {
    ...
    pub fn touch(&mut self, pos: CacheIndex) {
        let pos = pos.0;
        if pos == self.head {
            return
        }
        ...
        let prev = self.entries[pos as usize].prev;
        let next = self.entries[pos as usize].next;
        if pos != self.head {
    ...
    /// used.
    pub fn iter(&mut self) -> LRUCacheIterator<T, A> {
        LRUCacheIterator {
            pos: self.head,
    ...
    pub fn insert(&mut self, val: T) {
        let entry = Entry { val, prev: 0, next: self.head };
    ...
    ///
    /// Note: This only unlinks the entry from the linked list; it does not remove it from the
    /// `entries` array.
