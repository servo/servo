                              ...
                              -> RestyleDamage
    {
        match self.existing_style_for_restyle_damage(old_style, pseudo) {
            ...
            None => {
                ...
                //
                //     Also, note that some styled pseudo-elements may not have
                //     a frame even when the display value is other than none,
                //     so we need to actually compare both values here.
                //
                if let Some(old_style) = old_style {
                    debug_assert!(pseudo.is_some() ||
                                  old_style.get_box().clone_display() == display::T::none,
                                  "Should've reached the frame!");
