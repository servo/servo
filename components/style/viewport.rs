...
use values::specified::{Length, LengthInternal, LengthOrPercentageOrAuto, ViewportPercentageLength};
    ...
    fn from_meta(value: &str) -> Option<ViewportLength> {
        ...
        Some(match value {
            v if v.eq_ignore_ascii_case("device-width") =>
                specified!(Length::Inner(
                    LengthInternal::ViewportPercentage(ViewportPercentageLength::Vw(100.)))),
            v if v.eq_ignore_ascii_case("device-height") =>
                specified!(Length::Inner(
                    LengthInternal::ViewportPercentage(ViewportPercentageLength::Vh(100.)))),
            _ => {
