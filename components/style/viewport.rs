...
// FIXME: replace this with std::intrinsics::discriminant_value after the Rust upgrade
...
impl fmt::Display for ViewportConstraints {
    #[inline]
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.fmt_to_css(f)
    }
...
impl ViewportConstraints {
                     ...
                     -> Option<ViewportConstraints>
    {
        ...
        let initial_viewport = Size2D(Au::from_frac32_px(initial_viewport.width.get()),
                                      Au::from_frac32_px(initial_viewport.height.get()));
        ...
        let width = width.unwrap_or_else(|| match initial_viewport.height {
            ...
            initial_height => {
                let ratio = initial_viewport.width.to_frac32_px() / initial_height.to_frac32_px();
                Au::from_frac32_px(height.clone().unwrap().to_frac32_px() * ratio)
            }
        });
        ...
        let height = height.unwrap_or_else(|| match initial_viewport.width {
            ...
            initial_width => {
                let ratio = initial_viewport.height.to_frac32_px() / initial_width.to_frac32_px();
                Au::from_frac32_px(width.to_frac32_px() * ratio)
            }
        });

        Some(ViewportConstraints {
            size: TypedSize2D(width.to_frac32_px(), height.to_frac32_px()),
