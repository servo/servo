...
/// selectors. Doing this conservatively is expensive, and so we RestyleHints to
...
pub struct ElementSnapshot {
    ...
    pub attrs: Option<Vec<(AttrName, AttrValue)>>,
}

impl ElementSnapshot {
    pub fn new() -> ElementSnapshot {
        ElementSnapshot { state: None, attrs: None }
    }
    ...
    pub fn get_attr(&self, namespace: &Namespace, name: &Atom) -> Option<&AttrValue> {
        self.attrs.as_ref().unwrap().iter()
                  .find(|&&(ref n, _)| { n.local_name == *name && n.namespace == *namespace })
                  .map(|&(_, ref v)| v)
    }
    ...
    pub fn get_attr_ignore_ns(&self, name: &Atom) -> Option<&AttrValue> {
        self.attrs.as_ref().unwrap().iter()
                  .find(|&&(ref n, _)| { n.local_name == *name })
                  .map(|&(_, ref v)| v)
    }
}
...
impl<'a, E> Element for ElementWrapper<'a, E> where E: Element {
    ...
    fn each_class<F>(&self, mut callback: F) where F: FnMut(&Atom) {
        match self.snapshot.attrs {
            Some(_) => self.snapshot.get_attr(&ns!(""), &atom!("class"))
                                    .map_or((), |v| for c in v.as_tokens() { callback(c) }),
...
impl DependencySet {
    ...
    pub fn compute_hint<E>(&self, el: &E, snapshot: &ElementSnapshot, current_state: ElementState)
        ...
        let state_changes = current_state ^ snapshot.state.unwrap_or(current_state);
