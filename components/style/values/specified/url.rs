...
impl SpecifiedUrl {
    pub fn parse(context: &ParserContext, input: &mut Parser) -> Result<Self, ()> {
        // NB: Needs to be wrapped in input.try because this is called from
...
impl ToCss for SpecifiedUrl {
    fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
        ...
        } else if let Some(ref url) = self.resolved {
            url.to_css(dest)
        } else {
            // This can only happen if the url wasn't specified by the
            // user *and* it's an invalid url that has been transformed
            // back to specified value via the "uncompute"
            // functionality.
            dest.write_str("url(\"about:invalid\")")
        }
    }
}
