...
impl Parse for ClipRect {
    fn parse(context: &ParserContext, input: &mut Parser) -> Result<Self, ()> {
        Self::parse_quirky(context, input, AllowQuirks::Yes)
    }
}
...
impl ClipRectOrAuto {
    /// Parses a ClipRect or Auto, disallowing quirks.
    pub fn parse_non_quirky(context: &ParserContext, input: &mut Parser) -> Result<Self, ()> {
        if let Ok(v) = input.try(|i| ClipRect::parse_quirky(context, i, AllowQuirks::No)) {
            Ok(Either::First(v))
        } else {
