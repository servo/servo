...
impl Position {
    pub fn new(mut first_position: Option<PositionComponent>, mut second_position: Option<PositionComponent>,
        ...
        let (horiz_keyword, vert_keyword) = match (category(first_key), category(second_key)) {
            ...
            // FIXME: Don't allow logical keywords for Position. They are not in current spec yet.
...
impl Parse for HorizontalPosition {
    ...
    fn parse(context: &ParserContext, input: &mut Parser) -> Result<Self, ()> {
        ...
        let position = match position {
            Some(PositionComponent::Length(pos)) => Some(pos),
            Some(_) => return Err(()),
            None => None,
        };
...
impl ToComputedValue for HorizontalPosition {
    ...
    fn to_computed_value(&self, context: &Context) -> computed_position::HorizontalPosition {
        ...
        let horizontal = match keyword {
            Keyword::Right | Keyword::XEnd => { // TODO: What to do here?
...
impl Keyword {
    pub fn to_length_or_percentage(self) -> LengthOrPercentage {
        match self {
            ...
            // FIXME: What to do with logical keywords?
