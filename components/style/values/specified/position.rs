...
/// Single template area
pub struct TemplateAreas {
    /// `named area` containing for each single template area
    pub areas: Box<[NamedArea]>,
    /// The original CSS string value of each single template area
    pub strings: Box<[Box<str>]>,
    /// width of each single template area
    pub width: u32,
}

impl TemplateAreas {
    /// Transform `vector` of str into single `template area`
