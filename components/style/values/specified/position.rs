...
impl PositionValue<LengthOrPercentage> {
    /// Generic function for the computed value of a position. It takes another function
    /// `other_side_keyword` which checks whether the keyword contained (if any) belongs to the opposite
    /// side (`bottom` for `top`, and so on), so that we can generate the relative `length`` value.
    fn computed_value<F>(&self, context: &Context, other_side_keyword: F) -> ComputedLengthOrPercentage
        where F: FnOnce(Keyword) -> bool
    {
        match self.keyword {
            Some(Keyword::Center) => ComputedLengthOrPercentage::Percentage(0.5),
            Some(k) if other_side_keyword(k) => match self.position {
                Some(ref x) => {
                ...
                None => ComputedLengthOrPercentage::Percentage(0.0),
            },
...
impl ToComputedValue for HorizontalPosition {
    ...
    fn to_computed_value(&self, context: &Context) -> computed_position::HorizontalPosition {
        GenericHorizontalPosition(self.0.computed_value(context, |keyword| {
            match keyword {
                Keyword::Right | Keyword::XEnd => true,
                _ => false,
            }
        }))
    }
