...
pub struct TrackList<L> {
    ...
    /// given by `auto_repeat_idx`
    pub values_list: Vec<(Vec<String>, Either<TrackSize<L>, TrackRepeat<L>>)>,
    /// If it exists, the value at this index in the list above, is the `<line-names>?`
    /// followed by `<auto-repeat>`
    pub auto_repeat_idx: Option<u32>,
...
impl Parse for TrackList<LengthOrPercentage> {
    fn parse(context: &ParserContext, input: &mut Parser) -> Result<Self, ()> {
        ...
        loop {
            ...
            } else if let Ok(track_repeat) = input.try(|i| TrackRepeat::parse(context, i)) {
                if list_type == TrackListType::Explicit {
                    list_type = TrackListType::Normal;      // <explicit-track-list> doesn't contain repeat()
                }

                match track_repeat.repeat_type {
                    TrackRepeatType::Normal => {
                        atleast_one_not_fixed = true;
                    ...
                    TrackRepeatType::Auto => {
                        ...
                        is_auto = true;
                        list_type = TrackListType::Auto;
                        auto_repeat_idx = Some(values.len() as u32);
                    },
                    TrackRepeatType::Fixed => (),
                }

                values.push((current_names, Either::Second(track_repeat)));
            } else {
                if values.is_empty() {
                    return Err(())
                }
        ...
        Ok(TrackList {
            list_type: list_type,
            values_list: values,
            auto_repeat_idx: auto_repeat_idx,
...
impl<L: ToComputedValue> ToComputedValue for TrackList<L> {
    ...
    fn to_computed_value(&self, context: &Context) -> Self::ComputedValue {
        TrackList {
            ...
            auto_repeat_idx: self.auto_repeat_idx,
            line_names_last: self.line_names_last.clone(),
        }
    }
    ...
    fn from_computed_value(computed: &Self::ComputedValue) -> Self {
        TrackList {
            ...
            auto_repeat_idx: computed.auto_repeat_idx,
            line_names_last: computed.line_names_last.clone(),
        }
    }
}
