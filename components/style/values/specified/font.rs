...
impl ToCss for FontLanguageOverride {
    fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
        match *self {
            FontLanguageOverride::Normal => dest.write_str("normal"),
            FontLanguageOverride::Override(ref lang) => lang.to_css(dest),
            FontLanguageOverride::System(sys) => sys.to_css(dest),
        }
    }
}
...
impl Parse for FontLanguageOverride {
    ...
    fn parse<'i, 't>(_: &ParserContext, input: &mut Parser<'i, 't>) -> Result<FontLanguageOverride, ParseError<'i>> {
        if input.try(|input| input.expect_ident_matching("normal")).is_ok() {
            Ok(FontLanguageOverride::Normal)
        } else {
            input.expect_string().map(|s| {
                FontLanguageOverride::Override(s.as_ref().to_owned())
            }).map_err(|e| e.into())
        }
    }
}
