...
use values::specified::{AllowQuirks, LengthOrPercentage, NoCalcLength, Number as SpecifiedNumber};
...
impl ToComputedValue for FontSizeAdjust {
    ...
    fn from_computed_value(computed: &computed::FontSizeAdjust) -> Self {
        match *computed {
            ...
            computed::FontSizeAdjust::Number(ref v) => FontSizeAdjust::Number(SpecifiedNumber::from_computed_value(v)),
        }
    }
}
...
impl Parse for FontSizeAdjust {
    ...
    fn parse<'i, 't>(context: &ParserContext, input: &mut Parser<'i, 't>) -> Result<FontSizeAdjust, ParseError<'i>> {
        ...
        Ok(FontSizeAdjust::Number(SpecifiedNumber::parse_non_negative(context, input)?))
    }
}
