...
/// A length that cannot parse `calc` expressions
#[derive(Clone, PartialEq, Copy, Debug)]
#[cfg_attr(feature = "servo", derive(HeapSizeOf))]
pub enum LengthInternal {
...
impl HasViewportPercentage for LengthInternal {
    fn has_viewport_percentage(&self) -> bool {
        match *self {
            LengthInternal::ViewportPercentage(_) => true,
            _ => false,
        }
    }
}

impl ToCss for LengthInternal {
    fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
        match *self {
            LengthInternal::Absolute(length) => write!(dest, "{}px", length.to_f32_px()),
            LengthInternal::FontRelative(length) => length.to_css(dest),
            LengthInternal::ViewportPercentage(length) => length.to_css(dest),
            /* This should only be reached from style dumping code */
            LengthInternal::ServoCharacterWidth(CharacterWidth(i)) => write!(dest, "CharWidth({})", i),
        }
    }
}

impl Mul<CSSFloat> for LengthInternal {
    type Output = LengthInternal;

    #[inline]
    fn mul(self, scalar: CSSFloat) -> LengthInternal {
        match self {
            LengthInternal::Absolute(Au(v)) => LengthInternal::Absolute(Au(((v as f32) * scalar) as i32)),
            LengthInternal::FontRelative(v) => LengthInternal::FontRelative(v * scalar),
            LengthInternal::ViewportPercentage(v) => LengthInternal::ViewportPercentage(v * scalar),
            LengthInternal::ServoCharacterWidth(_) => panic!("Can't multiply ServoCharacterWidth!"),
        }
    }
}
...
impl LengthInternal {
    ...
    pub fn from_str(s: &str) -> Option<LengthInternal> {
        Some(match_ignore_ascii_case! { s,
            "xx-small" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX) * 3 / 5),
            "x-small" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX) * 3 / 4),
            "small" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX) * 8 / 9),
            "medium" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX)),
            "large" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX) * 6 / 5),
            "x-large" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX) * 3 / 2),
            "xx-large" => LengthInternal::Absolute(Au::from_px(FONT_MEDIUM_PX) * 2),

            // https://github.com/servo/servo/issues/3423#issuecomment-56321664
            "smaller" => LengthInternal::FontRelative(FontRelativeLength::Em(0.85)),
            "larger" => LengthInternal::FontRelative(FontRelativeLength::Em(1.2)),
            _ => return None
        })
    }

    /// Parse a given absolute or relative dimension.
    pub fn parse_dimension(value: CSSFloat, unit: &str) -> Result<LengthInternal, ()> {
        match_ignore_ascii_case! { unit,
            "px" => Ok(LengthInternal::Absolute(Au((value * AU_PER_PX) as i32))),
            "in" => Ok(LengthInternal::Absolute(Au((value * AU_PER_IN) as i32))),
            "cm" => Ok(LengthInternal::Absolute(Au((value * AU_PER_CM) as i32))),
            "mm" => Ok(LengthInternal::Absolute(Au((value * AU_PER_MM) as i32))),
            "q" => Ok(LengthInternal::Absolute(Au((value * AU_PER_Q) as i32))),
            "pt" => Ok(LengthInternal::Absolute(Au((value * AU_PER_PT) as i32))),
            "pc" => Ok(LengthInternal::Absolute(Au((value * AU_PER_PC) as i32))),
            // font-relative
            "em" => Ok(LengthInternal::FontRelative(FontRelativeLength::Em(value))),
            "ex" => Ok(LengthInternal::FontRelative(FontRelativeLength::Ex(value))),
            "ch" => Ok(LengthInternal::FontRelative(FontRelativeLength::Ch(value))),
            "rem" => Ok(LengthInternal::FontRelative(FontRelativeLength::Rem(value))),
            // viewport percentages
            "vw" => Ok(LengthInternal::ViewportPercentage(ViewportPercentageLength::Vw(value))),
            "vh" => Ok(LengthInternal::ViewportPercentage(ViewportPercentageLength::Vh(value))),
            "vmin" => Ok(LengthInternal::ViewportPercentage(ViewportPercentageLength::Vmin(value))),
            "vmax" => Ok(LengthInternal::ViewportPercentage(ViewportPercentageLength::Vmax(value))),
            _ => Err(())
        }
    }
}
...
#[cfg_attr(feature = "servo", derive(HeapSizeOf))]
pub enum Length {
    /// The `LengthInternal` type that cannot parse `calc`
    Inner(LengthInternal),
    /// A calc expression.
    ///
...
impl Default for Length {
    fn default() -> Length {
        Length::Inner(LengthInternal::Absolute(Au(0)))
    }
}
...
impl HasViewportPercentage for Length {
    fn has_viewport_percentage(&self) -> bool {
        match *self {
            Length::Inner(ref inner) => inner.has_viewport_percentage(),
...
impl ToCss for Length {
    fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
        match *self {
            Length::Inner(ref inner) => inner.to_css(dest),
...
impl Mul<CSSFloat> for Length {
    ...
    fn mul(self, scalar: CSSFloat) -> Length {
        match self {
            Length::Inner(inner) => Length::Inner(inner * scalar),
...
impl Length {
    ...
    pub fn from_str(s: &str) -> Option<Length> {
        LengthInternal::from_str(s).map(Length::Inner)
    }

    /// Parse a given absolute or relative dimension.
    pub fn parse_dimension(value: CSSFloat, unit: &str) -> Result<Length, ()> {
        LengthInternal::parse_dimension(value, unit).map(Length::Inner)
    }
    ...
    pub fn from_px(px_value: CSSFloat) -> Length {
        Length::Inner(LengthInternal::Absolute(Au((px_value * AU_PER_PX) as i32)))
    }
    ...
    pub fn parse(input: &mut Parser,
        ...
        for value in simplified {
            match value {
                ...
                SimplifiedValueNode::Length(LengthInternal::Absolute(Au(au))) =>
                    absolute = Some(absolute.unwrap_or(0) + au),
                SimplifiedValueNode::Length(LengthInternal::ViewportPercentage(v)) =>
                    match v {
                ...
                SimplifiedValueNode::Length(LengthInternal::FontRelative(f)) =>
                    match f {
...
impl LengthOrPercentage {
                      ...
                      -> Result<LengthOrPercentage, ()>
    {
        match try!(input.next()) {
            ...
            Token::Number(ref value) if value.value == 0. =>
                Ok(LengthOrPercentage::Length(Length::default())),
            Token::Function(ref name) if name.eq_ignore_ascii_case("calc") => {
    ...
    #[inline]
    pub fn take(&mut self) -> Self {
        let new = LengthOrPercentage::Length(Length::default());
        mem::replace(self, new)
    }
}
...
impl LengthOrPercentageOrAuto {
                      ...
                      -> Result<LengthOrPercentageOrAuto, ()>
    {
        match try!(input.next()) {
            ...
            Token::Number(ref value) if value.value == 0. =>
                Ok(LengthOrPercentageOrAuto::Length(Length::default())),
...
impl LengthOrPercentageOrNone {
                      ...
                      -> Result<LengthOrPercentageOrNone, ()>
    {
        match try!(input.next()) {
            ...
            Token::Number(ref value) if value.value == 0. =>
                Ok(LengthOrPercentageOrNone::Length(Length::default())),
            Token::Function(ref name) if name.eq_ignore_ascii_case("calc") => {
                let calc = try!(input.parse_nested_block(CalcLengthOrPercentage::parse_length_or_percentage));
...
impl Parse for LengthOrPercentageOrAutoOrContent {
    fn parse(_context: &ParserContext, input: &mut Parser) -> Result<Self, ()> {
        ...
        match try!(input.next()) {
            ...
            Token::Number(ref value) if value.value == 0. =>
                Ok(LengthOrPercentageOrAutoOrContent::Length(Length::default())),
            Token::Ident(ref value) if value.eq_ignore_ascii_case("auto") =>
