...
impl ToCss for AbsoluteLength {
    fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
        match *self {
            AbsoluteLength::Au(length) => write!(dest, "{}px", length.to_f32_px()),
...
impl Mul<CSSFloat> for AbsoluteLength {
    ...
    fn mul(self, scalar: CSSFloat) -> AbsoluteLength {
        match self {
            AbsoluteLength::Au(v) => AbsoluteLength::Au(Au(((v.0 as f32) * scalar) as i32)),
