...
impl GradientKind {
    ...
    pub fn parse_radial(input: &mut Parser) -> Result<GradientKind, ()> {
        ...
        let (shape, position) = if let Ok(position) = input.try(parse_position) {
            (EndingShape::Ellipse(LengthOrPercentageOrKeyword::Keyword(SizeKeyword::FarthestSide)), position)
            ...
            if let Ok(ident) = input.expect_ident() {
                match_ignore_ascii_case! { ident,
                    ...
                    _ => {
                        // If there is no indent, it should set to default.
                        // But also it should restore parser's internal state.
                        needs_comma = false;
...
fn parse_position(input: &mut Parser) -> Result<Position, ()> {
    if input.try(|input| input.expect_ident_matching("at")).is_ok() {
