    ...
    fn to_3d_matrix(&self, reference_box: Option<&Rect<Au>>) -> Result<Transform3D<f64>, ()> {
        ...
        let matrix = match *self {
            ...
            Skew(theta_x, theta_y) => {
                let theta_y = theta_y.map_or(0., |a| a.as_ref().radians64());
                Transform3D::create_skew(Radians::new(theta_x.as_ref().radians64()),
                                         Radians::new(theta_y))
            },
            SkewX(theta) => {
                Transform3D::create_skew(Radians::new(theta.as_ref().radians64()),
                                         Radians::new(0.))
            },
            SkewY(theta) => {
                Transform3D::create_skew(Radians::new(0.),
                                         Radians::new(theta.as_ref().radians64()))
            },
            Matrix3D(m) => m.into(),
            Matrix(m) => m.into(),
            PrefixedMatrix3D(_) | PrefixedMatrix(_) => {
                // `-moz-transform` is not implemented in Servo yet, and DOMMatrix doesn't support
                // this.
                unreachable!()
            },
