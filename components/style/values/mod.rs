...
use parser::{Parse, ParseWithContext, ParserContext};
...
macro_rules! define_keyword_type {
    ($name: ident, $css: expr) => {
        ...
        impl ParseWithContext for $name {
            fn parse(_: &::parser::ParserContext, input: &mut ::cssparser::Parser) -> Result<$name, ()> {
                input.expect_ident_matching($css).map(|_| $name)
            }
        }
...
impl<A: ParseWithContext, B: ParseWithContext> ParseWithContext for Either<A, B> {
