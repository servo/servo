...
enum NodeDataStyles {
    ...
    /// The field holds the previous style of the node. We represent it this
    /// way, rather than Previous(Option<NodeStyles>), to improve packing.
    ///
...
impl NodeData {
    ...
    pub fn previous_styles(&self) -> Option<&NodeStyles> {
        match self.styles {
            NodeDataStyles::EmptyPrevious => None,
            NodeDataStyles::Previous(ref s) => Some(s),
    ...
    pub fn previous_styles_mut(&mut self) -> Option<&mut NodeStyles> {
        match self.styles {
            NodeDataStyles::EmptyPrevious => None,
            NodeDataStyles::Previous(ref mut s) => Some(s),
            _ => panic!("Calling previous_styles without having gathered it"),
        }
    }
        ...
        where F: FnOnce() -> Option<NodeStyles>
    {
        ...
        self.styles = match mem::replace(&mut self.styles, Uninitialized) {
            Uninitialized => f().map_or(EmptyPrevious, |x| Previous(x)),
