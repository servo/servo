...
lazy_static! {
    ...
    pub static ref STYLE_THREAD_POOL: StyleThreadPool = {
        ...
        // Note: when using this pool, be sure to enable a breadth-first rayon
        // traversal. This causes the work queue to be always FIFO, rather than
        // FIFO for stealers and FILO for the owner (which is what rayon does
        // by default). This ensures that we process all the elements at a
        // given depth before proceeding to the next depth, which is important
        // for style sharing.
