...
<%helpers:longhand name="font-size" need_clone="True" animatable="True"
    ...
    use values::specified::{LengthOrPercentage, Length, LengthInternal, Percentage};
    ...
    impl ToComputedValue for SpecifiedValue {
        ...
        fn to_computed_value(&self, context: &Context) -> computed_value::T {
            match self.0 {
                LengthOrPercentage::Length(Length::Inner(LengthInternal::FontRelative(value))) => {
                    value.to_computed_value(context, /* use inherited */ true)
                }
                LengthOrPercentage::Length(Length::Inner(LengthInternal::ServoCharacterWidth(value))) => {
