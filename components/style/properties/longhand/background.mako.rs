        ...
        fn to_specified_value(computed: &computed_value::T, context: &UncomputeContext) -> Self {
            match *computed {
                ...
                computed_value::T(Some(ref image)) =>
                    SpecifiedValue(Some(ToComputedValue::to_specified_value(image, context))),
            }
        }
    }
            ...
            match *computed {
                computed_value::T::Explicit(ref size) => {
                    SpecifiedValue::Explicit(ExplicitSize {
                        width: ToComputedValue::to_specified_value(&size.width, context),
                        height: ToComputedValue::to_specified_value(&size.height, context),
                    })
                }
