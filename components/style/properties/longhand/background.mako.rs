...
<%helpers:vector_longhand name="background-repeat" animatable="False"
    ...
    impl ToComputedValue for SpecifiedValue {
        ...
        fn from_computed_value(computed: &computed_value::T) -> Self {
            match (computed.0, computed.1) {
                ...
                (horizontal, vertical) => {
                    if horizontal == vertical {
                        SpecifiedValue::Other(horizontal, None)
                    }
                    else {
                        SpecifiedValue::Other(horizontal, Some(vertical))
                    }
                },
            }
        }
    ...
    pub fn parse(_context: &ParserContext, input: &mut Parser) -> Result<SpecifiedValue, ()> {
        if input.try(|input| input.expect_ident_matching("repeat-x")).is_ok() {
            Ok(SpecifiedValue::RepeatX)
        }
        else if input.try(|input| input.expect_ident_matching("repeat-y")).is_ok() {
            Ok(SpecifiedValue::RepeatY)
        }
        else {
            let horizontal = try!(RepeatKeyword::parse(input));
