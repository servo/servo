    ...
    impl<'a> ToCss for LonghandsToSerialize<'a>  {
        fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
            serialize_radius_values(dest,
                &self._moz_outline_radius_topleft.0.clone(),
                &self._moz_outline_radius_topright.0.clone(),
                &self._moz_outline_radius_bottomright.0.clone(),
                &self._moz_outline_radius_bottomleft.0.clone(),
            )
        }
    }
</%helpers:shorthand>
