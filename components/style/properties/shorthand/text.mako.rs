...
<%helpers:shorthand name="text-decoration"
                    ...
                    ${' text-decoration-style text-decoration-color' if product != 'servo' or data.testing else ''}"
                    spec="https://drafts.csswg.org/css-text-decor/#propdef-text-decoration">

    % if product != "servo" or data.testing:
    ...
    impl<'a> ToCss for LonghandsToSerialize<'a>  {
        fn to_css<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
            ...
            % if product != "servo" or data.testing:
                if self.text_decoration_style != &text_decoration_style::get_initial_specified_value() {
                    dest.write_str(" ")?;
