...
<%helpers:shorthand name="background"
    ...
    impl<'a> LonghandsToSerialize<'a>  {
        fn to_css_declared<W>(&self, dest: &mut W) -> fmt::Result where W: fmt::Write {
            ...
            //  - color: it should appear zero or one times only.
            //  - size, origin and clip: they should have either zero or the same number
            //    of elements as image above (len).
            % for name in "image position_x position_y repeat attachment ".split():
            ...
            for i in 0..len {
                ...
                % for name in "image repeat attachment position_x position_y".split():
                    if let Some(${name}) = ${name} {
                        ...
                        // position_y is the last guaranteed property, so we don't necessarily
