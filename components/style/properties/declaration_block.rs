...
impl PropertyDeclarationBlock {
    ...
    pub fn new() -> Self {
        PropertyDeclarationBlock {
            ...
            important_declarations: SmallBitVec::new(),
    ...
    pub fn with_one(declaration: PropertyDeclaration, importance: Importance) -> Self {
        ...
        PropertyDeclarationBlock {
            ...
            important_declarations: SmallBitVec::from_elem(1, importance.important()),
            longhands: longhands,
        }
    }
    ...
    pub fn important_declarations(&self) -> &SmallBitVec {
    ...
    pub fn declaration_importance_iter(&self) -> DeclarationImportanceIterator {
        DeclarationImportanceIterator::new(&self.declarations, &self.important_declarations)
    }
    ...
    pub fn any_important(&self) -> bool {
        !self.important_declarations.all_false()
    }
    ...
    pub fn any_normal(&self) -> bool {
        !self.important_declarations.all_true()
    }
    ...
    pub fn get(&self, property: PropertyDeclarationId) -> Option<(&PropertyDeclaration, Importance)> {
        self.declarations.iter().position(|decl| decl.id() == property).map(|i| {
            ...
            let importance = if self.important_declarations.get(i as u32) {
        ...
        if !definitely_new {
            ...
            for (i, slot) in self.declarations.iter_mut().enumerate() {
                if slot.id() == declaration.id() {
                    let important = self.important_declarations.get(i as u32);
                    ...
                    if overwrite_more_important_and_reuse_slot {
                        *slot = declaration;
                        self.important_declarations.set(i as u32, importance.important());
                        return true;
                    }
            ...
            if let Some(index) = index_to_remove {
                ...
                self.important_declarations.remove(index as u32);
                self.declarations.push(declaration);
                self.important_declarations.push(importance.important());
                return true;
            }
        }
    ...
    pub fn set_importance(&mut self, property: &PropertyId, new_importance: Importance) -> bool {
        ...
        for (i, declaration) in self.declarations.iter().enumerate() {
            if declaration.id().is_or_is_longhand_of(property) {
                ...
                if self.important_declarations.get(i as u32) != is_important {
                    self.important_declarations.set(i as u32, is_important);
    ...
    pub fn remove_property(&mut self, property: &PropertyId) -> bool {
        ...
        let important_declarations = &mut self.important_declarations;
        let mut i = 0;
        self.declarations.retain(|declaration| {
            ...
            if remove {
                ...
                important_declarations.remove(i);
            } else {
                i += 1;
            }
            !remove
    ...
    pub fn from_animation_value_map(animation_value_map: &AnimationValueMap) -> Self {
        ...
        PropertyDeclarationBlock {
            ...
            important_declarations: SmallBitVec::from_elem(len as u32, false),
            longhands: longhands,
        }
    }
