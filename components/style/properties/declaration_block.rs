...
impl<'a, 'b, 'i> DeclarationParser<'i> for PropertyDeclarationParser<'a, 'b> {
    ...
    fn parse_value<'t>(&mut self, name: CompactCowStr<'i>, input: &mut Parser<'i, 't>)
        ...
        let id = match PropertyId::parse(&name) {
            Ok(id) => id,
            Err(()) if is_non_mozilla_vendor_identifier(&name) =>
                return Err(PropertyDeclarationParseError::UnknownVendorProperty.into()),
            Err(()) => return Err(PropertyDeclarationParseError::UnknownProperty(name).into()),
        };
