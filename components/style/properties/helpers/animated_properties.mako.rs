    ...
    fn to_animated_zero(&self) -> Result<Self, ()> {
        match *self {
            ...
            TransformOperation::MatrixWithPercents(..) => {
                // FIXME(nox): This should be a `MatrixWithPercents`.
                Err(())
            },
            TransformOperation::Skew(..) => {
                Ok(TransformOperation::Skew(Angle::zero(), Angle::zero()))
            },
            ...
            TransformOperation::Rotate(..) => {
                Ok(TransformOperation::Rotate(0.0, 0.0, 1.0, Angle::zero()))
            },
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        match (self, other) {
            ...
            (
                ...
                let ix = fx.animate(&tx, procedure)?;
                let iy = fy.animate(&ty, procedure)?;
                Ok(TransformOperation::Skew(ix, iy))
            }
            (
                ...
                let ix = fx.animate(&tx, procedure)?;
                let iy = fy.animate(&ty, procedure)?;
                let iz = fz.animate(&tz, procedure)?;
                Ok(TransformOperation::Translate(ix, iy, iz))
            }
            (
                &TransformOperation::Scale(fx, fy, fz),
                &TransformOperation::Scale(tx, ty, tz),
            ) => {
                let ix = animate_scale_factors(fx, tx, procedure)?;
                let iy = animate_scale_factors(fy, ty, procedure)?;
                let iz = animate_scale_factors(fz, tz, procedure)?;
                Ok(TransformOperation::Scale(ix, iy, iz))
            }
            (
                ...
                if fx == tx && fy == ty && fz == tz {
                    ...
                    let sum = matrix_f.animate(&matrix_t, procedure)?;
                    Ok(TransformOperation::Matrix(sum))
                }
            }
            (
                ...
                let sum = fd_matrix.animate(&td_matrix, procedure)?;
                Ok(TransformOperation::Matrix(sum))
            }
            _ => Err(()),
        }
...
impl Animate for TransformList {
    #[inline]
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        let this = self.0.as_ref().map_or(&[][..], |this| this);
        let other = other.0.as_ref().map_or(&[][..], |other| other);

        fn to_transform_list<T>(list: T) -> TransformList
        where
            T: AsRef<[TransformOperation]> + Into<Vec<TransformOperation>>,
        {
            TransformList(if list.as_ref().is_empty() {
                None
            } else {
                Some(list.into())
            })
        ...
        if procedure == Procedure::Add {
            return Ok(to_transform_list(
                this.iter().chain(other).cloned().collect::<Vec<_>>(),
            ));
        }

        let direct = this.iter().zip_longest(other).map(|it| {
            match it {
        ...
        if let Ok(list) = direct {
            // A direct transformation between the two lists was doable.
            return Ok(to_transform_list(list));
        }

        // Gecko's conversions cannot cope with an empty transform list in any
        // of the two stored in the AccumulateMatrix and InterpolateMatrix
        // variants.
        let to_non_empty_transform_list = |list: &[_]| {
            TransformList(if list.is_empty() {
                Some(vec![TransformOperation::Matrix(ComputedMatrix::identity())])
            } else {
                Some(list.into())
            })
        };
        ...
        match procedure {
            ...
            Procedure::Accumulate { count } => {
                Ok(TransformList(Some(vec![TransformOperation::AccumulateMatrix {
                    from_list: to_non_empty_transform_list(this),
                    to_list: to_non_empty_transform_list(other),
    ...
    fn compute_squared_distance(&self, other: &Self) -> Result<SquaredDistance, ()> {
        match (self, other) {
            (&TransformOperation::Matrix(ref this), &TransformOperation::Matrix(ref other)) => {
                Ok(this.compute_squared_distance(&other)?)
            },
            ...
            (&TransformOperation::Scale(fx, fy, fz), &TransformOperation::Scale(tx, ty, tz)) => {
                Ok(
                    fx.compute_squared_distance(&tx)? +
                    fy.compute_squared_distance(&ty)? +
            ...
            (&TransformOperation::Rotate(fx, fy, fz, fa), &TransformOperation::Rotate(tx, ty, tz, ta)) => {
                // A direction vector that cannot be normalized, such as [0,0,0], will cause the
                // rotation to not be applied. i.e. Use an identity matrix or rotate3d(0, 0, 1, 0).
                let get_normalized_vector_and_angle = |x: f32, y: f32, z: f32, angle| {
...
impl ComputeSquaredDistance for TransformList {
    #[inline]
    fn compute_squared_distance(&self, other: &Self) -> Result<SquaredDistance, ()> {
        self.0.iter().flat_map(|l| l).zip_longest(other.0.iter().flat_map(|l| l)).map(|it| {
            match it {
                EitherOrBoth::Both(this, other) => {
                    this.compute_squared_distance(other)
                },
                EitherOrBoth::Left(filter) | EitherOrBoth::Right(filter) => {
                    filter.to_animated_zero()?.compute_squared_distance(filter)
                },
            }
        }).sum()
    }
    ...
    L: Clone + RepeatableListAnimatable,
{
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        match procedure {
            Procedure::Add | Procedure::Accumulate { .. } => {
                // Non-additive.
                return Err(())
            },
            _ => {},
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        match (self, other) {
            ...
            (&Filter::DropShadow(ref this), &Filter::DropShadow(ref other)) => {
                // FIXME: Drop shadows should be animatable in Servo too.
                Ok(Filter::DropShadow(this.animate(other, procedure)?))
            },
            (&Filter::Url(_), &Filter::Url(_)) => {
                // URLs are not animatable.
                Err(())
            },
            % endif
            _ => {
                // FIXME: If specified the different filter functions,
...
impl ToAnimatedZero for AnimatedFilter {
    fn to_animated_zero(&self) -> Result<Self, ()> {
        match *self {
            % for func in ['Blur', 'HueRotate', 'Grayscale', 'Invert', 'Sepia']:
            Filter::${func}(ref this) => {
                Ok(Filter::${func}(this.to_animated_zero()?))
            },
            % endfor
            % for func in ['Brightness', 'Contrast', 'Opacity', 'Saturate']:
            Filter::${func}(_) => {
                Ok(Filter::${func}(NonNegative(1.)))
            },
            % endfor
            % if product == "gecko":
            Filter::DropShadow(ref this) => {
                // TODO: Drop shadows should be animatable in Servo too.
                Ok(Filter::DropShadow(this.to_animated_zero()?))
            },
            % endif
            _ => {
                // URLs are not animatable.
...
impl Animate for AnimatedFilterList {
    ...
    fn animate(&self, other: &Self, procedure: Procedure) -> Result<Self, ()> {
        ...
        Ok(AnimatedFilterList(self.0.iter().zip_longest(&other.0).map(|it| {
            match it {
                EitherOrBoth::Both(this, other) => {
                    this.animate(other, procedure)
                },
...
impl ComputeSquaredDistance for AnimatedFilterList {
    #[inline]
    fn compute_squared_distance(&self, other: &Self) -> Result<SquaredDistance, ()> {
        self.0.iter().zip_longest(&other.0).map(|it| {
            match it {
                EitherOrBoth::Both(from, to) => {
                    compute_filter_square_distance(from, to)
                },
                EitherOrBoth::Left(filter) | EitherOrBoth::Right(filter) => {
                    compute_filter_square_distance(&filter.to_animated_zero()?, filter)
                },
            }
        }).sum()
    }
