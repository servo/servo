...
<%self:impl_trait style_struct_name="Border"
    ...
    pub fn copy_border_image_source_from(&mut self, _other: &Self) {
        unsafe {
    ...
    pub fn set_${shorthand}_image(&mut self,
        ...
        for (image, geckoimage) in images.0.into_iter().zip(self.gecko.${image_layers_field}
            ...
            % else:
                ...
                match image {
                    T::Image(image) => set_image(image, &mut geckoimage.mImage),
...
fn set_image(image: Image, mut geckoimage: &mut structs::nsStyleImage) {
    match image {
