                            ...
                            if let LengthOrPercentageOrKeyword::LengthOrPercentage(first_len, second_len) = length {
                                match first_len {
                                    LengthOrPercentage::Length(au) => {
                                        unsafe {
                                            (*gecko_gradient).mRadiusX.set_value(CoordDataValue::Coord(au.0));
                                        }
                                    },
                                    LengthOrPercentage::Percentage(f) => {
                                        unsafe {
                                            (*gecko_gradient).mRadiusX.set_value(CoordDataValue::Percent(f));
                                        }
                                    },
                                    LengthOrPercentage::Calc(l_p) => {
                                        unsafe {
                                            (*gecko_gradient).mRadiusX.set_value(CoordDataValue::Calc(l_p.into()));
                                        }
                                    },
                                }
                            }
