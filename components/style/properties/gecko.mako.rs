...
<%def name="impl_keyword_setter(ident, gecko_ffi_name, keyword, style='snake', cast_type='u8')">
    #[allow(non_snake_case)]
    pub fn set_${ident}(&mut self, v: longhands::${ident}::computed_value::T) {
        ...
        let result = match v {
            % for value in keyword.values_for('gecko'):
                <% new_val = to_rust_ident(value) if style == 'snake' \
                        else to_camel_case(value) if style == 'camel' else value %>
                Keyword::${new_val} => structs::${keyword.gecko_constant(value)} ${keyword.maybe_cast(cast_type)},
            % endfor
        };
