...
<%def name="impl_common_image_layer_properties(shorthand)">
    ...
    % if shorthand != "background" and shorthand != "mask":
...
<%self:impl_trait style_struct_name="SVG"
    ...
    pub fn copy_mask_position_${orientation[0]}_from(&mut self, other: &Self) {
        ...
        self.gecko.mMask.mPosition${orientation[0].upper()}Count
                = cmp::min(1, other.gecko.mMask.mPosition${orientation[0].upper()}Count);
        self.gecko.mMask.mLayers.mFirstElement.mPosition =
            other.gecko.mMask.mLayers.mFirstElement.mPosition;
        unsafe {
            Gecko_EnsureImageLayersLength(&mut self.gecko.mMask,
                                          other.gecko.mMask.mLayers.len(),
                                          LayerType::Mask);
        }
        for (layer, other) in self.gecko.mMask.mLayers.iter_mut()
                                  .zip(other.gecko.mMask.mLayers.iter()) {
            layer.mPosition.m${orientation[0].upper()}Position
                = other.mPosition.m${orientation[0].upper()}Position;
        }
        self.gecko.mMask.mPosition${orientation[0].upper()}Count
                = other.gecko.mMask.mPosition${orientation[0].upper()}Count;
    }
    ...
    pub fn clone_mask_position_${orientation[0]}(&self)
        ...
        use values::computed::position::${orientation[1]}Position;
        longhands::mask_position_${orientation[0]}::computed_value::T(
            self.gecko.mMask.mLayers.iter()
                .take(self.gecko.mMask.mPosition${orientation[0].upper()}Count as usize)
                .map(|position| ${orientation[1]}Position(position.mPosition.m${orientation[0].upper()}Position.into()))
                .collect()
        )
    }
    ...
    pub fn set_mask_position_${orientation[0]}(&mut self,
        ...
        unsafe {
          Gecko_EnsureImageLayersLength(&mut self.gecko.mMask, v.0.len(),
                                        LayerType::Mask);
        }

        self.gecko.mMask.mPosition${orientation[0].upper()}Count = v.0.len() as u32;
        for (servo, geckolayer) in v.0.into_iter().zip(self.gecko.mMask
                                                           .mLayers.iter_mut()) {
            geckolayer.mPosition.m${orientation[0].upper()}Position = servo.0.into();
        }
    }
