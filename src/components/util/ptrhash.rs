...
/// The number of bits in the "upper section" (key of the hash table).
static UPPER_BITS: u64 = 55;

/// The number of bits in the "lower section" (bitfield).
...
pub struct PtrHashSetIterator<'self> {
    hash_iter: HashMapIterator<'self,HashKey,BitSet>,
    priv value: Option<(HashKey, BitSet, u64)>,
}

impl<'self> PtrHashSetIterator<'self> {
    #[inline]
    fn pump(&mut self) {
...
impl<'self> Iterator<u64> for PtrHashSetIterator<'self> {
    #[inline]
