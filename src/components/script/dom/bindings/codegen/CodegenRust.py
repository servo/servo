...
static PrototypeClass: JSClass = Struct_JSClass {
  ...
  construct: None,
  trace: None,
  reserved: [0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void,  // 05
                    0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void,  // 10
                    0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void,  // 15
                    0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void,  // 20
                    0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void,  // 25
                    0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void, 0 as *mut libc::c_void,  // 30
                    0 as *mut libc::c_void]  // 31
};
...
class CGDefineDOMInterfaceMethod(CGAbstractMethod):
  ...
  js_info.dom_static.proxy_handlers.insert(PrototypeList::id::%s as uint,
                                           CreateProxyHandler(&traps, cast::transmute(&Class.dom_class)));
...
class CGGenericMethod(CGAbstractBindingMethod):
    ...
    def generate_code(self):
        return CGIndenter(CGGeneric(
            "let info = RUST_FUNCTION_VALUE_TO_JITINFO(JS_CALLEE(cx, &mut *vp));\n"
            "return CallJitMethodOp(info, cx, object_handle(&obj), this.unsafe_get() as *libc::c_void, argc, &mut *vp);"))
...
class CGDOMJSProxyHandler_getOwnPropertyDescriptor(CGAbstractExternMethod):
    ...
    def getBody(self):
        ...
        if indexedGetter or indexedSetter:
            setOrIndexedGet += "let index = GetArrayIndexFromId(cx, *id.unnamed_field1);\n"
        ...
        if namedGetter:
            ...
            namedGet = ("\n" +
                        "if flags & 0x02 /*SET*/ == 0 && RUST_JSID_IS_STRING(*id.unnamed_field1) != 0 && !HasPropertyOnPrototype(cx, proxy, id) {\n" +
                        "  let name = Some(jsid_to_str(cx, *id.unnamed_field1));\n" +
                        "  let this = UnwrapProxy(proxy);\n" +
                        "  let this = JS::from_raw(this);\n" +
                        "  let mut this = this.root();\n" +
        ...
        return setOrIndexedGet + """let expando: *mut JSObject = GetExpandoObject(*proxy.unnamed_field1);
...
class CGDOMJSProxyHandler_defineProperty(CGAbstractExternMethod):
    ...
    def getBody(self):
        ...
        if indexedSetter:
            ...
            set += ("let index = GetArrayIndexFromId(cx, *id.unnamed_field1);\n" +
                    "if index.is_some() {\n" +
                    "  let index = index.unwrap();\n" +
                    "  let this = UnwrapProxy(proxy);\n" +
                    "  let this = JS::from_raw(this);\n" +
            ...
            set += ("if GetArrayIndexFromId(cx, *id.unnamed_field1).is_some() {\n" +
                    "  return 0;\n" +
        ...
        if namedSetter:
            if not self.descriptor.operations['NamedCreator'] is namedSetter:
                raise TypeError("Can't handle creator that's different from the setter")
            set += ("if RUST_JSID_IS_STRING(*id.unnamed_field1) != 0 {\n" +
                    "  let name = Some(jsid_to_str(cx, *id.unnamed_field1));\n" +
                    "  let this = UnwrapProxy(proxy);\n" +
                    "  let this = JS::from_raw(this);\n" +
                    "  let mut this = this.root();\n" +
            ...
            set += ("if RUST_JSID_IS_STRING(*id.unnamed_field1) {\n" +
                    "  let name = Some(jsid_to_str(cx, *id.unnamed_field1));\n" +
                    "  let this = UnwrapProxy(proxy);\n" +
                    "  let this = JS::from_raw(this);\n" +
                    "  let mut this = this.root();\n" +
...
class CGDOMJSProxyHandler_hasOwn(CGAbstractExternMethod):
    ...
    def getBody(self):
        indexedGetter = self.descriptor.operations['IndexedGetter']
        if indexedGetter:
            indexed = ("let index = GetArrayIndexFromId(cx, *id.unnamed_field1);\n" +
                       "if index.is_some() {\n" +
                       "  let index = index.unwrap();\n" +
                       "  let this = UnwrapProxy(proxy);\n" +
                       "  let this = JS::from_raw(this);\n" +
        ...
        namedGetter = self.descriptor.operations['NamedGetter']
        if namedGetter:
            named = ("if RUST_JSID_IS_STRING(*id.unnamed_field1) != 0 && !HasPropertyOnPrototype(cx, proxy, id) {\n" +
                     "  let name = Some(jsid_to_str(cx, *id.unnamed_field1));\n" +
                     "  let this = UnwrapProxy(proxy);\n" +
                     "  let this = JS::from_raw(this);\n" +
                     "  let mut this = this.root();\n" +
        ...
        return indexed + """let expando = GetExpandoObject(*proxy.unnamed_field1);
if expando.is_not_null() {
  let mut b: JSBool = 1;
...
class CGDOMJSProxyHandler_get(CGAbstractExternMethod):
    def __init__(self, descriptor):
        ...
        self.descriptor = descriptor
    def getBody(self):
        getFromExpando = """let expando = GetExpandoObject(*proxy.unnamed_field1);
if expando.is_not_null() {
  let mut hasProp = 0;
  ...
  if hasProp != 0 {
    return JS_GetPropertyById(cx, object_handle(&expando), id, vp);
        ...
        indexedGetter = self.descriptor.operations['IndexedGetter']
        if indexedGetter:
            getIndexedOrExpando = ("let index = GetArrayIndexFromId(cx, *id.unnamed_field1);\n" +
                                   "if index.is_some() {\n" +
                                   "  let index = index.unwrap();\n" +
                                   "  let this = UnwrapProxy(proxy);\n" +
                                   "  let this = JS::from_raw(this);\n" +
...
class CGClassConstructHook(CGAbstractExternMethod):
    ...
    def generate_code(self):
        preamble = """
  let global = global_object_for_js_object(JS_CALLEE(cx, &mut *vp).to_object()).root();
...
''').substitute({'name': name}))]
    ...
    def UnionTypes(config):
        ...
        curr = CGImports(curr, [], [
            ...
            'js::glue::{CallJitMethodOp, CreateProxyHandler}',
