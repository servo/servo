...
""" % (toStringBool(descriptorProvider.workers),
    ...
    if type.isGeckoInterface():
        ...
        if descriptor.interface.isCallback():
            ...
            pointerType = '' #pointerType = descriptor.pointerType
            nullable = type.nullable() or isCallbackReturnValue or True #XXXjdm
            if nullable:
                declType = CGGeneric("Option<%s>" % (pointerType + name));
            else:
                declType = CGGeneric("%s" % (pointerType + name))
            newval = "%s::new(JSVAL_TO_OBJECT(${val}))" % name
            if nullable:
                newval = "Some(%s)" % newval
            conversion = ("  ${declName} = %s;\n" % newval)
...
class CGAbstractMethod(CGThing):
    ...
    def _template(self):
        if self.templateArgs is None:
            return ''
        return '<%s>' % ', '.join(self.templateArgs)
...
class ClassMethod(ClassItem):
    def __init__(self, name, returnType, args, inline=False, static=False,
                 ...
                 templateArgs=None, visibility='pub', body=None,
...
class ClassConstructor(ClassItem):
    ...
    def __init__(self, args, inline=False, bodyInHeader=False,
        ...
        assert len(self.baseConstructors) <= 1
        self.body = body
...
class CGClass(CGThing):
    ...
    def declare(self):
        ...
        myself += 'pub struct%s %s%s' % (self.indent, self.name, specialization)
        result += myself
...
class CGNativeMember(ClassMethod):
    def __init__(self, descriptorProvider, member, name, signature, extendedAttrs,
                 breakAfter=True, passJSBitsAsNeeded=True, visibility="pub",
    ...
    def getArgs(self, returnType, argList):
        ...
        if self.member.isMethod() and self.member.isLegacycaller():
            ...
            args.insert(0, Argument("JSVal", "aThisVal"))
        # And jscontext bits.
        if needCx(returnType, argList, self.extendedAttrs,
            ...
            if needScopeObject(returnType, argList, self.extendedAttrs,
                ...
                args.insert(1, Argument("*JSObject", "obj"))
...
class CallbackMember(CGNativeMember):
    def __init__(self, sig, name, descriptorProvider, needThisHandling):
        ...
        self.exceptionCode=("return%s;" % self.getDefaultRetval())
...
class CallbackMethod(CallbackMember):
    ...
    def getCall(self):
        ...
        return string.Template("${getCallable}"
                ...
                "                        ${argc}, ${argv}, &rval) == 0 } {\n"
                "  //XXXjdm aRv.Throw(NS_ERROR_UNEXPECTED);\n"
                "  return${errorReturn};\n"
                "}\n").substitute(replacements)
...
#""")
    ...
    def InterfaceTypes(config):
        ...
        descriptors = config.getDescriptors(register=True, hasInterfacePrototypeObject=True)
        cgDecls = [CGGeneric(declare="pub use %s::%s;\n" % (pathToType(d), d.name)) for d in descriptors]
        curr = CGList(cgDecls)
        curr = CGWrapper(curr, pre=AUTOGENERATED_WARNING_COMMENT)
        return curr
