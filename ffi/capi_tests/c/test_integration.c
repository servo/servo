...
static void wake_callback(void) {
    atomic_store(&waker_triggered, 1);
}

// WebView delegate callbacks
static void on_load_status_changed(struct WebView *webview,
                                   int32_t load_status,
                                   void *user_data) {
    (void)webview;
...
static void on_screenshot(const uint8_t *data,
                          uint32_t width,
                          uint32_t height,
                          int32_t error,
                          void *user_data);

static void on_new_frame_ready(struct WebView *webview,
                               void *user_data) {
...
int test_webview_load_and_screenshot(void) {
    ...
    ServoWebViewBuilder *webview_builder = servo_webview_builder_create(servo, ctx);
    if (!webview_builder) {
