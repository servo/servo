    ...
    unsafe {
       if copy != 0 {
           if !src.is_null() && src_len > 0 {
             ...
             ptr::copy_memory((*output).str, src, src_len as usize);
...
pub extern "C" fn cef_string_utf8_cmp(a: *const cef_string_utf8_t, b: *const cef_string_utf8_t) -> c_int {
    unsafe {
        ...
        let astr = slice::from_raw_buf(&astr, (*a).length as usize);
        let bstr = slice::from_raw_buf(&bstr, (*b).length as usize);
        match astr.cmp(bstr) {
            Ordering::Less => -1,
            Ordering::Equal => 0,
            Ordering::Greater => 1
...
pub extern "C" fn cef_string_utf16_to_utf8(src: *const u16, src_len: size_t, output: *mut cef_string_utf8_t) -> c_int {
    unsafe {
        let ustr = slice::from_raw_buf(&src, src_len as usize);
...
pub extern "C" fn cef_string_utf16_cmp(a: *const cef_string_utf16_t, b: *const cef_string_utf16_t) -> c_int {
    unsafe {
        ...
        let astr: &[u16] = slice::from_raw_buf(&astr, (*a).length as usize);
        let bstr: &[u16] = slice::from_raw_buf(&bstr, (*b).length as usize);
        match astr.cmp(bstr) {
            Ordering::Less => -1,
            Ordering::Equal => 0,
            Ordering::Greater => 1
