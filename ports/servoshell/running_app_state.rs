...
#[cfg(all(any(coverage, llvm_pgo), any(target_os = "android", target_env = "ohos")))]
use libc::c_char;
...
impl RunningAppState {
    ...
    pub(crate) fn schedule_exit(&self) {
        ...
        #[cfg(all(any(coverage, llvm_pgo), any(target_os = "android", target_env = "ohos")))]
        {
            ...
            let filename = format!("{}/profile-%h-%p.profraw", profile_path.to_str().expect("Should be unicode"));
