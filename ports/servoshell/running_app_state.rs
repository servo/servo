...
#[cfg(feature = "gamepad")]
use servo::{GamepadHapticEffectType, GamepadProvider, IpcResponder};
use url::Url;
...
impl GamepadProvider for ServoshellGamepadProvider {
    fn play_haptic_effect(
        ...
        mut responder: IpcResponder<bool>,
    ) {
        match self.gamepad_support.borrow_mut().as_mut() {
            Some(gamepad_support) => {
                gamepad_support.play_haptic_effect(
                    gamepad_index,
                    effect_type,
    ...
    fn stop_haptic_effect(
        ...
        let stopped = match self.gamepad_support.borrow_mut().as_mut() {
            Some(gamepad_support) => gamepad_support.stop_haptic_effect(gamepad_index),
            None => false,
        };
        let _ = responder.send(stopped);
    }
}
