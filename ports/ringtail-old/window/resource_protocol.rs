...
impl ResourceProtocolHandler {
    pub fn response_for_path(
        ...
        log::info!("Resource protocol handler processing path: {:?}", path);
        ...
        let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent() // steps out into /ports/
            .and_then(|p| p.parent()) // steps out into workspace root /ringtail/
            .map(|p| p.join("resources"))
            .expect("Failed to locate workspace resources directory");
