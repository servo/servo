...
/*
fn ev_ioc_g_name(len: c_int) -> c_int {
    ioc(IOC_READ, 'E' as c_int, 0x06, len)
}
*/
...
fn setup_input_device(device_path: &Path,
                      _: &Sender<Option<WindowEvent>>) {
    println!("setting up device at {}", device_path.as_str().unwrap());
/*
    let device;
...
/*
    unsafe {
        let name: [c_char, ..64];
        ioctl(device.as_raw_fd(), ev_ioc_g_name(name.len() as c_int), &name);
    }
*/
}
...
*/
    ...
    //let mut buf: Vec<linux_input_event> = Vec::with_capacity(16);
    let mut buf: [u8, ..(16 * 16)] = unsafe { zeroed() };
    let mut slots: [input_slot, ..10] = unsafe { zeroed() };
    // XXX: can't use slots.len()! complains that slots is uninitialized
    for i in range(0, 10) {
    slots[i] = input_slot {
            tracking_id: -1,
            x: -1,
...
/*
        let read = match device.read(buf.as_mut_slice()) {
            Ok(len) => len,
            Err(msg) => {
                println!("Couldn't read device! {}", msg);
                return;
            },
        };
*/
        ...
        if read % (size_of::<linux_input_event>() as i32) != 0 {
            panic!("Unexpected input device read length!");
        }
            ...
            match event.evt_type {
                EV_SYN => {
                    match event.code {
                        ...
                        _ => {
                           println!("Unhandled EV_SYN code {}", event.code);
                        },
                    }
                },
                EV_ABS => {
                    match event.code {
                        ...
                        _ => {
                            println!("Unhandled EV_ABS code {}", event.code);
                        },
                    }
                },
                _ => {
                    println!("Unhandled event type - {}", event.evt_type);
                },
            }
        }
    }
...
pub fn run_input_loop(event_sender: &Sender<Option<WindowEvent>>) {
    ...
    spawn(proc () {
        let inputdir = Path::new("/dev/input/");
        let devices = fs::readdir(&inputdir);
        match devices {
            Ok(vec) => {
                for device_path in vec.iter() {
                    setup_input_device(device_path, &sender);
                }
            },
            Err(msg) => {
                println!("Failed to read /dev/input! {}", msg);
                return
            }
        }
