...
use libc::c_int;
use msg::compositor_msg::{Blank, IdleRenderState};
use msg::compositor_msg::{ReadyState, RenderState};
use std::cell::Cell;
...
impl GonkNativeWindowBuffer {
    pub fn new(dev: *mut alloc_device, width: i32, height: i32, format: c_int, usage: c_int) -> *mut GonkNativeWindowBuffer {
        ...
        if ret != 0 { panic!("Failed to allocate gralloc buffer!") }
...
impl Window {
    ...
    pub fn new() -> Rc<Window> {
        ...
        unsafe {
            ...
            if ret != 0 { panic!("Failed to get HWC module!") }
        }
        ...
        unsafe {
            ...
            if ret != 0 { panic!("Failed to get HWC device!") }
            hwc_device = transmute(device);
            if (*hwc_device).common.version <= (1 << 8) { panic!("HWC too old!") }
        }

        let attrs: [u32, ..4] = [2, 3, 4, 0];
        let mut values: [i32, ..4] = [0, 0, 0, 0];
        unsafe {
            let _ = ((*hwc_device).getDisplayAttributes)(hwc_device, 0, 0, attrs.as_ptr(), values.as_mut_ptr());
            //if ret != 0 { panic!("Failed to get HWC attributes!") }
        }
        ...
        unsafe {
            ...
            if ret1 != 0 { panic!("Failed to get gralloc moudle!") }

            let ret2 = "gpu0".with_c_str(|s| ((*(*gralloc_mod).methods).open)(gralloc_mod, s, &mut device));
            if ret2 != 0 { panic!("Failed to get gralloc moudle!") }
        ...
        if ret1 != 1 { panic!("Failed to initialize EGL!") }
        ...
        if ret2 != 1 { panic!("Failed to choose a config") }
        ...
        if ret3 != 1 { panic!("Failed to make current!") }
