...
/// Preferences enabled when ringtaill is launched with the `--enable-experimental-web-platform-features` flag.
///
...
#[derive(Clone)]
pub(crate) struct RingtailPreferences {
    /// A URL to load when starting ringtaill.
    ...
    /// Whether or not to run ringtaill in headless mode. While running in headless
...
impl Default for RingtailPreferences {
    fn default() -> Self {
        Self {
            clean_shutdown: false,
            device_pixel_ratio_override: None,
            headless: false,
            homepage: "https://voxelite.neocities.org/ringtail/".into(),
            initial_window_size: Size2D::new(1024, 740),
            no_native_titlebar: true,
            screen_size_override: None,
            simulate_touch_events: false,
            searchpage: "https://voxelite.neocities.org/Service?q=%s".into(),
            tracing_filter: None,
            url: None,
...
pub(crate) enum ArgumentParsingResult {
    ChromeProcess(Opts, Preferences, RingtailPreferences),
...
#[bpaf(options, version(VERSION), usage("ringtaill [OPTIONS] URL"))]
...
struct CmdArgs {
    ...
    #[bpaf(positional("URL"), fallback(String::from("https://voxelite.neocities.org/ringtail/")))]
    url: String,
}
...
fn parse_arguments_helper(args_without_binary: Args) -> ArgumentParsingResult {
    ...
    let ringtaill_preferences = RingtailPreferences {
    ...
    ArgumentParsingResult::ChromeProcess(opts, preferences, ringtaill_preferences)
}
...
fn parse_diagnostics_logging(cli_options: Vec<String>) -> Result<DiagnosticsLogging, ()> {
    ...
    if cli_options.contains(&"help".into()) {
        ...
        println!("Usage: ringtaill -Z option,[option,...]\n\twhere options include:");
...
fn test_create_prefs_map() {
    let json_str = "{
        ...
        \"shell.homepage\": \"https://voxelite.neocities.org/ringtail/\"
    }";
    assert_eq!(read_prefs_map(json_str).len(), 3);
}

#[cfg(test)]
fn test_parse(arg: &str) -> (Opts, Preferences, RingtailPreferences) {
    ...
    match parse_command_line_arguments(args_split.as_slice()) {
        ...
        ArgumentParsingResult::ChromeProcess(opts, preferences, ringtaill_preferences) => {
            (opts, preferences, ringtaill_preferences)
        },
