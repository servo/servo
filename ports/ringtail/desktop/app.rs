...
#[cfg(feature = "gamepad")]
use crate::running_app_state::RingtailGamepadDelegate;
use crate::window::{PlatformWindow, RingtailWindowId};
...
pub struct App {
    ...
    servoshell_preferences: RingtailPreferences,
...
impl App {
    pub fn new(
        opts: Opts,
        preferences: Preferences,
        servo_shell_preferences: RingtailPreferences,
    ...
    pub fn init(&mut self, active_event_loop: Option<&ActiveEventLoop>) {
        ...
        let _ =
            protocol_registry.register("ringtail", protocols::servo::ServoProtocolHandler::default());
        let _ = protocol_registry.register(
            "resource",
        ...
        let running_state = Rc::new(RunningAppState::new(
            ...
            #[cfg(feature = "gamepad")]
            RingtailGamepadDelegate::maybe_new().map(Rc::new),
        ));
...
impl ApplicationHandler<AppEvent> for App {
    ...
    fn window_event(
        ...
        if let Some(window) = state.window(RingtailWindowId::from(u64::from(window_id))) &&
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, app_event: AppEvent) {
        ...
        if let Some(window) = app_event
            .window_id()
            .and_then(|window_id| state.window(RingtailWindowId::from(u64::from(window_id)))) &&
            let Some(headed_window) = window.platform_window().as_headed_window()
        {
