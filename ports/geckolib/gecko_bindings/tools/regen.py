...
COMPILATION_TARGETS = {
  ...
  "structs": {
    ...
    "match_headers": [
      ...
      "CSSVariableValues.h", "nsFont.h", "nsTHashtable.h", "PLDHashTable.h",
      "nsColor.h", "nsStyleStruct.h", "nsStyleCoord.h", "RefPtr.h",
      "nsISupportsImpl.h", "gfxFontConstants.h", "gfxFontFamilyList.h",
      "gfxFontFeatures.h", "imgRequestProxy.h", "nsIRequest.h",
      "imgIRequest.h", "CounterStyleManager.h", "nsStyleConsts.h",
      "nsCSSValue.h", "SheetType.h", "nsIPrincipal.h", "nsDataHashtable.h",
      "nsCSSScanner.h", "Types.h", "utility", "nsTArray", "pair",
      "SheetParsingMode.h", "StaticPtr.h", "nsProxyRelease.h"
    ],
    "blacklist": [
      "IsDestructibleFallbackImpl", "IsDestructibleFallback",
      "nsProxyReleaseEvent", "FallibleTArray", "nsTArray_Impl",
      "__is_tuple_like_impl", "tuple_size", "tuple", "__make_pair_return_impl",
      "__make_pair_return", "tuple_element", "_Itup_cat"
    ],
    "opaque_types": [
  ...
  "bindings": {
    ...
    "structs_types": [
      ...
      "nsStyleEffects", "nsStyleImage", "nsStyleGradient", "nsStyleCoord",
      "nsStyleGradientStop",

      "SheetParsingMode", "nsMainThreadPtrHandle", "nsMainThreadPtrHolder",
      "nscolor", "nsFont", "FontFamilyList", "FontFamilyType", "nsIAtom",
    ],
  }
}
...
def extend_object(obj, other):
  ...
  if not isinstance(obj, dict) or not isinstance(other, dict):
...
def build(objdir, target_name, kind_name=None,
  ...
  assert ((kind_name is None and "build_kinds" not in current_target)
          or (kind_name in current_target["build_kinds"]))
  ...
  if current_target.get("test", False) and not skip_test:
    ...
    output = None
    try:
      output = subprocess.check_output(["rustc", output_filename, "--test", "-o", tests_file.name], stderr=subprocess.STDOUT)
      output = output.decode('utf8')
    except subprocess.CalledProcessError as e:
      print("FAIL\n", e.output.decode('utf8'))
      return 1
...
def main():
  ...
  parser.add_argument('--target', help='The target to build, either "structs" or "bindings"')
  parser.add_argument('--kind',   help='Kind of build')
  parser.add_argument('--bindgen', help='Override bindgen binary')
  parser.add_argument('--output', '-o', help='Output of the script')
  parser.add_argument('--skip-test', action='store_true', help='Skip automatic tests, useful for debugging')
