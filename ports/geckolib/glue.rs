...
fn with_arc_stylesheet<F>(raw: *mut ServoArcStyleSheet, cb: F) where F: FnOnce(&Arc<Stylesheet>) {
...
pub extern "C" fn Servo_AppendStyleSheet(raw_sheet: *mut ServoArcStyleSheet,
    ...
    with_arc_stylesheet(raw_sheet, |sheet| {
        data.stylesheets.retain(|x| *x != *sheet);
...
pub extern "C" fn Servo_StyleSheetHasRules(raw_sheet: *mut ServoArcStyleSheet) -> ::libc::c_int {
    let mut has_rules: ::libc::c_int = 1;
    with_arc_stylesheet(raw_sheet, |sheet| if sheet.rules.is_empty() { has_rules = 0 });
    has_rules
}
