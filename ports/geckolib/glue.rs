...
pub extern "C" fn Servo_ReparentStyle(
    ...
    if let Some(element) = element {
        // NOTE(emilio): This relies on element.is_none() == pseudo.is_none(),
        // which the caller guarantees, fortunately. But this doesn't handle the
        // IS_ROOT_ELEMENT flag correctly! I think it's not possible to wrap a
        // root element in a first-line frame, so this may be fine.
        if element.is_link() {
