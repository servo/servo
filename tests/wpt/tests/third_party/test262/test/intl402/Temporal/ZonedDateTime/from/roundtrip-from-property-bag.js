...
for (const [calendar, year, month, monthCode, day, era, eraYear, descr] of cases) {
  const dateFromYearMonth = Temporal.ZonedDateTime.from({ year, month, day, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
  ...
  const dateFromYearMonthCode = Temporal.ZonedDateTime.from({ year, monthCode, day, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
  TemporalHelpers.assertPlainDateTime(
  ...
  const dateFromEraMonth = Temporal.ZonedDateTime.from({ era, eraYear, month, day, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
  TemporalHelpers.assertPlainDateTime(
    dateFromEraMonth.toPlainDateTime(),
    year, month, monthCode, day, 12, 34, 0, 0, 0, 0,
    `${descr} (${calendar}) - created from era, era year, and month`,
    era, eraYear);

  const dateFromEraMonthCode = Temporal.ZonedDateTime.from({ era, eraYear, monthCode, day, hour: 12, minute: 34, timeZone: "UTC", calendar }, options);
  TemporalHelpers.assertPlainDateTime(
    dateFromEraMonthCode.toPlainDateTime(),
    year, month, monthCode, day, 12, 34, 0, 0, 0, 0,
