...
for (const [calendar, year, month, monthCode, day, era, eraYear, descr] of year2000Cases) {
  const string = `2000-01-01T12:34Z[UTC][u-ca=${calendar}]`;
  const dateFromString = Temporal.ZonedDateTime.from(string);
  TemporalHelpers.assertPlainDateTime(
    dateFromString.toPlainDateTime(),
...
for (const [calendar, year, month, monthCode, day, era, eraYear, descr] of year1Cases) {
  const string = `0001-01-01T12:34Z[UTC][u-ca=${calendar}]`;
  const dateFromString = Temporal.ZonedDateTime.from(string);
  TemporalHelpers.assertPlainDateTime(
    dateFromString.toPlainDateTime(),
    year, month, monthCode, day, 12, 34, 0, 0, 0, 0,
