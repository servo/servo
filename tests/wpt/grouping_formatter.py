...
class GroupingFormatter(base.BaseFormatter):
    ...
    @staticmethod
    def get_lines_for_unexpected_result(test_name, status, expected, message, stack):
        ...
        test_name = test_name.encode('utf-8').encode('string-escape')
    ...
    def get_output_for_unexpected_subtests(self, test_name, unexpected_subtests):
        ...
        def add_subtest_failure(lines, subtest, stack=None):
            lines += type(self).get_lines_for_unexpected_result(
        ...
        def make_subtests_failure(test_name, subtests, stack=None):
            ...
            return type(self).wrap_and_indent_lines(lines, "  ") + "\n"
    ...
    def test_end(self, data):
        ...
        if test_status == "CRASH":
            stack = self.test_output[test_name].get() + data.get('stack', "")
        else:
        ...
        if had_unexpected_test_result:
            ...
            lines = type(self).get_lines_for_unexpected_result(
                ...
                stack)
            output += type(self).wrap_and_indent_lines(lines, "  ") + "\n"
