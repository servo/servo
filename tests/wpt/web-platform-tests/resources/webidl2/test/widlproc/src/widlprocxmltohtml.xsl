        ...
        <xsl:template match="Exception|Valuetype|Const">
            <xsl:if test="descriptive">
                <xsl:message terminate="yes">element
                    <xsl:value-of select="name()" /> not supported</xsl:message>
            </xsl:if>
        </xsl:template>
        ...
        <xsl:template match="Argument">
            ...
            <xsl:choose>
                <xsl:when test="$nodesc">
                    ...
                    <xsl:if test="@in">
                        <xsl:value-of select="concat(@in, ' ')" /></xsl:if>
                    <xsl:if test="@optional">
                        <xsl:value-of select="concat(@optional, ' ')" /></xsl:if>
                    <xsl:apply-templates select="Type" />
    ...
    and that interface contains just one attribute.-->
        <xsl:template match="def-instantiated">
            ...
            <p>
                ...
                <code><xsl:apply-templates select="../../Attribute/Type"/></code> is instantiated, and the resulting object appears in the global namespace as
                        ...
                        <tr>
                            ...
                            <td>
                                <xsl:for-each select="Operation">
                                    ...
                                    <xsl:apply-templates select="Type" />
                                    <xsl:text> </xsl:text>
                                    <a href="#{concat(@name,generate-id(.))}">
                                        <xsl:value-of select="@name" />
                                    </a>
                                    <xsl:text>(</xsl:text>
                                    <xsl:for-each select="ArgumentList/Argument">
                                        <xsl:variable name="type">
                                            <xsl:apply-templates select="Type" /></xsl:variable>
