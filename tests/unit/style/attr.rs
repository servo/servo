...
fn test_length_calc() {
    ...
    assert_eq!(calc.calculate(Some(Au(10))), Some(Au(12)));
    assert_eq!(calc.calculate(Some(Au(0))), Some(Au(10)));
    assert_eq!(calc.calculate(None), None);

    let calc = CalcLengthOrPercentage { length: Au(10), percentage: None };
    assert_eq!(calc.calculate(Some(Au(0))), Some(Au(10)));
    assert_eq!(calc.calculate(None), Some(Au(10)));
}
