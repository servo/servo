...
fn test_text_emphasis_position() {
    ...
    assert_eq!(over_right, SpecifiedValue::Value(HorizontalValue::Over, VerticalValue::Right));

    let over_left = parse_longhand!(text_emphasis_position, "over left");
    assert_eq!(over_left, SpecifiedValue::Value(HorizontalValue::Over, VerticalValue::Left));

    let under_right = parse_longhand!(text_emphasis_position, "under right");
    assert_eq!(under_right, SpecifiedValue::Value(HorizontalValue::Under, VerticalValue::Right));

    let under_left = parse_longhand!(text_emphasis_position, "under left");
    assert_eq!(under_left, SpecifiedValue::Value(HorizontalValue::Under, VerticalValue::Left));

    let right_over = parse_longhand!(text_emphasis_position, "right over");
    assert_eq!(right_over, SpecifiedValue::Value(HorizontalValue::Over, VerticalValue::Right));

    let left_over = parse_longhand!(text_emphasis_position, "left over");
    assert_eq!(left_over, SpecifiedValue::Value(HorizontalValue::Over, VerticalValue::Left));

    let right_under = parse_longhand!(text_emphasis_position, "right under");
    assert_eq!(right_under, SpecifiedValue::Value(HorizontalValue::Under, VerticalValue::Right));

    let left_under = parse_longhand!(text_emphasis_position, "left under");
    assert_eq!(left_under, SpecifiedValue::Value(HorizontalValue::Under, VerticalValue::Left));
}
