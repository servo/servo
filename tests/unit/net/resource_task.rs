...
#[test]
fn test_cancelled_listener_should_cause_error() {
    let resource_task = new_resource_task("".to_owned(), None);
    let (sender, receiver) = ipc::channel().unwrap();
    let (id_sender, id_receiver) = ipc::channel().unwrap();
    let url = Url::parse("foo://bar.com").unwrap();
    resource_task.send(ControlMsg::Load(LoadData::new(url, None), LoadConsumer::Channel(sender), Some(id_sender))).unwrap();
