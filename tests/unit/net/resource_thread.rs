...
use net_traits::{ResourceMsg, LoadData, LoadConsumer, LoadContext, NetworkError, ProgressMsg};
...
fn test_exit() {
    ...
    resource_thread.send(ResourceMsg::Exit).unwrap();
}
...
fn test_bad_scheme() {
    ...
    resource_thread.send(ResourceMsg::Load(LoadData::new(LoadContext::Browsing, url, None, None, None),
...
fn test_cancelled_listener() {
    ...
    resource_thread.send(ResourceMsg::Cancel(res_id)).unwrap();
    // synchronize with the resource_thread loop, so that we don't simply send everything at once!
    resource_thread.send(ResourceMsg::Synchronize(sync_sender)).unwrap();
