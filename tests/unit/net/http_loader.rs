...
fn test_when_cookie_set_marked_httpsonly_secure_isnt_sent_on_http_request() {
    ...
    let sec_url = Url::parse("https://example.com").unwrap();
...

    {
        ...
        let cookie = Cookie::new_wrapped(
            CookiePair::new("mozillaIs".to_owned(), "theBest; Secure;".to_owned()),
            &cookie_url,
            CookieSource::HTTP
        ).unwrap();
        cookie_jar.push(cookie, CookieSource::HTTP);
    ...
    assert_cookie_for_domain(cookie_jar.clone(), "https://example.com", "mozillaIs=theBest; Secure;");
