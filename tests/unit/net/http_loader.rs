...
fn assert_referer_header_matches(origin_info: &LoadOrigin, request_url: &str, expected_referrer: &str) {
...
impl<'a> LoadOrigin for NoReferrer<'a> {
    fn referrer_url(&self) -> Option<Url> {
    Some(Url::parse(self.referrer_url).unwrap())
    }
fn referrer_policy(&self) -> Option<ReferrerPolicy> {
