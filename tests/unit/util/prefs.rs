...
fn test_default_config_dir_create_read_write() {
  ...
  \"shell.homepage\": \"https://google.com\"\
    ...
    config_path.push("my_config.json");
    if let Ok(ref mut fd) = File::create(&config_path) {
      fd.write_all(json_str.as_bytes());
    }

    if let Ok(ref mut fd) = File::open(&config_path) {
    fd.read_to_string(&mut expected_json);
    }
