    ...
    for idx, line in enumerate(lines):
        ...
        if current_table == "ignore" and invalid_dirs:
            for d in invalid_dirs:
                if any(fd in line for fd in ["'{}'".format(d), '"{}"'.format(d)]):
                    yield config_file, idx + 1, "invalid listed ignored directory '%s'" % d
        ...
        if current_table == "ignore" and invalid_files:
            for f in invalid_files:
                if any(ff in line for ff in ["'{}'".format(f), '"{}"'.format(f)]):
                    yield config_file, idx + 1, "invalid listed ignored file '%s'" % f
