...
def check_directory_files(only_changed_files):
    for directory, file_extensions in config['check_ext'].items():
        ...
        for filename in files:
            if not any(filename.endswith(ext) for ext in file_extensions):
                ...
                message = "Unexpected extension found for {name}. We only expect {ext} in {dir_name}".format(**details)
...
def scan(only_changed_files=False, progress=True):
    ...
    directory_errors = check_directory_files(only_changed_files)
    # standard checks
