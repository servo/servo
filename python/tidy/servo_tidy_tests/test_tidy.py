...
class CheckTidiness(unittest.TestCase):
    ...
    def test_directory_checks(self):
        ...
        with mock.patch.dict('servo_tidy.tidy.config', mock_config):
            ...
            self.assertEqual("Unexpected extension found for test.rs. We only expect webidl, test in {0}".format(error_dir), errors.next()[2])
            self.assertEqual("Unexpected extension found for test2.rs. We only expect webidl, test in {0}".format(error_dir), errors.next()[2])
            self.assertNoMoreErrors(errors)
