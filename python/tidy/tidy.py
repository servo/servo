...
def run_cargo_deny_lints():
    ...
    result = subprocess.run(["cargo-deny",
                             "--format=json",
                             "--all-features",
                             "check",
                             "licenses",
    ...
    for line in result.stderr.splitlines():
        ...
        # by the configuration file.
            ...
            label = error_fields["labels"][0]
            errors.append((
                CARGO_LOCK_FILE,
                1,
                f"{error_fields['message']}:\n {label['span']}"
            ))
            crate = CargoDenyKrate(error_fields["graphs"][0])
        # This detects if an exception has been added for a license or a duplicated crate,
        # but that exception wasn't used.
        elif error_code in ("license-exception-not-encountered", "unnecessary-skip"):
            label = error_fields["labels"][0]
            errors.append((
                CARGO_DENY_CONFIG_FILE,
                label["line"],
                f"{error_fields['message']}: {label['span']}"
            ))
        elif error_severity == "error":
            errors.append((CARGO_LOCK_FILE, 1, error_fields["message"]))
