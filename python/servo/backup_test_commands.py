...
class MachCommands(CommandBase):
    ...
    def test_webgl(self, release, log_raw, test_name=None):
        ...
        test_dir = path.join(webgl_dir, 'conformance', 'attribs')
        ...
        class CommandRunner(object):
            def __init__(self, file):
                self.file = file
                self.cmd = ['./mach', 'run', version, file, '-o', '/dev/null']
        ...
        for group in chunker(files, 4):
            command_runners = [CommandRunner(file) for file in group]
            ...
            for file in group:
                ...
                try:
                    with open(meta_file) as meta_file:
                        expected_outputs[file] = meta_file.read()
                except Exception:
                    expected_outputs[file] = ''

            # Give the threads a 10 second timeout, then terminate them
            sleep(10)
            for runner in command_runners:
                output = runner.terminate()

                print('got output for file %s: %s' % (file_index, file))
                file_index += 1

                if expected_outputs[file] != output:
                    results[file] = output
