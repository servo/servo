...
# linux_standalone.py
#
# This file is a bit special compared to the other platforms, since it can be executed either standalone
# or via mach (from `linux.py`). This allows us to pre-install dependencies in devcontainers, without
# needing to add all of mach to the container build context.
# This also makes it much simpler to test if the package installation still works.
# Do not add any dependencies on other mach files, or on python modules that are not shipped with python.
...
def linux_platform_bootstrap(distro: str, force: bool, yes: bool) -> bool:
    if distro.lower() == "nixos":
    ...
    if distro.lower() not in [
        ...
        print(
            f"mach bootstrap does not support {distro}."
    ...
    installed_something = _install_non_gstreamer_dependencies(distro, force, yes)
    return installed_something


def _install_non_gstreamer_dependencies(distro: str, force: bool, yes: bool = False) -> bool:
    def check_sudo() -> bool:
    ...
    pkgs = []
    command = []
    if distro in ["Ubuntu", "Debian GNU/Linux", "Raspbian GNU/Linux"]:
        ...
        if subprocess.call(["dpkg", "-s"] + pkgs, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE) != 0:
            install = True
    elif distro in ["CentOS", "CentOS Linux", "Fedora", "Fedora Linux", "Fedora Linux Asahi Remix"]:
        command = ["dnf", "install"]
...
if __name__ == "__main__":
    ...
    linux_platform_bootstrap(distro=distro_name(), force=force, yes=yes)
