...
class PackageCommands(CommandBase):
    ...
    def package(self, release=False, dev=False, android=None, debug=False, debugger=None):
        ...
        if android:
            ...
            vesion_command = [binary_path, '--version']
            p = subprocess.Popen(vesion_command, stdout=subprocess.PIPE, universal_newlines=True)
            version, err = p.communicate()
            if err:
                print("Error occurred when getting Servo version: " + err)
                return 1
