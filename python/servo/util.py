...
def append_paths_to_env(env: os._Environ, key: str, paths: Union[str, List[str]]):
    if isinstance(paths, list):
        paths = os.pathsep.join(paths)

    existing_value = env.get(key, None)
    if existing_value:
        new_value = existing_value + os.pathsep + paths
    else:
        new_value = paths
    env[key] = new_value


def prepend_paths_to_env(env: os._Environ, key: str, paths: Union[str, List[str]]):
    if isinstance(paths, list):
        paths = os.pathsep.join(paths)
    ...
    new_value = paths
    if existing_value:
        new_value += os.pathsep + existing_value
    env[key] = new_value
