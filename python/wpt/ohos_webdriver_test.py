...
class OHOSWebDriverController:
    ...
    def compare_with_expectations(self, test_path: str, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        has_expected_failures = len(analysis["expected_failures"]) > 0
        has_subtest_expectations = len(expectations["subtest_expectations"]) > 0
        has_expectations_file = analysis["has_expectations"]

        # For tests without expectation files, everything is expected to PASS
        # Any failure (test level or subtest level) is unexpected
        if not has_expectations_file:
            if has_unexpected_subtests:
                # Subtests failed but should have passed (no expectations = expect all to pass)
                analysis["summary"] = "UNEXPECTED_SUBTEST_RESULTS"
            elif not analysis["test_matches_expectation"]:
                # Test failed but should have passed
                if mapped_actual == "PASS":
                    # This shouldn't happen since test_expected defaults to PASS
                    analysis["summary"] = "EXPECTED"
                else:
                    # Test failed when it should have passed
                    analysis["summary"] = "UNEXPECTED_FAIL"
            else:
                # Test passed as expected (no expectation file = expect PASS)
                analysis["summary"] = "EXPECTED"
        elif has_unexpected_subtests:
            analysis["summary"] = "UNEXPECTED_SUBTEST_RESULTS"
        elif not analysis["test_matches_expectation"]:
            # Test level expectation mismatch
            if expected_status == "FAIL" and mapped_actual == "PASS":
                analysis["summary"] = "UNEXPECTED_PASS"
            elif expected_status == "PASS" and mapped_actual == "FAIL":
                analysis["summary"] = "UNEXPECTED_FAIL"
            else:
                analysis["summary"] = "UNEXPECTED"
        else:
            # Test matches expectation at the test level
            if has_subtest_expectations and has_expected_failures and not has_unexpected_subtests:
                # Test failed as expected with all subtest failures being expected
                analysis["summary"] = "EXPECTED"
            elif not has_subtest_expectations:
                # Simple test with no subtests, matches expectation
                analysis["summary"] = "EXPECTED"
            elif has_expected_failures and not has_unexpected_subtests:
                # All failing subtests were expected
                analysis["summary"] = "EXPECTED"
            else:
                # Test level matches but something unexpected in subtests
                analysis["summary"] = "EXPECTED"
    ...
    def print_test_summary(self, results: List[Dict[str, Any]]) -> None:
        ...
        for result in results:
            ...
            if summary in expectation_counts:
                ...
                expectation_counts["UNEXPECTED"] += 1
