...
fun getSubTargetDir(debug: Boolean, arch: String): String {
  val buildTypeDirectory =
      System.getenv("SERVO_TARGET_DIR")?.let { File(it).name } ?: if (debug) "debug" else "release"
...
fun Project.getNdkDir(): String {
  ...
  if (ndkRoot == null) {
    ...
    localProperties.inputStream().use { instr -> properties.load(instr) }
