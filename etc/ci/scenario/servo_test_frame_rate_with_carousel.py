...
def operator():
    ...
    print("Click 'Categories'.", file=sys.stderr)
    ...
    print("Searching for 'first_good'...", file=sys.stderr)
    first_good = driver.find_element(By.CSS_SELECTOR, "#goodsGroup > uni-view:nth-child(1) > img")
    print("Clicking 'first_good'.", file=sys.stderr)
    first_good.click()

    time.sleep(5)

    # Step 5. Click for details
    print("Searching for 'first_good_details'...", file=sys.stderr)
    ...
    print("Clicking 'first_good_details'.", file=sys.stderr)
    content.click()
    time.sleep(5)
