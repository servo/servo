...
def operator(driver: webdriver = None):
    ...
    with NonBlockingMemoryLogging(memory_logging_options) as memory_logging:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--local",
        action="store_true",
        help="run the scenario locally",
    )
    args = parser.parse_args()
    if args.local:
        common_function_for_local.run_test(operator, "open_servo_org", session_history_max_length=0)
    else:
