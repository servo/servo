...
def operator():
    ...
    if driver is not False:
        ...
        subprocess.run(cmd, capture_output=True, text=True, timeout=10)
        time.sleep(5)
        birthday_ = driver.find_element(By.CSS_SELECTOR, "#app > uni-app > uni-page > uni-page-wrapper > uni-page-body > uni-view > uni-view.idx-swiper.m-bgWhite.m-main > uni-scroll-view:nth-child(1) > div > div > div > uni-view:nth-child(4) > uni-view:nth-child(2)")
        birthday_.click()
        time.sleep(10)

        # Step 4. Catch trace
        process = subprocess.Popen("hdc shell hitrace -b 81920 -t 10 nweb ace app ohos zimage zmedia zcamera zaudio ability distributeddatamgr graphic sched freq irq sync mmc rpc workq idle disk pagecache memreclaim binder -o /data/local/tmp/my_trace.html", stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        time.sleep(4)
