...
def operator():
    ...
    cmd = 'hdc file recv /data/local/tmp/my_trace.html {}'.format(os.path.join(os.path.dirname(__file__), 'traces', 'my_trace.html'))
    subprocess.run(cmd, capture_output=True, text=True, timeout=10)

    # 4. Calculate frame rate
    frame_rate = commonFunction.calcuteFrameRate(os.path.join(os.path.dirname(__file__), 'traces', 'my_trace.html'))
