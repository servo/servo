...
- a visible native Winit window titled `Severin`
- a window-backed Servo rendering context sized from the requested `width` and `height`
...
`App.pump()` runs one bounded owner-thread turn: it pumps pending native Winit window events, lets Servo process its in-process work, paints/presents redraws for the visible document UI, collects completed bridge evaluations, schedules one top-level-document outbound-drain evaluation when needed, and schedules or completes reply delivery. Python hosts should call it repeatedly while they want the embedded page and bridge to make progress. It does not start a background runtime, worker thread, helper process, daemon, socket, callback thread, framebuffer protocol, PNG presentation path, or second GUI toolkit.
...
The normal user-facing Severin mode is a visible native window owned by `severin.App`. Servo renders the document UI that the person sees in that window; page JavaScript owns the visible application interface, Python owns policy/backend semantics, and Rust owns only Servo lifetime plus the private receipt routing between them.

The bridge is not a split-presentation channel. Severin does not export frames to Python as the central API, does not route presentation through PNGs or framebuffer messages, does not add GTK or another GUI toolkit, and does not spawn `servoshell` or call `servoshell::main()`. The bounded Python-controlled `App.pump()` method remains the owner-thread integration point for native-window events, Servo progress, paint/present work, and bridge delivery.
